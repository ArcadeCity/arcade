...
const LINE_VERTEX_ATTRIBUTES_NUV: VertexDescriptor = {
    attributes: [
        ...LINE_VERTEX_ATTRIBUTES.attributes,
...
const HP_LINE_VERTEX_ATTRIBUTES_NUV: VertexDescriptor = {
    ...
    stride:
        HP_LINE_VERTEX_ATTRIBUTES.stride + NORMAL_UV_VERTEX_ATTRIBUTES.stride,
}
...
function getVertexDescriptor(
    hasNormalsAndUvs: boolean,
    highPrecision: boolean
): VertexDescriptor {
    if (highPrecision) {
        return hasNormalsAndUvs
            ? HP_LINE_VERTEX_ATTRIBUTES_NUV
            : HP_LINE_VERTEX_ATTRIBUTES
    } else {
        return hasNormalsAndUvs
            ? LINE_VERTEX_ATTRIBUTES_NUV
...
export function createLineGeometry(
    ...
    const vertexColors =
        colors !== undefined && colors.length && polyline.length
    ...
    for (let i = 0; i < pointCount; ++i) {
        ...
        const T1 =
            isClosed && i === 0 ? tangents.length - 3 : Math.max(0, i - 1) * 3
        const T2 =
            isClosed && i === pointCount - 1
                ? 0
                : Math.min(i * 3, tangents.length - 3)
...
export class LineGroup {
    ...
    get vertexAttributes(): VertexAttributeDescriptor[] {
        return getVertexDescriptor(this.hasNormalsAndUvs, this.highPrecision)
            .attributes
    }
...
function computeBitangent(
    ...
    if (!t0.equals(t1)) {
        angle =
            Math.acos(t0.dot(t1)) * Math.sign(n.dot(tmpV.copy(t0).cross(t1)))
