...
import {
    BufferAttribute, BufferGeometry, Camera, Float32BufferAttribute,
    InterleavedBuffer, InterleavedBufferAttribute, Matrix4,
    PointsMaterialParameters, ShaderMaterial, Vector3
} from 'three'

import {
    HighPrecisionLineMaterial, HighPrecisionLineMaterialParameters,
    HighPrecisionPointMaterial, isHighPrecisionPointMaterial
} from '@arca/materials'
...
export namespace HighPrecisionUtils {
     ...
     */
    export interface HighPrecisionLineParams
        extends HighPrecisionLineMaterialParameters {
        /**
    ...
    export function createHighPrecisionCameraPos(
        ...
        const eyePos = new Vector3(0, 0, 0).applyMatrix4(
            objectInverseWorldMatrix
        )
    ...
    export function updateHpUniforms(
        ...
        if (shaderMaterial !== undefined && shaderMaterial.isMaterial) {
            if (
                ...
                shaderMaterial.uniforms.u_mvp.value = new Float32Array(
                    mvp.elements
                )
                shaderMaterial.uniforms.u_eyepos.value = new Float32Array(
                    highPrecisionCameraInfo.eyePosHi.toArray()
                )
                shaderMaterial.uniforms.u_eyepos_lowpart.value =
                    new Float32Array(highPrecisionCameraInfo.eyePosLo.toArray())
            } else {
    ...
    export function setPositions(
        ...
        object.bufferGeometry.setAttribute(
            'positionLow',
            attributes.positionLow
        )
    ...
    export function createLine(
        ...
        const addCircles =
            params.addCircles !== undefined ? params.addCircles : false
        const wireFrame =
            params.wireFrame !== undefined ? params.wireFrame : false
        ...
        const positionLowAttribute = new InterleavedBufferAttribute(
            buffer,
            3,
            3,
            false
        ...
        const lineObject = wireFrame
            ? new HPL.HighPrecisionWireFrameLine(
                  hpLineGeometry,
                  hpSolidMaterial
              )
    ...
    export function createPoints(
        ...
        const hpPointsMaterial = isHighPrecisionPointMaterial(
            materialParameters
        )
            ? materialParameters
            : new HighPrecisionPointMaterial(materialParameters)

        const pointsObject = new HPP.HighPrecisionPoints(
            hpPointsGeometry,
            hpPointsMaterial
        )
