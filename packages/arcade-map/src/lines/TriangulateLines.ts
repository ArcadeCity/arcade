...
export function triangulateLine(
    ...
    if (startWithCircle) {
        ...
        const lineAngle =
            points.length !== 3
                ? angleVec
                      .set(points[3] - points[0], points[4] - points[1])
                      .angle()
                : 0
    ...
    for (let i = 0; i < N; ++i) {
        ...
        if (i + 1 < N) {
            ...
            if (i > 0) {
                ...
                if (useBevel) {
                    const inclineWidth =
                        width / Math.cos(bt.angleTo(prevBt) / 2)
...
export function reconstructLine(
    ...
    for (
        let i = startOffset * 3, i2 = i * 2;
        i < outBuffer.length;
        i += 3, i2 += 6
    ) {
        outBuffer[i] = inBuffer[i2] + (inBuffer[i2 + 3] - inBuffer[i2]) * 0.5
        outBuffer[i + 1] =
            inBuffer[i2 + 1] + (inBuffer[i2 + 3 + 1] - inBuffer[i2 + 1]) * 0.5
        outBuffer[i + 2] =
            inBuffer[i2 + 2] + (inBuffer[i2 + 3 + 2] - inBuffer[i2 + 2]) * 0.5
    }
    return outBuffer
}
 ...
 */
export function reconstructLineWidth(
    inBuffer: Float32Array,
    startIndex: number
): number {
