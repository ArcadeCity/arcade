...
import { Env } from './Env'
import {
    BooleanLiteralExpr, CallExpr, CaseExpr, Expr, ExprScope, ExprVisitor,
    HasAttributeExpr, InterpolateExpr, LiteralExpr, LookupExpr, MatchExpr,
    MatchLabel, NullLiteralExpr, NumberLiteralExpr, ObjectLiteralExpr, StepExpr,
    StringLiteralExpr, VarExpr
} from './Expr'
 ...
 * @hidden
 */
export class ExprInstantiator
    implements ExprVisitor<Expr, InstantiationContext>
{
    ...
    visitMatchExpr(match: MatchExpr, context: InstantiationContext): Expr {
        ...
        const branches: Array<[MatchLabel, Expr]> = match.branches.map(
            ([label, branch]) => {
    ...
    visitCaseExpr(expr: CaseExpr, context: InstantiationContext): Expr {
        ...
        for (const [condition, branch] of expr.branches) {
            ...
            if (!condition.isDynamic() && deps.properties.size === 0) {
                if (
                    Boolean(
                        newCondition.evaluate(emptyEnv, ExprScope.Condition)
                    )
                ) {
