...
import {
    BufferAttribute, BufferElementType, Geometry, GeometryType,
...
export namespace ThreeBufferUtils {
    export function getBufferElementType(
    ...
    export function fromThreeBufferGeometry(
        ...
        const attributeNames = Object.getOwnPropertyNames(
            bufferGeometry.attributes
        )
        for (const name of attributeNames) {
            const attribute = bufferGeometry.attributes[name]
            // FIXME: Also support InterleavedBufferAttribute
            const vertexAttribute = fromThreeBufferAttribute(
                attribute as ThreeBufferAttribute
            )
        ...
        if (index !== undefined) {
            count =
                bufferGeometry.index === null ? 0 : bufferGeometry.index.count
        } else {
            // If there is no index buffer, try to deduce the count from the position attribute.
            const posAttr = bufferGeometry.attributes
                .position as ThreeBufferAttribute
            if (posAttr === undefined) {
                throw new Error(
                    'Missing position attibute to deduce item count'
                )
            }
            count = posAttr.count
        }
