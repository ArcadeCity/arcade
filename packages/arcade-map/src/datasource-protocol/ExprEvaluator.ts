...
import {
    ...
    HasAttributeExpr, InterpolateExpr, LiteralExpr, LookupExpr, MatchExpr,
    NullLiteralExpr, NumberLiteralExpr, ObjectLiteralExpr, StepExpr,
    StringLiteralExpr, Value, VarExpr
} from './Expr'
...
function cubicInterpolate(
    ...
    if (
        ...
        return new RGBA(
            ...
            THREE.MathUtils.clamp(
                cP * vP.a + c0 * v0.a + c1 * v1.a + cN * vN.a,
                0,
                1
            )
        )
        ...
        return new Pixels(
            cP * vP.value + c0 * v0.value + c1 * v1.value + cN * vN.value
        )
    } else if (
...
export class ExprEvaluator implements ExprVisitor<Value, ExprEvaluatorContext> {
    ...
    visitCaseExpr(match: CaseExpr, context: ExprEvaluatorContext): Value {
        if (context.scope === ExprScope.Value) {
            const firstDynamicCondition = match.branches.findIndex(
                ([condition, _]) => condition.isDynamic()
            )
            ...
            if (firstDynamicCondition !== -1) {
                ...
                for (let i = 0; i < match.branches.length; ++i) {
                    ...
                    if (
                        !Expr.isExpr(evaluatedCondition) &&
                        !Boolean(evaluatedCondition)
                    ) {
    ...
    visitInterpolateExpr(
        ...
        if (context.scope === ExprScope.Value) {
            ...
            switch (expr.mode[0]) {
                ...
                default:
                    throw new Error(
                        `interpolation mode ${JSON.stringify(
                            expr.mode
                        )} is not supported`
                    )
            }
            ...
            if (typeof v0 === 'number' && typeof v1 === 'number') {
                ...
                return v0.map((x, i) =>
                    THREE.MathUtils.lerp(x, (v1 as number[])[i], t)
                )
            }

            throw new Error(
                `todo: mix(${JSON.stringify(v0)}, ${JSON.stringify(v1)}, ${t})`
            )
        }
    }
}
