...
import {
    BooleanLiteralExpr, CallExpr, CaseExpr, Expr, ExprVisitor, HasAttributeExpr,
    InterpolateExpr, LookupExpr, MatchExpr, NullLiteralExpr, NumberLiteralExpr,
    ObjectLiteralExpr, StepExpr, StringLiteralExpr, VarExpr
} from './Expr'
...
export class ExprPool implements ExprVisitor<Expr, void> {
    ...
    visitMatchExpr(expr: MatchExpr, context: void): Expr {
        ...
        const branches: typeof expr.branches = expr.branches.map(
            ([label, body]) => [label, body.accept(this, context)]
        )
    ...
    private visitCallExprImpl(
        ...
        const expressions = expr.args.map((childExpr) =>
    ...
    visitStepExpr(expr: StepExpr, context: void): Expr {
        ...
        for (const step of this.m_stepExprs) {
            if (
                ...
                stops.every(
                    ([key, value], i) =>
                        key === step.stops[i][0] && value === step.stops[i][1]
                )
            ) {
                return step
            }
    ...
    visitInterpolateExpr(expr: InterpolateExpr, context: void): Expr {
        ...
        for (const interp of this.m_interpolateExprs) {
            if (
                ...
                stops.every(
                    ([key, value], i) =>
                        key === interp.stops[i][0] &&
                        value === interp.stops[i][1]
                )
            ) {
                return interp
            }
