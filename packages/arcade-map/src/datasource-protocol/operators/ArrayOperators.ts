...
import {
    CallExpr, Expr, JsonArray, NumberLiteralExpr, StringLiteralExpr
} from '../Expr'
...
function checkElementTypes(arg: Expr, array: JsonArray) {
    if (
        !(arg instanceof StringLiteralExpr) ||
        !VALID_ELEMENT_TYPES.includes(arg.value)
    ) {
        throw new Error(
            `expected "boolean", "number" or "string" instead of '${JSON.stringify(
                arg
            )}'`
        )
    }
    ...
    array.forEach((element, index) => {
        if (typeof element !== ty) {
            throw new Error(
                `expected array element at index ${index} to have type '${ty}'`
            )
        }
    })
}
