    ...
    for (let i = 0; i < polygonContourEdges.length; ++i) {
        if (polygonContourEdges[i]) {
            if (isExtruded === true) {
                ...
                const vFootprint1 =
                    vertexOffset + ((i + 1) % polygonContourEdges.length) * 2
                ...
                const prevEdgeIdx =
                    (i === 0 ? polygonContourEdges.length : i) - 1
                if (polygonContourEdges[prevEdgeIdx]) {
                    if (wallEdgeSlope !== undefined) {
                        ...
                        const v1x =
                            polygonContour[
                                ((i + 1) % polygonContourEdges.length) *
                                    vertexStride
                            ]
                        const v1y =
                            polygonContour[
                        ...
                        if (
                            prevEdgeGoal
                                ...
                                .normalize()
                                .dot(
                                    currEdgeGoal.sub(currEdgeStart).normalize()
                                ) <= wallEdgeSlope
                        ) {
                ...
                const vRoof0 =
                    vertexOffset + ((i + 1) % polygonContourEdges.length)
