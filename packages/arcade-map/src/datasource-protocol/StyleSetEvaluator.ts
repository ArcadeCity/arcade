...
import {
    BooleanLiteralExpr, CallExpr, CaseExpr, Env, Expr, ExprScope, ExprVisitor,
    HasAttributeExpr, InterpolateExpr, isJsonExpr, JsonExpr, LiteralExpr,
    LookupExpr, MatchExpr, NullLiteralExpr, NumberLiteralExpr,
    ObjectLiteralExpr, StepExpr, StringLiteralExpr, Value, VarExpr
} from './Expr'
...
import {
    AttrScope, getTechniqueAttributeDescriptor
} from './TechniqueDescriptors'
import {
    IndexedTechnique, setTechniqueRenderOrderOrPriority, Technique
} from './Techniques'
 ...
 */
function getStyleAttributeScope(
    style: InternalStyle,
    attrName: string
): AttrScope {
...
class StyleConditionClassifier
    implements ExprVisitor<Expr | undefined, Expr | undefined>
{
    ...
    visitNullLiteralExpr(
        expr: NullLiteralExpr,
        enclosingExpr: Expr | undefined
    ): Expr {
        return expr
    }
...
class OptimizedSubSetKey {
    ...
    set(
        ...
        if (layer === undefined) {
            const envLayer =
                env !== undefined ? env.lookup('$layer') : undefined
        ...
        if (geometryType === undefined) {
            const envGeometryType =
                env !== undefined ? env.lookup('$geometryType') : undefined
            geometryType =
                typeof envGeometryType === 'string'
                    ? envGeometryType
                    : undefined
        }
...
export class StyleSetEvaluator {
    ...
    private readonly m_tmpOptimizedSubSetKey: OptimizedSubSetKey =
        new OptimizedSubSetKey()
    ...
    constructor(private readonly m_options: StyleSetOptions) {
        ...
        this.styleSet = resolveReferences(
            this.m_options.styleSet,
            this.m_definitions
        )
    ...
    getMatchingTechniques(
        ...
        if (
            ...
            this.m_previousEnv &&
            this.m_featureDependencies.every(
                (p) => this.m_previousEnv?.lookup(p) === env.lookup(p)
            )
        ) {
    ...
    wantsLayer(layer: string): boolean {
        return (
            this.getOptimizedStyleSet(
                this.m_tmpOptimizedSubSetKey.set(layer, undefined)
            ).length > 0
        )
    }
    ...
    wantsFeature(layer: string, geometryType?: string): boolean {
        return (
            this.getOptimizedStyleSet(
                this.m_tmpOptimizedSubSetKey.set(layer, geometryType)
            ).length > 0
        )
    }
    ...
    private compileStyleSet() {
        ...
        this.styleSet.forEach((style) => {
            this.getOptimizedStyleSet(
                this.m_tmpOptimizedSubSetKey.set(
                    style.layer,
                    style._geometryType
                )
            )
    ...
    private checkZoomLevel(env: Env, style: InternalStyle) {
        if (
            style.minZoomLevel === undefined &&
            style.maxZoomLevel === undefined
        ) {
            return true
        }
    ...
    private getTechniqueForStyleMatch(env: Env, style: InternalStyle) {
        ...
        if (style._dynamicTechniques !== undefined) {
            const dynamicAttributes = this.evaluateTechniqueProperties(
                style,
                env
            )
                    ...
                    return JSON.stringify(attrValue, (_, value) => {
                        if (value instanceof ArrayBuffer) {
                            ...
                            if (arrayBufferId === undefined) {
                                arrayBufferId = (value as any).id = this
                                    .m_nextArrayBufferId++
                            }
    ...
    private checkStyleDynamicAttributes(style: InternalStyle) {
        if (
            style._dynamicTechniqueAttributes !== undefined ||
        ...
        const processAttribute = (
            ...
            if (Expr.isExpr(attrValue)) {
                ...
                if (deps.featureState) {
                    if (attrName !== 'enabled') {
                        logger.log(
                            'feature-state is not supported in this context'
                        )
                    } else {
        ...
        for (const p in style) {
            ...
            if (
                ['when', 'technique', 'layer', 'attr', 'description'].includes(
                    p
                )
            ) {
                continue
    ...
    private evaluateTechniqueProperties(
        ...
        return style._dynamicTechniqueAttributes.map(([attrName, attrExpr]) => {
            try {
                ...
                logger.error(
                    `failed to evaluate expression '${attrExpr.toJSON()}': ${error}`
                )
    ...
    private createTechnique(
        ...
        if (style._dynamicFeatureAttributes !== undefined) {
            for (const [
                attrName,
                attrValue,
            ] of style._dynamicFeatureAttributes) {
                technique[attrName] = attrValue
            }
        }
        ...
        if (style._dynamicForwardedAttributes !== undefined) {
            for (const [
                attrName,
                attrValue,
            ] of style._dynamicForwardedAttributes) {
                technique[attrName] = attrValue
            }
        }
 ...
 */
export function makeDecodedTechnique(
