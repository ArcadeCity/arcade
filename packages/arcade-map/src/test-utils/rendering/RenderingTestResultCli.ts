...
async function genConsoleReport(
    ...
    results.forEach((info) => {
        ...
        if (
            ...
            if (
                (info.approveDifference === true || forceApprovals) &&
                info.actualImagePath
            ) {
                logger.log(
                ...
                logger.log(
                    `${referencePath}: failed, ${info.mismatchedPixels} mismatched pixels`
                )
