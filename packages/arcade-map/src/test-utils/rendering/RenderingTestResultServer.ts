...
import {
    getOutputImagePath, loadSavedResults
} from './RenderingTestResultCommon'
...
export async function postIbctFeedback(
    req: express.Request,
    ...
    try {
        ...
        if (payload.comparisonResult && payload.comparisonResult.diffImage) {
            ...
            imageResult.mismatchedPixels =
                payload.comparisonResult.mismatchedPixels
        }
...
export async function getIbctReport(
    req: express.Request,
    res: express.Response
) {
    try {
        const [failed, report] = await genHtmlReport(
            currentResults,
            {},
            outputBasePath
        )
...
export function getReferenceImage(req: express.Request, res: express.Response) {
    ...
    const relPath = getOutputImagePath(
        { ...imageProps, extra: '.reference' },
        outputBasePath
    )
 ...
 *          "coresdk/@arca/test-utils/lib/rendering/RenderingTestResultServer"
 *         );
