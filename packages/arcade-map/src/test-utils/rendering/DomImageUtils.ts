...
export function createImgElement(data: ImageData | ImageBitmap | string) {
    ...
    if (typeof data === 'string') {
        ...
        if (context === null) {
            throw new Error(
                '#createImg: unable to obtain 2d context from canvas'
            )
        }
        ...
        return await new Promise<ImageData>((resolve, reject) => {
            canvas.toBlob((blob) => {
                if (blob === null) {
                    reject(
                        new Error(
                            '#canvasToImageData: unable to capture image from canvas'
                        )
                    )
                    return
                }
        ...
        new THREE.ImageLoader().load(
            ...
            (image) => {
                ...
                if (context === null) {
                    reject(
                        new Error(
                            '#loadImageData: unable to create 2d context out of canvas'
                        )
                    )
                    return
                }
            ...
            (errorEvent) => {
                logger.error(
                    `#loadImageData: failed to load image from ${url}`,
                    errorEvent
                )
                reject(
                    new Error(`#loadImageData failed to load image from ${url}`)
                )
            }
        )
    })
...
export async function imageDataFromImage(
