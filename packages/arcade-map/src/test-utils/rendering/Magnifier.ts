...
export function installMagnifier(
    ...
    let ready = false
    Promise.all([
        waitImageLoaded(actual),
        waitImageLoaded(expected),
        waitImageLoaded(diff),
    ]).then(() => {
    ...
    function renderZoomer() {
        requestAnimationFrame(() => {
            ...
            const centerOutlineOffset = Math.floor(
            ...
            const pixelColors = [
                ctx.getImageData(
                    SIZE * SCALE * 0 + centerPixelOffset,
                    centerPixelOffset,
                    1,
                    1
                ),
                ctx.getImageData(
                    SIZE * SCALE * 1 + centerPixelOffset,
                    centerPixelOffset,
                    1,
                    1
                ),
            ].map((image) => {
                return [image.data[0], image.data[1], image.data[2]] as [
                    number,
                    number,
                    number
                ]
            ...
            text.innerHTML = `
                Position: ${sx} x ${sy} (of ${actualClone.width} x ${
                actualClone.height
            })<br>
                Actual: [${rgbToString(colorActual)}]
                    (${htmlColor(colorActual)} ${colorIndicator(
                colorActual
            )})<br>
                Expected: [${rgbToString(colorExpected)}]
                    (${htmlColor(colorExpected)} ${colorIndicator(
                colorExpected
            )})<br>
