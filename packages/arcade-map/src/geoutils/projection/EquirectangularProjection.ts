...
class EquirectangularProjection extends Projection {
    ...
    unprojectPoint(worldPoint: Vector3Like): GeoCoordinates {
        const geoPoint = GeoCoordinates.fromRadians(
            (worldPoint.y * EquirectangularProjection.worldToGeoScale) /
                this.unitScale -
                Math.PI * 0.5,
            (worldPoint.x * EquirectangularProjection.worldToGeoScale) /
                this.unitScale -
                Math.PI,
            worldPoint.z
        )
    ...
    projectBox<WorldBoundingBox extends Box3Like | OrientedBox3Like>(
        ...
        const worldCenter = this.projectPoint(
            new GeoCoordinates(
                geoBox.center.latitude,
                geoBox.center.longitude,
                0
            )
        )
        const { latitudeSpanInRadians, longitudeSpanInRadians, altitudeSpan } =
            geoBox
        const sizeX =
            longitudeSpanInRadians * EquirectangularProjection.geoToWorldScale
        const sizeY =
            latitudeSpanInRadians * EquirectangularProjection.geoToWorldScale
        if (!result) {
            result = new THREE.Box3() as Box3Like as WorldBoundingBox
        }
...
export const normalizedEquirectangularProjection: Projection =
    new EquirectangularProjection(1)
...
export const equirectangularProjection: Projection =
    new EquirectangularProjection(EarthConstants.EQUATORIAL_CIRCUMFERENCE)
