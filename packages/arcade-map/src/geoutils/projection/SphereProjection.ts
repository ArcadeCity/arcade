...
import {
    GeoCoordinatesLike, isGeoCoordinatesLike
} from '../coordinates/GeoCoordinatesLike'
...
class SphereProjection extends Projection {
    ...
    projectPoint<WorldCoordinates extends Vector3Like>(
        geoPoint: GeoCoordinatesLike,
        result: WorldCoordinates = MathUtils.newVector3(
            0,
            0,
            0
        ) as WorldCoordinates
    ...
    projectBox<Bounds extends Box3Like | OrientedBox3Like>(
        ...
        if (isBox3Like(result)) {
            ...
            if (geoBox.longitudeSpan >= 90) {
                const bounds = makeBox3(
                    geoBox,
                    new THREE.Box3() as Box3Like,
                    this.unitScale
                )
            ...
            if (south >= 0) {
                ...
                width = Math.abs(
                    cosSouth *
                        (cosMidX * (sinWest - sinEast) +
                            sinMidX * (cosEast - cosWest))
                )
                ...
                if (north <= 0) {
                    ...
                    maxY =
                        ...
                        sinMidY *
                            cosNorth *
                            (sinMidX * sinEast + cosMidX * cosEast)
                }
            ...
            result.position.x =
                result.position.x - result.zAxis.x * result.extents.z
            result.position.y =
                result.position.y - result.zAxis.y * result.extents.z
            result.position.z =
                result.position.z - result.zAxis.z * result.extents.z
    ...
    reprojectPoint(
        ...
        if (
            sourceProjection === mercatorProjection ||
            sourceProjection === webMercatorProjection
        ) {
