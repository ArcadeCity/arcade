...
class TransverseMercatorProjection extends Projection {
    ...
    static clampGeoPoint(geoPoint: GeoCoordinatesLike, _unitScale: number) {
        ...
        if (distanceToPoleSq < rsq) {
            ...
            const dir = 1
            const offsetLon =
                deltaLon === 0 && deltaLat === 0 ? r * dir : deltaLon
    ...
    projectPoint<WorldCoordinates extends Vector3Like>(
        ...
        const clamped = TransverseMercatorProjection.clampGeoPoint(
            geoPoint,
            this.unitScale
        )
        ...
        const lambda =
            THREE.MathUtils.degToRad(clamped.longitude - offset * 360) -
            this.m_lambda0
        ...
        result.x =
            this.unitScale *
            (THREE.MathUtils.clamp(result.x * outScale + 0.5, 0, 1) + offset)
        result.y =
            this.unitScale *
            THREE.MathUtils.clamp(result.y * outScale + 0.5, 0, 1)
    ...
    unprojectPoint(worldPoint: Vector3Like): GeoCoordinates {
        ...
        const lambda =
            this.m_lambda0 +
            Math.atan2(Math.sinh(x), Math.cos(D)) +
            offset * tau
    ...
    unprojectBox(worldBox: Box3Like): GeoBox {
        ...
        const longitudes = geoPoints
            .filter((g) => Math.abs(g.latitude) < 90)
            .map((g) => g.longitude)
...
export class TransverseMercatorUtils {
    ...
    static POLE_EDGE_DEG: number = THREE.MathUtils.radToDeg(
        TransverseMercatorUtils.POLE_EDGE
    )
    static POLE_RADIUS: number = 90 - TransverseMercatorUtils.POLE_EDGE_DEG
    static POLE_RADIUS_SQ: number = Math.pow(
        TransverseMercatorUtils.POLE_RADIUS,
        2
    )
     ...
     */
    static alignLatitude(
        points: GeoCoordinatesLike[],
        referencePoint: GeoCoordinatesLike
    ): void {
        const EPSILON = 1e-9
...
export const transverseMercatorProjection: Projection =
    new TransverseMercatorProjection(EarthConstants.EQUATORIAL_CIRCUMFERENCE)
