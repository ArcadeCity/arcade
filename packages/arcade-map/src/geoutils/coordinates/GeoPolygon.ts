...
export class GeoPolygon implements GeoPolygonLike {
    ...
    getCentroid(): GeoCoordinates | undefined {
        ...
        this.m_coordinates.forEach((coordinate, index) => {
            ...
            longitude +=
                (coordinate.longitude + previousCoordinate.longitude) * f
            previousIndex = index
        })
    ...
    private sortCCW() {
        ...
        this.m_coordinates.sort(
            (a: GeoCoordinatesLike, b: GeoCoordinatesLike) => {
    ...
    private wrapCoordinatesAround() {
        const firstAntimerCrossIndex = this.m_coordinates.findIndex(
            (val: GeoCoordinatesLike, index: number) => {
                const prevLonIndex =
                    index === 0 ? this.m_coordinates.length - 1 : index - 1
        ...
        for (let i = 0; i < this.m_coordinates.length; i++) {
            const index =
                (firstAntimerCrossIndex + i) % this.m_coordinates.length
            const currentLon = this.m_coordinates[index].longitude
            const nextLon =
                this.m_coordinates[(index + 1) % this.m_coordinates.length]
                    .longitude
    ...
    private getEastAndWest(center: GeoCoordinates): {
        east: number
        west: number
    } {
