...
export namespace TileKeyUtils {
    ...
    export function worldCoordinatesToTileKey(
        ...
        const column = Math.min(
            cx - 1,
            Math.floor((cx * (worldPoint.x - min.x)) / worldSizeX)
        )
        const row = Math.min(
            cy - 1,
            Math.floor((cy * (worldPoint.y - min.y)) / worldSizeY)
        )
    ...
    export function geoRectangleToTileKeys(
        ...
        const columnCount =
            tilingScheme.subdivisionScheme.getLevelDimensionX(level)
     ...
     *
     * @param tileKey - The unique {@link @arca/geoutils#TileKey}
     *                  from which to compute the unique key.
     * @param offset - How much the given {@link @arca/geoutils#TileKey} is offset
    ...
    export function getParentKeyFromKey(
        ...
        const { offset, mortonCode } = extractOffsetAndMortonKeyFromKey(
            calculatedKey,
            bitshift
        )
        const parentTileKey = TileKey.fromMortonCode(
            TileKey.parentMortonCode(mortonCode)
        )
