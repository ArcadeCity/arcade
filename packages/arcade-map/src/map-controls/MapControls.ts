...
import * as geoUtils from '@arca/geoutils'
import {
    CameraUtils, EventDispatcher, MapView, MapViewEventNames, MapViewUtils
} from '@arca/mapview'
...
export class MapControls extends EventDispatcher {
    ...
    private readonly m_panDistanceFrameDelta: THREE.Vector3 =
        new THREE.Vector3()
    ...
    private readonly m_rotateGlobeQuaternion: THREE.Quaternion =
        new THREE.Quaternion()
    private readonly m_lastRotateGlobeAxis: THREE.Vector3 = new THREE.Vector3()
    private m_lastRotateGlobeAngle: number = 0
    private readonly m_lastRotateGlobeFromVector: THREE.Vector3 =
        new THREE.Vector3()
    ...
    private readonly m_zoomTargetNormalizedCoordinates: THREE.Vector2 =
        new THREE.Vector2()
    ...
    constructor(readonly mapView: MapView, disposeWithMapView = true) {
        ...
        this.assignZoomAfterTouchZoomRender =
            this.assignZoomAfterTouchZoomRender.bind(this)
    ...
    setZoomLevel(
        ...
        this.m_zoomTargetNormalizedCoordinates.set(
            screenTarget.x,
            screenTarget.y
        )
    ...
    get zoomLevelTargeted(): number {
        return this.m_targetedZoom === undefined
            ? this.currentZoom
            : this.m_targetedZoom
    }
    ...
    get tiltState(): TiltState {
        if (this.m_tiltState === undefined) {
            this.m_tiltState =
                this.currentTilt < EPSILON ||
                this.m_tiltState === TiltState.Down
                    ? TiltState.Tilted
                    : TiltState.Down
        }
    ...
    private resetNorth() {
        ...
        if (this.inertiaEnabled) {
            ...
            const resetNorthFinished =
                animationTime > this.m_resetNorthAnimationDuration
    ...
    private tilt() {
        if (this.m_tiltRequested !== undefined) {
            this.m_targetedTilt = Math.max(
                Math.min(this.m_tiltRequested, this.maxTiltAngle),
                0
            )
        ...
        if (this.inertiaEnabled) {
            ...
            const currentTime = performance.now()
            this.m_tiltAnimationTime =
                (currentTime - this.m_tiltAnimationStartTime) / 1000
            const tiltFinished =
    ...
    private easeOutCubic(
        ...
        return (
            startValue + (endValue - startValue) * (1 - Math.pow(1 - time, 3))
        )
    }
    ...
    private handleZoom() {
        ...
        if (this.m_zoomDeltaRequested !== 0) {
            this.m_targetedZoom = Math.max(
                Math.min(
                    this.zoomLevelTargeted + this.m_zoomDeltaRequested,
                    this.maxZoomLevel
                ),
                this.minZoomLevel
            )
        ...
        if (this.inertiaEnabled && this.zoomInertiaDampingDuration > 0) {
            ...
            const currentTime = performance.now()
            this.m_zoomAnimationTime =
                (currentTime - this.m_zoomAnimationStartTime) / 1000
            const zoomFinished =
        ...
        this.currentZoom =
            !this.inertiaEnabled ||
            Math.abs(this.zoomLevelTargeted - this.m_startZoom) < EPSILON
                ...
                : this.easeOutCirc(
                      ...
                      Math.min(
                          1,
                          this.m_zoomAnimationTime /
                              this.zoomInertiaDampingDuration
                      )
                  )
    ...
    private handlePan() {
        ...
        if (applyInertia) {
            const currentTime = performance.now()
            this.m_panAnimationTime =
                (currentTime - this.m_panAnimationStartTime) / 1000
            const panFinished =
            ...
            if (
                this.mapView.projection.type === geoUtils.ProjectionType.Planar
            ) {
                ...
                geoUtils.ProjectionType.Spherical
            ) {
            ...
            this.m_currentPanDistanceOrAngleIndex =
                (this.m_currentPanDistanceOrAngleIndex + 1) %
                USER_INPUTS_TO_CONSIDER
            this.m_recentPanDistancesOrAngles[
                this.m_currentPanDistanceOrAngleIndex
            ] = panDistanceOrAngle
            this.m_lastAveragedPanDistanceOrAngle =
                this.m_recentPanDistancesOrAngles.reduce((a, b) => a + b) /
                USER_INPUTS_TO_CONSIDER
        }
    ...
    private mouseDown(event: MouseEvent) {
        ...
        if (event.altKey === true) {
            const { width, height } = utils.getWidthAndHeightFromCanvas(
                this.domElement
            )
            this.m_initialMousePosition.copy(
                utils.calculateNormalizedDeviceCoordinates(
                    mousePos.x,
                    mousePos.y,
                    width,
                    height
            ...
            CameraUtils.getPrincipalPoint(
                this.mapView.camera,
                this.m_initialMousePosition
            )
        }
    ...
    private mouseWheel(event: WheelEvent) {
        ...
        this.setZoomLevel(
            this.mapView.zoomLevel -
                this.zoomLevelDeltaOnMouseWheel * Math.sign(event.deltaY),
            screenTarget
        )
    ...
    private updateCurrentRotation() {
        ...
        if (this.mapView.projection.type === geoUtils.ProjectionType.Planar) {
            ...
            this.m_touchState.initialRotation =
                this.m_touchState.currentRotation
        }
    ...
    private convertTouchPoint(
        ...
        if (oldTouchState !== undefined) {
            ...
            const toWorld =
                vectors === undefined ? new THREE.Vector3() : vectors.toWorld
            ...
            const result = MapViewUtils.rayCastWorldCoordinates(
                this.mapView,
                to.x,
                to.y
            )
    ...
    private updateTouches(touches: TouchList) {
        const length = Math.min(
            touches.length,
            this.m_touchState.touches.length
        )
        for (let i = 0; i < length; ++i) {
            const oldTouchState = this.m_touchState.touches[i]
            const newTouchState = this.convertTouchPoint(
                touches[i],
                oldTouchState
            )
            if (newTouchState !== undefined && oldTouchState !== undefined) {
                newTouchState.initialWorldPosition =
                    oldTouchState.initialWorldPosition
    ...
    private zoomOnDoubleClickOrTap(x: number, y: number) {
        ...
        const ndcCoords = utils.calculateNormalizedDeviceCoordinates(
            x,
            y,
            width,
            height
        ...
        this.setZoomLevel(
            this.currentZoom + this.zoomLevelDeltaOnDoubleClick,
            ndcCoords
        )
    }
    ...
    private getWorldPositionWithElevation(
        ...
        const from = utils.calculateNormalizedDeviceCoordinates(
            fromX,
            fromY,
            width,
            height
        ...
        if (this.mapView.elevationProvider !== undefined) {
            elevationProviderResult = this.mapView.elevationProvider.rayCast(
                fromX,
                fromY
            )
        }
        ...
        if (elevationProviderResult === undefined) {
            fromWorld = MapViewUtils.rayCastWorldCoordinates(
                this.mapView,
                from.x,
                from.y
            )
            ...
            const fromGeoAltitude =
                this.mapView.projection.unprojectAltitude(fromWorld)
    ...
    private panFromTo(fromWorld: THREE.Vector3, toWorld: THREE.Vector3): void {
        ...
        if (this.mapView.projection.type === geoUtils.ProjectionType.Planar) {
            ...
            this.m_lastRotateGlobeAxis
                .crossVectors(fromWorld, toWorld)
                .normalize()
