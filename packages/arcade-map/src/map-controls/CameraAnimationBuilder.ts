...
import { GeoCoordinates, MathUtils } from '@arca/geoutils'
import { LookAtParams, MapView } from '@arca/mapview'

import {
    CameraKeyTrackAnimationOptions, ControlPoint
} from './CameraKeyTrackAnimation'
...
export class CameraAnimationBuilder {
    ...
    static appendControlPoint(
        ...
        if (
            ...
            (point.timestamp === undefined ||
                point.timestamp <=
                    options.controlPoints[options.controlPoints.length - 1]
                        .timestamp)
        ) {
            point.timestamp =
                options.controlPoints[options.controlPoints.length - 1]
    ...
    static createBowFlyToOptions(
        ...
        mapView.projection.projectPoint(
            startControlPoint.target,
            startWorldTarget
        )
        let maxAltitude =
            altitude ??
