...
import { GeoCoordinates } from '@arca/geoutils'
import {
    LookAtParams, MapView, MapViewEventNames, MapViewUtils, RenderEvent
} from '@arca/mapview'
...
export class CameraKeyTrackAnimation {
    ...
    constructor(
        ...
        this.m_options.rotateOnlyClockwise =
            this.m_options.rotateOnlyClockwise ?? true

        this.m_name =
            (this.m_options.name ?? 'CameraKeyTrackAnimation') + Date.now()
        ...
        this.m_options.controlPoints.map((point) => {
            const worldPos = this.m_mapView.projection.projectPoint(
                point.target
            )
            ...
            rot.setFromAxisAngle(
                this.m_azimuthAxis,
                THREE.MathUtils.degToRad(point.heading)
            )
            rot.multiply(
        ...
        this.m_animationClip = new THREE.AnimationClip(
            'cameraflyoveranimation',
            -1,
            [lookAtTrack, rotationTrack, distanceTrack]
        )
        ...
        this.m_animationAction = this.m_animationMixer.clipAction(
    ...
    set loop(value: THREE.AnimationActionLoopStyles) {
        ...
        this.m_animationAction.setLoop(
            this.m_options.loop,
            this.m_options.repetitions as number
        )
    }
    ...
    private updateCameraFromDummy() {
        ...
        const vec0 = this.m_altitudeAxis
            .clone()
            .applyQuaternion(this.m_dummy.quaternion)
        let angle = vec0.angleTo(this.m_altitudeAxis)
        if (this.m_options.rotateOnlyClockwise) {
            const direction = this.m_azimuthAxis.dot(
                vec0.normalize().cross(this.m_altitudeAxis)
            )
            if (direction >= 0) {
        ...
        const target = this.m_mapView.projection.unprojectPoint(
            this.m_dummy.position
        )
        ...
        if (
            ...
            logger.error(
                'Cannot update due to invalid data',
                tilt,
                heading,
                distance,
                target
