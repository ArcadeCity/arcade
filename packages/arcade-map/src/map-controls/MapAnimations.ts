...
import { GeoCoordinates, GeoCoordinatesLike } from '@arca/geoutils'
import { MapView } from '@arca/mapview'
import { PerformanceTimer } from '@arca/utils'
...
export class CameraRotationAnimation extends CameraAnimation {
    ...
    start(time?: number, onFinished?: () => void): void {
        if (this.running) {
            throw new Error(
                'Animation already running' + this.name !== undefined
                    ? this.name
                    : ''
            )
        }
    ...
    stop(): void {
        if (!this.running) {
            throw new Error(
                'Animation not running' + this.name !== undefined
                    ? this.name
                    : ''
            )
        }
    ...
    private startTween(time?: number): void {
        ...
        this.tween = new TWEEN.Tween({ rotation: 0 })
            .to(
                { rotation: this.endAngle - this.m_lastRotationValue },
                this.duration
            )
            .onComplete(() => {
                this.stop()
            ...
            .onUpdate(({ rotation }) => {
                ...
                rotZ.setFromEuler(
                    new THREE.Euler(0, 0, THREE.MathUtils.degToRad(rotation))
                )
...
export class CameraPanAnimation extends CameraAnimation {
    ...
    constructor(
        mapView: MapView,
        options: CameraPanAnimationOptions,
        public name?: string
    ) {
        super(mapView, name)
        ...
        this.m_geoCoordinates =
            options.geoCoordinates !== undefined ? options.geoCoordinates : []
    }
    ...
    start(time?: number, onFinished?: () => void): void {
        ...
        this.tween = new TWEEN.Tween(from)
            ...
            .onUpdate(({ latitude, longitude, altitude }) => {
                this.mapView.geoCenter = new GeoCoordinates(
                    latitude,
                    longitude,
                    altitude
                )
...
function checkSetupTween() {
    ...
    interpolationMap.set(
        InterpolationFunction.Bezier,
        TWEEN.Interpolation.Bezier
    )
    interpolationMap.set(
        InterpolationFunction.CatmullRom,
