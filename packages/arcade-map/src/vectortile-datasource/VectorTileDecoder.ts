 ...
 */
import {
    AttrEvaluationContext, DecodedTile, DecoderOptions, GeometryKind,
    IndexedTechnique, OptionsMap, ValueMap
} from '@arca/datasource-protocol'
import {
    MapEnv, StyleSetEvaluator
} from '@arca/datasource-protocol/index-decoder'
import { Projection, TileKey } from '@arca/geoutils'
import {
    ThemedTileDecoder, TileDecoderService, WorkerServiceManager
} from '@arca/mapview-decoder/index-worker'
import { assert, LoggerManager, PerformanceTimer } from '@arca/utils'

import {
    GeoJsonVtDataAdapter
} from './adapters/geojson-vt/GeoJsonVtDataAdapter'
...
import {
    OmvDecoderOptions, VECTOR_TILE_DECODER_SERVICE_TYPE
} from './OmvDecoderDefs'
import {
    VectorTileDataEmitter, VectorTileDataEmitterOptions
} from './VectorTileDataEmitter'
 ...
 * @internal
 */
export interface VectorTileDataProcessorOptions
    extends VectorTileDataEmitterOptions {
...
export class VectorTileDataProcessor implements IGeometryProcessor {
    ...
    constructor(
        ...
        m_options.showMissingTechniques =
            m_options.showMissingTechniques ?? false
    }
    ...
    private processFeature(
        ...
        const techniques = this.applyKindFilter(
            this.m_styleSetEvaluator.getMatchingTechniques(
                env,
                layer,
                geometryType
            ),
