...
import {
    AttrEvaluationContext, AttributeMap, BufferAttribute,
    composeTechniqueTextureName, DecodedTile, evaluateTechniqueAttr,
    ExtrudedPolygonTechnique, FillTechnique, Geometry, GeometryType,
    getFeatureId, getFeatureText, Group, IndexedTechnique,
    InterleavedBufferAttribute, isExtrudedLineTechnique,
    isExtrudedPolygonTechnique, isFillTechnique, isLabelRejectionLineTechnique,
    isLineMarkerTechnique, isLineTechnique, isPoiTechnique, isShaderTechnique,
    isSolidLineTechnique, isSpecialDashesLineTechnique, isStandardTechnique,
    isTextTechnique, LineMarkerTechnique, PathGeometry, PoiGeometry,
    PoiTechnique, scaleHeight, StyleColor, Technique, TextGeometry,
    TextPathGeometry, TextTechnique, TextureCoordinateType,
    textureCoordinateType
} from '@arca/datasource-protocol'
import {
    Env, IMeshBuffers, StyleSetEvaluator, Value
} from '@arca/datasource-protocol/index-decoder'
import {
    clipPolygon, EdgeLengthGeometrySubdivisionModifier,
    SphericalGeometrySubdivisionModifier, SubdivisionMode
} from '@arca/geometry'
import {
    GeoBox, GeoCoordinates, normalizedEquirectangularProjection, ProjectionType,
    Vector3Like, webMercatorProjection
} from '@arca/geoutils'
import { LineGroup, triangulateLine } from '@arca/lines'
import { ExtrusionFeatureDefs } from '@arca/materials'
import { assert, getOptionValue, LoggerManager, Math2D } from '@arca/utils'

import { DecodeInfo } from './DecodeInfo'
import { ILineGeometry, IPolygonGeometry } from './IGeometryProcessor'
import {
    tile2world, webMercatorTile2TargetTile, webMercatorTile2TargetWorld,
    world2tile
} from './OmvUtils'
...
type TexCoordsFunction = (
    tilePos: THREE.Vector2,
    tileExtents: number
) => THREE.Vector2
...
export class VectorTileDataEmitter {
    ...
    processPointFeature(
        ...
        for (const technique of techniques) {
            ...
            if (wantsPoi) {
                ...
                imageTexture = evaluateTechniqueAttr(
                    context,
                    poiTechnique.imageTexture
                )
                ...
                if (poiTechnique.poiName !== undefined) {
                    ...
                    const poiNameFieldValue = env.lookup(
                        poiTechnique.poiNameField
                    ) as string
                    imageTexture = poiNameFieldValue
                } else if (typeof poiTechnique.imageTextureField === 'string') {
                    const imageTextureValue = env.lookup(
                        poiTechnique.imageTextureField
                    ) as string
                    imageTexture = composeTechniqueTextureName(
                        imageTextureValue,
                        poiTechnique
                    )
                }
            ...
            const scaleHeights = scaleHeight(
                context,
                technique,
            ...
            for (const pos of tilePositions) {
                if (shouldCreateTextGeometries) {
                    ...
                    const text = getFeatureText(
                        context,
                        textTechnique,
                        this.m_options.languages
                    )
                ...
                offsetDirections.push(
                    (env.lookup('offset_direction') as number) ?? 0
                )
    ...
    processLineFeature(
        ...
        for (const technique of techniques) {
            ...
            if (!computeTexCoords) {
                ...
                const otherTexCoordType =
                    this.getTextureCoordinateType(technique)
                assert(
                    otherTexCoordType === undefined ||
                        texCoordinateType === otherTexCoordType
                )
            }

            hasIndividualLineSegments =
                hasIndividualLineSegments ||
                isSpecialDashesLineTechnique(technique)

            hasContinuousLineSegments =
                hasContinuousLineSegments || !hasIndividualLineSegments
        }
        ...
        for (const polyline of geometry) {
            ...
            if (hasIndividualLineSegments) {
                ...
                if (pointCount > 1) {
                    ...
                    for (let i = 0; i < pointCount - 1; i++) {
                        ...
                        if (computeTexCoords) {
                            computeTexCoords(pos1, extents).toArray(
                                lineUvs,
                                lineUvs.length
                            )
                            computeTexCoords(pos2, extents).toArray(
                                lineUvs,
                                lineUvs.length
                            )
                        }
                        if (hasUntiledLines) {
                            ...
                            // vertex lies.
                            let offset =
                                this.findRelativePositionInLine(
                                    tmpV3,
                                    untiledLine
                                ) / lineDist
            ...
            if (hasContinuousLineSegments) {
                ...
                polyline.positions.forEach((pos) => {
                    ...
                    if (computeTexCoords) {
                        computeTexCoords(pos, extents).toArray(
                            lineUvs,
                            lineUvs.length
                        )
                    }
        ...
        for (const technique of techniques) {
            ...
            if (isLineTechnique(technique) || isSolidLineTechnique(technique)) {
                ...
                const lineType = isLineTechnique(technique)
                    ? LineType.Simple
                    : LineType.Complex
                ...
                if (this.m_options.skipShortLabels) {
                    ...
                    const worldUnitsPerPixel =
                        tileSizeWorld / this.m_decodeInfo.tileSizeOnScreen
                ...
                if (isTextTechnique(technique)) {
                    ...
                    for (const path of validLines) {
                        const pathLengthSqr =
                            Math2D.computeSquaredLineLength(path)
                    ...
                    // TODO: Move to decoder independent parts of code.
                    if (
                        typeof lineMarkerTechnique.imageTextureField ===
                        'string'
                    ) {
                        const imageTextureValue = env.lookup(
                        ...
                        if (
                            typeof lineMarkerTechnique.imageTexturePrefix ===
                            'string'
                        ) {
                            imageTexture =
                                lineMarkerTechnique.imageTexturePrefix +
                                imageTexture
                        }
                        if (
                            typeof lineMarkerTechnique.imageTexturePostfix ===
                            'string'
                        ) {
                            imageTexture =
                                imageTexture +
                                lineMarkerTechnique.imageTexturePostfix
                        }
                    }
                ...
                const { positions, indices, groups, featureStarts, objInfos } =
                    meshBuffers
                const start = indices.length

                const lineWidth = evaluateTechniqueAttr<number>(
                    context,
                    technique.lineWidth
                )
    ...
    processPolygonFeature(
        ...
        techniques.forEach((technique) => {
            ...
            const computeTexCoords = this.getComputeTexCoordsFunc(
                technique,
                objectBounds
            )
            ...
            const isLine =
                isSolidLineTechnique(technique) || isLineTechnique(technique)
            if (isPolygon) {
                ...
                const needIndividualLineSegments =
                    isSpecialDashesLineTechnique(technique)

                polygons.forEach((rings) => {
                    const lines: number[][] = []
                    const offsets: number[][] | undefined =
                        needIndividualLineSegments ? [] : undefined
                    rings.forEach((ring) => {
                        ...
                        let ringLength = 0
                        let lastSegmentOffset = 0
                        let segmentOffsets: number[] | undefined =
                            needIndividualLineSegments ? [] : undefined
    ...
    protected splitJaggyLines(
        ...
        while (pathsToCheck.length > 0) {
            ...
            for (let i = 0; i < path.length - 3; i += 3) {
                ...
                if (i > 0) {
                    const theta = Math.atan2(
                        tempPreviousTangent.x * tangent.y -
                            tangent.x * tempPreviousTangent.y,
            ...
            if (splitIndex > 0) {
                ...
                // coordinates.
                const pathLengthSqr = computeBoundingBoxSizeSqr(
                    path,
                    0,
                    path.length
                )
    ...
    private getTextureCoordinateType(
        technique: Technique
    ): TextureCoordinateType | undefined {
        ...
        switch (texCoordType) {
            ...
            case TextureCoordinateType.EquirectangularSpace:
                return (
                    ...
                    const uv =
                        normalizedEquirectangularProjection.reprojectPoint(
    ...
    private applyLineTechnique(
        ...
        const lineGroupGeometries = linesGeometry.find(
            (aLine) => aLine.technique === techniqueIndex
        )
    ...
    private applyPolygonTechnique(
        ...
        const geometryType = isExtruded
            ? GeometryType.ExtrudedPolygon
        ...
        let height = evaluateTechniqueAttr<number>(
            context,
            extrudedPolygonTechnique.height
        )
        ...
        if (floorHeight === undefined) {
            ...
            floorHeight =
                featureMinHeight !== undefined && !isFilled
                    ? featureMinHeight
                    : 0
        }
        ...
        height = Math.max(
            floorHeight + ExtrusionFeatureDefs.MIN_BUILDING_HEIGHT,
            height
        )
        ...
        const scaleHeights =
            isExtruded &&
            scaleHeight(context, extrudedPolygonTechnique, tileLevel)
        ...
        for (const polygon of polygons) {
            ...
            for (let i = 0; i < exteriorRing.points.length; ++i) {
                ...
                if (exteriorRing.textureCoords !== undefined) {
                    vertices.push(
                        exteriorRing.textureCoords[i].x,
                        exteriorRing.textureCoords[i].y
                    )
                }
            ...
            while (ringIndex < polygon.length) {
                ...
                for (let i = 0; i < hole.points.length; ++i) {
                    ...
                    if (hole.textureCoords !== undefined) {
                        vertices.push(
                            hole.textureCoords[i].x,
                            hole.textureCoords[i].y
                        )
                    }
            ...
            try {
                ...
                if (isSpherical) {
                    ...
                    if (texCoordType !== undefined) {
                        uvAttr = new THREE.BufferAttribute(
                            new Float32Array(uvArray),
                            2
                        )
                        geom.setAttribute('uv', uvAttr)
                    }
                    const edgeAttr = new THREE.BufferAttribute(
                        new Float32Array(edgeArray),
                        1
                    )
                    geom.setAttribute('edge', edgeAttr)
                    const wallAttr = new THREE.BufferAttribute(
                        new Float32Array(wallArray),
                        1
                    )
                ...
                for (let i = 0; i < vertices.length; i += vertexStride) {
                    ...
                    tempFootDisp
                        .copy(tempVertNormal)
                        .multiplyScalar(floorHeight * scaleFactor)
                    positions.push(
                    ...
                    if (isExtruded) {
                        tempRoofDisp
                            .copy(tempVertNormal)
                            .multiplyScalar(height * scaleFactor)
                        positions.push(
    ...
    private createGeometries() {
        this.m_meshBuffers.forEach((meshBuffers, techniqueIdx) => {
            ...
            if (meshBuffers.extrusionAxis.length > 0) {
                const extrusionAxis = new Float32Array(
                    meshBuffers.extrusionAxis
                )
                assert(
            ...
            if (meshBuffers.indices.length > 0) {
                geometry.index = createIndexBufferAttribute(
                    meshBuffers.indices,
                    positionCount - 1
                )
            }
    ...
    private processSimpleLines(linesArray: LinesGeometry[]) {
        linesArray.forEach((linesGeometry) => {
            ...
            const geometry: Geometry = {
                ...
                index: createIndexBufferAttribute(
                    indices,
                    vertices.length / attr.itemCount - 1
                ),
    ...
    private findOrCreateMeshBuffers(
        ...
        if (buffers !== undefined) {
            if (buffers.type !== type) {
                logger.error(`MeshBuffer has been created with wrong type "${
                    GeometryType[type]
                }"
                    ...
                    if (!isExtruded) {
                        ...
                        if (maxSlope !== undefined) {
                            if (prevRingVertex !== undefined) {
                                const prevPos =
                                    (featureBaseVertex + prevRingVertex * 2) * 3
                                const currPos =
                                    (featureBaseVertex + currRingVertex * 2) * 3
                                const nextPos =
                                    (featureBaseVertex + nextRingVertex * 2) * 3
                                tmpEdgeA
                                    .set(
    ...
    private addWalls(
        ...
        while (currRingVertex < featureVertexCount) {
            while (currRingVertex !== firstRingVertex) {
                ...
                const nextRingVertex =
                    vertices[currRingVertex * vertexStride + featureStride + 1]
                if (nextRingVertex < 0) {
                    break
                } else {
                    indices.push(
