...
import { ValueMap } from '@arca/datasource-protocol/index-decoder'
import { webMercatorProjection } from '@arca/geoutils'
            ...
            switch (feature.type) {
                ...
                case VTJsonGeometryType.LineString: {
                    const lineGeometries =
                        feature.geometry as VTJsonPosition[][]
                    ...
                    lineGeometries.forEach((lineGeometry) => {
                        const lastPos =
                            lastLine?.positions[lastLine.positions.length - 1]
                        ...
                        if (lastPos?.x === startx && lastPos?.y === starty) {
                            ...
                            // start a new line
                            const positions = lineGeometry.map(
                                ([x, y]) => new Vector2(x, y)
                            )
                    ...
                    lines.forEach((line) => {
                        ;(line as any).untiledPositions = line.positions.map(
                            (tilePos) => {
                                tile2world(
                                    VT_JSON_EXTENTS,
                                    decodeInfo,
                                ...
                                return webMercatorProjection.unprojectPoint(
                                    worldPos
                                )
                            }
                        )
