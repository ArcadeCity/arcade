...
import { ValueMap } from '@arca/datasource-protocol'
import { clipLineString } from '@arca/geometry'
import {
    GeoCoordinates, GeoPointLike, Vector2Like, webMercatorProjection
} from '@arca/geoutils'
...
function convertPoint<VectorType extends Vector2Like>(
    ...
    webMercatorProjection.projectPoint(
        GeoCoordinates.fromGeoPoint(geoPoint),
        worldP
    )
...
function convertPolygonGeometry(
    ...
    return geometry.coordinates.map((polygon) =>
        convertRings(polygon, decodeInfo)
    )
}
...
function convertPointGeometry(
    ...
    return geometry.coordinates.map((geoPoint) =>
        ...
        for (const feature of featureCollection.features) {
            switch (feature.geometry.type) {
                ...
                case 'MultiLineString': {
                    let geometry = convertLineGeometry(
                        feature.geometry,
                        decodeInfo
                    )
                ...
                case 'Polygon':
                case 'MultiPolygon': {
                    const geometry = convertPolygonGeometry(
                        feature.geometry,
                        decodeInfo
                    )
                ...
                case 'Point':
                case 'MultiPoint': {
                    const geometry = convertPointGeometry(
                        feature.geometry,
                        decodeInfo
                    )
