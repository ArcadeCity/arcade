...
import {
    MapEnv, StyleSetEvaluator, Value, ValueMap
} from '@arca/datasource-protocol/index-decoder'
import { TileKey } from '@arca/geoutils'
import { assert, ILogger, LoggerManager } from '@arca/utils'

import { DataAdapter } from '../../DataAdapter'
import { DecodeInfo } from '../../DecodeInfo'
import {
    IGeometryProcessor, ILineGeometry, IPolygonGeometry
} from '../../IGeometryProcessor'
import {
    ComposedDataFilter, OmvFeatureFilter, OmvFeatureModifier,
    OmvGenericFeatureFilter, OmvGenericFeatureModifier
} from '../../OmvDataFilter'
import {
    FeatureModifierId, OmvDecoderOptions, OmvFeatureFilterDescription,
    OmvGeometryType
} from '../../OmvDecoderDefs'
import {
    OmvPoliticalViewFeatureModifier
} from '../../OmvPoliticalViewFeatureModifier'
import { isArrayBufferLike } from '../../OmvUtils'
import { StyleSetDataFilter } from '../../StyleSetDataFilter'
import {
    FeatureAttributes, GeometryCommands, isClosePathCommand, isLineToCommand,
    isMoveToCommand, OmvVisitor, visitOmv
} from './OmvData'
...
function decodeFeatureId(
    ...
    if (feature.hasOwnProperty('id')) {
        ...
        if (typeof id === 'number') {
            ...
            if (
                logger !== undefined &&
                id.greaterThan(Number.MAX_SAFE_INTEGER)
            ) {
                logger.error(
...
export function asGeometryType(
    feature: com.mapbox.pb.Tile.IFeature | undefined
): OmvGeometryType {
...
function roundUpPolygonCoordinates(
    geometry: IPolygonGeometry[],
    layerExtents: number
) {
    geometry.forEach((polygon) =>
        polygon.rings.forEach((r) => roundUpCoordinates(r, layerExtents))
    )
}

function roundUpLineCoordinates(
...
export class OmvDataAdapter implements DataAdapter, OmvVisitor {
    ...
    configure(
        ...
        if (options.filterDescription !== undefined) {
            if (options.filterDescription !== null) {
                ...
                this.m_dataFilter = new OmvGenericFeatureFilter(
                    filterDescription
                )
                ...
                if (featureModifiersIds !== undefined) {
                    featureModifiersIds.forEach((fmId) => {
                        featureModifiers.push(
                            createFeatureModifier(filterDescription, fmId)
                        )
                    })
                } else {
                    featureModifiers.push(
        ...
        if (options.politicalView !== undefined) {
            ...
            if (politicalView.length !== 0) {
                ...
                const povFeatureModifier = new OmvPoliticalViewFeatureModifier(
                    politicalView
                )
                if (featureModifiers) {
            ...
            this.m_featureModifiers =
                featureModifiers && featureModifiers.length > 0
                    ? featureModifiers
                    : undefined
        }
        this.m_roundUpCoordinatesIfNeeded =
            options.roundUpCoordinatesIfNeeded ?? false
    }
        ...
        this.m_geometryCommands.accept(feature.geometry, {
            ...
            visitCommand: (command) => {
                if (isMoveToCommand(command)) {
                    geometry.push(
                        new Vector3(command.position.x, command.position.y, 0)
                    )
                }
            },
        })
    ...
    visitPolygonFeature(feature: com.mapbox.pb.Tile.IFeature): void {
        ...
        // modifier, processFeature will later apply it to all using Function.apply().
        const modifierFunc =
            this.m_featureModifiers?.[0].doProcessPolygonFeature
            ...
            visitCommand: (command) => {
                if (isMoveToCommand(command)) {
                    ...
                    if (currentRing !== undefined && currentRing.length > 0) {
                        const currentRingWinding = Math.sign(
                            ShapeUtils.area(currentRing)
                        )
    ...
    private filterAndModifyFeature(
        ...
        if (
            this.m_dataFilter &&
            filterFunc!.apply(this.m_dataFilter, [
                layerName,
                geometryType,
                storageLevel,
            ]) === false
        ...
        if (
            this.m_featureModifiers?.find((fm) => {
                ...
                return !modifierFunc!.apply(fm, [
                    layerName,
                    env,
                    this.m_tileKey.level,
                ])
            }) !== undefined
        ) {
            return undefined
