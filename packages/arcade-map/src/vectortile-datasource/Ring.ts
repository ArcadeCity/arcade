...
import { ClippedVertex } from '@arca/geometry'
...
export class Ring {
    ...
    toArray(array: number[] = [], offset: number = 0): number[] {
        this.points.forEach((p, i) =>
            p.toArray(array, offset + this.vertexStride * i)
        )
        this.textureCoords?.forEach((p, i) =>
            p.toArray(array, offset + this.vertexStride * i + 2)
        )
        return array
    }
    ...
    isProperEdge(index: number): boolean {
        ...
        if (this.hasClipInfo) {
            ...
            const currAtEdge =
                curr.x % this.extents === 0 || curr.y % this.extents === 0
            ...
            const nextAtEdge =
                next.x % this.extents === 0 || next.y % this.extents === 0
