...
import { DataProvider } from '@arca/mapview-decoder'
import { ITransferManager, TransferManager } from '@arca/transfer-manager'
...
export interface OmvRestClientParameters {
     ...
     *  - `{z}` - zoom level of tile, @see {@link @arca/geoutils#TileKey.level}
     *  - `{x}` - horizontal coordinate of tile (column number),
     *            see {@link @arca/geoutils#TileKey.column}
     *  - `{y}` - vertical coordinate of Tile (row number),
     *            see {@link @arca/geoutils#TileKey.row}
     *
...
export class OmvRestClient extends DataProvider {
    ...
    private applyAuthCode(
        ...
        const authMethod =
            this.params.authenticationMethod ?? this.getDefaultAuthMethod()
    ...
    private dataUrl(tileKey: TileKey): string {
        ...
        switch (this.params.apiFormat) {
            ...
            default:
                logger.warn(
                    `Not supported API format: ${this.params.apiFormat}`
                )
                break
        }
    ...
    private addQueryParams(url: string, queryParams: QueryParameters): string {
        ...
        for (const prop in queryParams) {
            ...
            queryString += `${sep}${encodeURIComponent(
                prop
            )}=${encodeURIComponent(queryParams[prop])}`
            if (sep === '?') {
                sep = '&'
            }
        }
