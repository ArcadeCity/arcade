 ...
 */
import { getPropertyValue, isTextTechnique } from '@arca/datasource-protocol'
import { TileKey } from '@arca/geoutils'
import {
    DataSource, debugContext, TextElement, TextElementType, Tile,
    TileGeometryCreator
} from '@arca/mapview'
import {
    ContextualArabicConverter, FontUnit, TextLayoutStyle, TextRenderStyle
    ...
    if (debugGeometry.indices.length > 0) {
        ...
        debugGeometry.geometry.setAttribute(
            ...
            new THREE.BufferAttribute(
                new Float32Array(debugGeometry.positions),
                3
            )
        )
...
export class OmvDebugLabelsTile extends Tile {
    ...
    loadingFinished() {
        ...
        const debugTextPathsFull = debugContext.getValue(
            'DEBUG_TEXT_PATHS_FULL'
        )
        ...
        const indexFilter = debugContext.getValue(
            'DEBUG_TEXT_PATHS.FILTER.INDEX'
        )
        ...
        if (this.textElementGroups.count() > 0) {
            ...
            this.textElementGroups.forEach((textElement: TextElement) => {
                ...
                if (
                    textElement.path !== undefined &&
                    Array.isArray(textElement.path)
                ) {
        ...
        if (this.preparedTextPaths !== undefined) {
            ...
            for (const textPath of this.preparedTextPaths) {
                ...
                if (
                    ...
                    (textPath as any)._isDebug !== undefined
                ) {
                    continue
                }
                ...
                if (createDebugInfo) {
                    for (let i = 0; i < textPath.path.length; i += 3) {
                        ...
                        const z =
                            zHeight + textPath.path[i + 2] + i / 3 - centerZ
                        ...
                        if (debugTextPathsFull || isRedPoint) {
                            ...
                            if (debugTextPathsFull) {
                                ...
                                const labelElement = new TextElement(
                                    ContextualArabicConverter.instance.convert(
                                        label
                                    ),
                                    new THREE.Vector3(
                                ...
                                labelElement.minZoomLevel = getPropertyValue(
                                    minZoomLevel,
                                    env
                                )
                                labelElement.mayOverlap = true
