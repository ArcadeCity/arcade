...
import {
    OmvFeatureFilterDescription, OmvFilterDescription,
    OmvFilterFeatureAttribute, OmvFilterString, OmvGeometryType,
    ...
    constructor(
        ...
        if (options) {
            this.m_processLayersDefault =
                options.processLayersDefault !== undefined
                    ? options.processLayersDefault
                    : true
            this.m_processPointsDefault =
                options.processPointsDefault !== undefined
                    ? options.processPointsDefault
                    : true
            this.m_processLinesDefault =
                options.processLinesDefault !== undefined
                    ? options.processLinesDefault
                    : true
...
export class OmvGenericFeatureFilter implements OmvFeatureFilter {
    ...
    wantsLayer(layer: string, level: number): boolean {
        if (
            OmvGenericFeatureFilter.matchLayer(
                layer,
        ...
        if (
            OmvGenericFeatureFilter.matchLayer(
                layer,
                this.description.layersToIgnore,
                level
            )
        ) {
            return false
    ...
    get hasKindFilter(): boolean {
        return (
            this.enabledKinds !== undefined || this.disabledKinds !== undefined
        )
    }
    ...
    private wantsFeature(
        ...
        for (const item of itemsToProcess) {
            ...
            if (
                ...
                item.geometryTypes.includes(geometryType)
            ) {
                return true
            }
        }
...
export class ComposedDataFilter implements OmvFeatureFilter {
    ...
    wantsPointFeature(
        ...
        return this.filters.reduce<boolean>(
            (result, filter) =>
                result && filter.wantsPointFeature(layer, geometryType, level),
            true
        )
    }
    ...
    wantsLineFeature(
        ...
        return this.filters.reduce<boolean>(
            (result, filter) =>
                result && filter.wantsLineFeature(layer, geometryType, level),
            true
        )
    }
    ...
    wantsPolygonFeature(
        ...
        return this.filters.reduce<boolean>(
            (result, filter) =>
                result &&
                filter.wantsPolygonFeature(layer, geometryType, level),
            true
        )
    }
...
export class OmvGenericFeatureModifier implements OmvFeatureModifier {
        ...
        for (const item of items) {
            if (item.featureAttribute !== undefined) {
                if (
                    ...
                    env.lookup(item.featureAttribute.key) ===
                        item.featureAttribute.value
                ) {
                    return true
                }
            }
    ...
    protected doProcessFeature(
        ...
        if (
            layer === undefined ||
            (itemsToProcess.length === 0 && itemsToIgnore.length === 0)
        ) {
            return defaultResult
        }
        ...
        if (
            featureClass &&
            OmvGenericFeatureModifier.matchItems(
                layer,
                featureClass,
                itemsToProcess
            )
        ...
        if (
            OmvGenericFeatureModifier.matchAttribute(layer, env, itemsToProcess)
        ) {
            return true
        }

        if (
            OmvGenericFeatureModifier.matchAttribute(layer, env, itemsToIgnore)
        ) {
            return false
        }
