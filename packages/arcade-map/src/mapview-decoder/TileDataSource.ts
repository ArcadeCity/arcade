 ...
 */
import {
    getStyles, ITileDecoder, OptionsMap, Theme, TileInfo
} from '@arca/datasource-protocol'
import { TileKey, TilingScheme } from '@arca/geoutils'
import {
    ConcurrentDecoderFacade, CopyrightInfo, CopyrightProvider, DataSource,
    DataSourceOptions, ThemeLoader, Tile, TileLoaderState
} from '@arca/mapview'
import { ILogger, LoggerManager } from '@arca/utils'
...
export class TileDataSource<TileType extends Tile = Tile> extends DataSource {
    protected readonly logger: ILogger =
        LoggerManager.instance.create('TileDataSource')
    ...
    async connect() {
        await Promise.all([
            this.m_options.dataProvider.register(this),
     ...
     *
     * Applies new {@arca/datasource-protocol StyleSet} and definitions from theme only
        ...
        const promise = new Promise<TileInfo | undefined>((resolve, reject) => {
            ...
            tileLoader.loadAndDecode().then((loaderState) => {
                if (loaderState === TileLoaderState.Ready) {
                    ...
                    reject(
                        new Error(
                            `TileDataSource#getInfoTile wrong final state: ${loaderState}`
                        )
                    )
                }
            })
