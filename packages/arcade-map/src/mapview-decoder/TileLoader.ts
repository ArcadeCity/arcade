...
import {
    DecodedTile, ITileDecoder, RequestController, TileInfo
} from '@arca/datasource-protocol'
import { TileKey } from '@arca/geoutils'
import { BaseTileLoader, DataSource, TileLoaderState } from '@arca/mapview'
...
export class TileLoader extends BaseTileLoader {
    ...
    protected startDecodeTile(
        ...
        if (payload === undefined) {
            logger.error(
                'TileLoader#startDecodeTile: Cannot decode without payload'
            )
            return
        }
...
export class TileInfoLoader extends TileLoader {
    ...
    protected startDecodeTile(
        ...
        if (payload === undefined) {
            logger.error(
                'TileInfoLoader#startDecodeTile: Cannot decode without payload'
            )
            return
        }
