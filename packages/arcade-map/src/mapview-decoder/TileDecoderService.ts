...
import {
    addBuffersToTransferList, BufferAttribute, getProjection,
    InterleavedBufferAttribute, ITileDecoder, WorkerDecoderProtocol
...
export class TileDecoderService extends WorkerService {
    ...
    private async handleDecodeTileRequest(
        ...
        decodedTile?.geometries.forEach((geom) => {
            geom.vertexAttributes?.forEach((attr) =>
                transferBufferAttribute(attr)
            )
            geom.interleavedVertexAttributes?.forEach((attr) =>
                transferBufferAttribute(attr)
            )
    ...
    private handleTileInfoRequest(
        ...
        return this.m_decoder
            .getTileInfo(request.data, tileKey, projection)
            .then((tileInfo) => {
                const transferList: ArrayBuffer[] =
                    tileInfo !== undefined &&
                    tileInfo.transferList !== undefined
    ...
    private handleConfigurationMessage(
        message: WorkerDecoderProtocol.ConfigurationMessage
    ) {
