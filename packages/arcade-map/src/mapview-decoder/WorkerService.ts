...
export abstract class WorkerService {
    ...
    protected handleRequest(request: any): Promise<WorkerServiceResponse> {
        throw new Error(
            `ServiceAdapter[${this.serviceId}]: Invalid request '${request.type}'`
        )
    }
    ...
    private readonly onMessage = (message: MessageEvent) => {
        if (
            typeof message.data.service !== 'string' ||
            message.data.service !== this.serviceId
        ) {
            return
        }
        ...
        try {
                    ...
                    .then((response) => {
                        this.doSendResponse(
                            requestEntry,
                            {
                                ...
                                type: WorkerServiceProtocol.ServiceMessageName
                                    .Response,
                                messageId: request.messageId,
            ...
            logger.error(
                `[${this.serviceId}]: Unhandled exception when handling ${message.type}`
            )
        }
    }
    ...
    private tryHandleMessage(message: any): void {
        try {
            ...
            logger.error(
                `[${this.serviceId}]: Failed, handling message ${message.type}`
            )
        }
    }
