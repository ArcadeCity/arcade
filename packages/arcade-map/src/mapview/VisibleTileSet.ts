...
import {
    GeoCoordinates, Projection, ProjectionType, TileKey, TileKeyUtils,
    TilingScheme
} from '../geoutils'
...
class DataSourceCache {
    ...
    constructor(
        ...
        this.m_tileCache = new LRUCache<string, Tile>(
            cacheSize,
            (tile: Tile) => {
                if (
    ...
    get(
        ...
        return this.m_tileCache.get(
            DataSourceCache.getKey(mortonCode, offset, dataSource)
        )
    }
     ...
     */
    forEach(
        callback: (tile: Tile, key: TileCacheId) => void,
        inDataSource?: DataSource
    ): void {
        this.m_tileCache.forEach((entry: Tile, key: TileCacheId) => {
            if (
                inDataSource === undefined ||
                entry.dataSource === inDataSource
            ) {
                callback(entry, key)
            }
        })
...
export class VisibleTileSet {
    ...
    constructor(
        ...
        this.options.maxTilesPerFrame = Math.floor(
    ...
    set maxTilesPerFrame(value: number) {
        if (value < 0) {
            throw new Error(
                'Invalid value, this will result in no tiles ever showing'
            )
        }
    ...
    set resourceComputationType(computationType: ResourceComputationType) {
        ...
        this.m_dataSourceCache.setCapacity(
            this.options.tileCacheSize,
            computationType
        )
    }
    ...
    updateClipPlanes(maxElevation?: number, minElevation?: number): ViewRanges {
        ...
        const { camera, projection, elevationProvider } =
    ...
    updateRenderList(
        ...
        for (const {
            dataSource,
            visibleTileKeys,
        } of visibleTileKeysResult.tileKeys) {
            ...
            this.dataSourceTileList.push({
                ...
                numTilesLoading:
                    visibleResult.numTilesLoading +
                    dependentResult.numTilesLoading,
                visibleTiles: [
                    ...visibleResult.visibleTiles,
                    ...dependentResult.visibleTiles,
                ],
                renderedTiles: new Map<number, Tile>(),
            })
            allVisibleTilesLoaded =
                allVisibleTilesLoaded && visibleResult.allDataSourceTilesLoaded
        }
        ...
        this.dataSourceTileList.forEach((renderListEntry) => {
            ...
            tiles.forEach((tile) => {
                ...
                minElevation = MathUtils.min2(
                    minElevation,
                    tile.geoBox.minAltitude
                )
                maxElevation = MathUtils.max2(
                    maxElevation,
                    tile.geoBox.maxAltitude
                )
            })
        })
        ...
        viewRangesChanged =
            viewRangesEqual(newViewRanges, oldViewRanges) === false
    ...
    getRenderedTile(
        ...
        const dataSourceVisibleTileList = this.dataSourceTileList.find(
            (list) => {
        ...
        if (dataSource !== undefined) {
            this.m_dataSourceCache.evictSelected((tile: Tile, _) => {
                return (
                    ...
                    (filter !== undefined ? filter(tile) : true)
                )
            })
    ...
    markTilesDirty(dataSource?: DataSource, filter?: (tile: Tile) => boolean) {
        if (dataSource === undefined) {
            ...
            const renderListEntry = this.dataSourceTileList.find(
    ...
    /**
     * Dispose tiles that are marked for removal by {@link @arca/lrucache#LRUCache} algorithm.
     */
    disposePendingTiles() {
    ...
    private processVisibleTiles(
        ...
        for (
            ...
            allDataSourceTilesLoaded =
                allDataSourceTilesLoaded && tile.allGeometryLoaded
            if (!tile.allGeometryLoaded) {
                ...
                if (
                    this.processDelayTileRendering(
                        tile,
                ...
                for (const tileKey of tile.dependencies) {
                    if (
                        visibleTileKeys.find(
                            (tileKeyEntry) =>
                                tileKeyEntry.tileKey.mortonCode() ===
                                tileKey.mortonCode()
                        ) === undefined &&
    ...
    private populateRenderedTiles() {
        this.dataSourceTileList.forEach((renderListEntry) => {
            ...
            if (
                ...
                dataSource.allowOverlappingTiles === false
            ) {
            ...
            const { searchDirection } = this.getSearchDirection(
                dataSource,
                dataZoomLevel
            )
    ...
    private findDown(
        ...
        const { offset, mortonCode } =
            TileKeyUtils.extractOffsetAndMortonKeyFromKey(tileKeyCode)
        const tileKey = TileKey.fromMortonCode(mortonCode)

        const tilingScheme = dataSource.getTilingScheme()
        for (const childTileKey of tilingScheme.getSubTileKeys(tileKey)) {
            const childTileCode = TileKeyUtils.getKeyForTileKeyAndOffset(
                childTileKey,
                offset
            )
    ...
    private findUp(
        ...
        const { offset, mortonCode } =
            TileKeyUtils.extractOffsetAndMortonKeyFromKey(parentCode)
        const parentTile = this.m_dataSourceCache.get(
            mortonCode,
            offset,
            dataSource
        )
        ...
        if (
            nextLevelDiff < this.options.quadTreeSearchDistanceUp &&
            parentTileKey.level !== 0
        ) {
    ...
    private markDataSourceTilesDirty(
        ...
        dataSourceCache.forEach((tile, key) => {
            if (
                (filter === undefined || filter(tile)) &&
                !retainedTiles.has(key)
            ) {
    ...
    private getVisibleTileKeysForDataSources(
        ...
        if (elevationRangeSource !== undefined) {
            ...
            this.m_frustumIntersection.updateFrustum(
                this.m_cameraOverride.projectionMatrix
            )
        } else {
            this.m_frustumIntersection.updateFrustum()
        }
        ...
        for (const [tilingScheme, bucket] of dataSourceBuckets) {
            const zoomLevels = bucket.map((dataSource) =>
                dataSource.getDataZoomLevel(zoomLevel)
            )
            ...
            allBoundingBoxesFinal =
                allBoundingBoxesFinal && result.calculationFinal
            ...
            for (const dataSource of bucket) {
                ...
                const dataZoomLevel = dataSource.getDataZoomLevel(zoomLevel)
                for (const tileKeyEntry of result.tileKeyEntries
                    .get(dataZoomLevel)!
                    .values()) {
                    if (
                        dataSource.canGetTile(
                            dataZoomLevel,
                            tileKeyEntry.tileKey
                        )
                    ) {
