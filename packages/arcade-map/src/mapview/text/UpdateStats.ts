...
import { IChannel } from '@arca/utils'
...
export class UpdateStats {
    ...
    log() {
        ...
        this.m_logger.debug(
            'Placed labels',
            this.results[PrePlacementResult.Ok]
        )
        this.m_logger.debug(
            'Invisible',
            this.results[PrePlacementResult.Invisible]
        )
        this.m_logger.debug(
            'Poi not ready',
            this.results[PrePlacementResult.NotReady]
        )
        this.m_logger.debug('Too far', this.results[PrePlacementResult.TooFar])
        this.m_logger.debug(
            'Duplicate',
            this.results[PrePlacementResult.Duplicate]
        )
    }
}
