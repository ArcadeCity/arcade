...
import {
    AttributeMap, Env, getFeatureId, getPropertyValue, IndexedTechniqueParams,
    isTextTechnique, LineMarkerTechnique, MapEnv, PoiTechnique, TextTechnique
} from '@arca/datasource-protocol'
import {
    ContextualArabicConverter, TextLayoutStyle, TextRenderStyle
} from '@arca/text-canvas'
import { assert, LoggerManager, MathUtils } from '@arca/utils'
...
/**
 * Constructs {@link TextElement} objects from {@link @arca/datasource-protocol/Technique},
...
export class TextElementBuilder {
    ...
    static composeRenderOrder(baseRenderOrder: number, offset: number): number {
        return (
            baseRenderOrder * TextElementBuilder.RENDER_ORDER_UP_BOUND + offset
        )
    }
     ...
     * Constructor
     *
     * @param m_env - The {@link @link @arca/datasource-protocol#MapEnv} used to evaluate
     * technique properties.
     * @param m_styleCache - To cache instances of {@link @arca/text-canvas/TextRenderStyle}
     * and {@link @arca/text-canvas/TextLayoutStyle}.
     */
    constructor(
     ...
     *
     * @param technique - The {@link @arca/datasource-protocol/Technique}.
     * @return This builder.
     */
    withTechnique(
        technique: (PoiTechnique | LineMarkerTechnique | TextTechnique) &
            IndexedTechniqueParams
    ): this {
        ...
        this.m_fadeNear =
            getPropertyValue(technique.fadeNear, this.m_env) ?? undefined
        this.m_fadeFar =
            getPropertyValue(technique.fadeFar, this.m_env) ?? undefined
        this.m_minZoomLevel =
            getPropertyValue(technique.minZoomLevel, this.m_env) ?? undefined
        this.m_maxZoomLevel =
            getPropertyValue(technique.maxZoomLevel, this.m_env) ?? undefined
        this.m_distanceScale =
            technique.distanceScale ?? DEFAULT_TEXT_DISTANCE_SCALE
        ...
        const techniqueRenderOrder =
            getPropertyValue(technique.renderOrder, this.m_env) ?? 0
    ...
    build(
        ...
        textElement.textFadeTime =
            technique.textFadeTime !== undefined
                ? technique.textFadeTime * 1000
                : undefined
