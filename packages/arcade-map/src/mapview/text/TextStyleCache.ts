 ...
 */
import {
    ColorUtils, getPropertyValue, isPoiTechnique, LineMarkerTechnique, MapEnv,
    PlacementToken, PoiTechnique, TextStyleDefinition, TextTechnique
} from '@arca/datasource-protocol'
import {
    DefaultTextStyle, FontStyle, FontUnit, FontVariant, HorizontalAlignment,
    HorizontalPlacement, resolvePlacementAndAlignment, TextCanvas,
    TextLayoutParameters, TextLayoutStyle, TextPlacement, TextPlacements,
    TextRenderParameters, TextRenderStyle, VerticalAlignment, VerticalPlacement,
    WrappingMode
} from '@arca/text-canvas'
import { getOptionValue, LoggerManager } from '@arca/utils'
 ...
 * {@link @arca/datasource-protocol#Theme}'s TextStyle.
 */
...
export class TextStyleCache {
    ...
    /**
     * Retrieves a {@link TextElementStyle} for {@link @arca/datasource-protocol#Theme}'s
     * [[TextStyle]] id.
     */
    ...
    /**
     * Gets the appropriate {@link @arca/text-canvas#TextRenderStyle}
    ...
    createRenderStyle(
        ...
        if (technique.color !== undefined) {
            let hexColor = evaluateColorProperty(
                technique.color,
                discreteZoomEnv
            )
        ...
        const renderParams = {
            fontName: getOptionValue(
                technique.fontName,
                defaultRenderParams.fontName
            ),
            fontSize: {
                unit: FontUnit.Pixel,
            ...
            rotation: getOptionValue(
                technique.rotation,
                defaultRenderParams.rotation
            ),
            color: getOptionValue(
                color,
                getOptionValue(
                    defaultRenderParams.color,
                    DefaultTextStyle.DEFAULT_COLOR
                )
            ),
        ...
        const themeRenderParams = this.getTextElementStyle(
            technique.style
        ).renderParams
    ...
    /**
     * Create the appropriate {@link @arca/text-canvas#TextLayoutStyle}
     * to use for a label. Depends heavily on the label's
     * [[Technique]] and the current zoomLevel.
     *
    ...
    createLayoutStyle(
        ...
        const hAlignment = getPropertyValue(
            technique.hAlignment,
            discreteZoomEnv
        ) as string | null
        const vAlignment = getPropertyValue(
        ...
        const textPlacements = isPoiTechnique(technique)
            ? (getPropertyValue(
                  (technique as PoiTechnique).placements,
                  discreteZoomEnv
              ) as string | null)
            : null

        const { horizontalAlignment, verticalAlignment, placements } =
            parseAlignmentAndPlacements(hAlignment, vAlignment, textPlacements)

        const wrapping = getPropertyValue(
    ...
    private initializeTextCanvas(
        ...
        if (style.textCanvas === undefined) {
            ...
            let alternativeTextCanvas = textCanvases.get(
    ...
    private createTextElementStyle(
        ...
        return {
            name: styleName,
            fontCatalog: getOptionValue(
                style.fontCatalogName,
                this.m_defaultStyle.fontCatalog
            ),
            renderParams: {
...
function parseTechniqueHAlignValue(
    hAlignment: string | undefined | null
): HorizontalAlignment {
...
function parseTechniqueVAlignValue(
    vAlignment: string | undefined | null
): VerticalAlignment {
    return vAlignment === 'Above' ||
        vAlignment === 'Center' ||
        vAlignment === 'Below'
...
function parseTechniquePlacements(
    placementsString: string | undefined | null
): TextPlacements {
