...
import { assert, LoggerManager, LogLevel } from '@arca/utils'
...
function findDuplicateByText(
    ...
    const isBetterDuplicate: DuplicateCmp =
        element.type === TextElementType.PathLabel
            ? isBetterPathDuplicate
            : isBetterPointDuplicate
    ...
    for (let i = 0; i < entryCount; ++i) {
        ...
        const distSquared = elementPosition.distanceToSquared(
            cachedElement.position
        )
...
export class TextElementStateCache {
    private readonly m_referenceMap = new Map<
        TextElementGroup,
    ...
    getOrSet(
        ...
        groupState = new TextElementGroupState(
            textElementGroup,
            tileKey,
            textElementFilter
        )
    ...
    get sortedGroupStates(): TextElementGroupState[] {
        if (this.m_sortedGroupStates === undefined) {
            ...
            this.m_sortedGroupStates.sort(
                (a: TextElementGroupState, b: TextElementGroupState) => {
    ...
    private get(
        textElementGroup: TextElementGroup
    ): TextElementGroupState | undefined {
    ...
    private set(
        textElementGroup: TextElementGroup,
        textElementGroupState: TextElementGroupState
    ) {
