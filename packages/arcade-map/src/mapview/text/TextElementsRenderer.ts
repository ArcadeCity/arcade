 ...
 */
import {
    FontCatalogConfig, LineMarkerTechnique, TextStyleDefinition
} from '@arca/datasource-protocol'
import { TileKey, Vector3Like } from '@arca/geoutils'
import {
    AdditionParameters, DEFAULT_TEXT_CANVAS_LAYER, FontCatalog,
    TextBufferAdditionParameters, TextBufferCreationParameters, TextCanvas,
    TextLayoutStyle, TextRenderStyle
} from '@arca/text-canvas'
import {
    assert, LoggerManager, LogLevel, Math2D, MathUtils, PerformanceTimer
} from '@arca/utils'
...
import {
    IBox, LineWithBound, ScreenCollisions, ScreenCollisionsDebug
...
import {
    checkReadyForPlacement, computeViewDistance, getMaxViewDistance,
    getWorldPosition, isPathLabelTooSmall, placeIcon, PlacementResult,
    placePathLabel, placePointLabel, pointToPlaneDistance, PrePlacementResult
} from './Placement'
...
import {
    TextElementFilter, TextElementGroupState
} from './TextElementGroupState'
import {
    initializeDefaultOptions, TextElementsRendererOptions
...
const placementStats = PRINT_LABEL_DEBUG_INFO
    ? new PlacementStats(logger)
    : undefined
...
function addTextToCanvas(
    ...
    tmpAdditionParams.pickingData = textElement.userData
        ? textElement
        : undefined
...
function addTextBufferToCanvas(
    ...
    const opacity =
        textRenderState!.opacity * fadeFactor * textElement.renderStyle!.opacity
    ...
    tmpBufferAdditionParams.backgroundOpacity = backgroundIsVisible
        ...
        : 0.0
    tmpBufferAdditionParams.pickingData = textElement.userData
        ? textElement
        : undefined
    canvas.addTextBufferObject(
...
function shouldRenderPointText(
    ...
    const poiTextMaxDistance = getMaxViewDistance(
        viewState,
        options.maxDistanceRatioForPoiLabels!
    )
    ...
    return (
        !poiInfo || poiInfo.isValid === true || poiInfo.iconIsOptional === true
    )
}

function shouldRenderPoiText(
    labelState: TextElementState,
    viewState: ViewState
) {
...
function createDefaultFontCatalogConfig(
    defaultFontCatalogUrl: string
): FontCatalogConfig {
    return {
...
export class TextElementsRenderer {
    ...
    private readonly m_textElementStateCache: TextElementStateCache =
        new TextElementStateCache()
    ...
    constructor(
        ...
        this.m_textCanvasFactory =
            textCanvasFactory ?? new TextCanvasFactory(this.m_renderer)
    ...
    placeText(dataSourceTileList: DataSourceTileList[], time: number) {
        const tileTextElementsChanged =
            checkIfTextElementsChanged(dataSourceTileList)

        const textElementsAvailable =
            this.hasOverlayText() ||
            tileTextElementsChanged ||
            hasTextElements(dataSourceTileList)
    ...
    hasOverlayText(): boolean {
        return (
            this.m_overlayTextElements !== undefined &&
            this.m_overlayTextElements.length > 0
        )
    }
    ...
    pickTextElements(
        ...
        this.m_poiRenderer.pickTextElements(
            screenPosition,
            (pickData: any | undefined) => {
        ...
        dataSourceTileList.forEach((renderListEntry) => {
            ...
            for (const tile of renderListEntry.renderedTiles.values()) {
                for (const pathBlockingElement of tile.blockingElements) {
                    ...
                    for (
                        let i = 1;
                        i < pathBlockingElement.points.length;
                        i++
                    ) {
                        this.m_screenProjector.project3(
    ...
    private placeTextElementGroup(
        ...
        const elevationMap = elevationProvider?.getDisplacementMap(
            groupState.tileKey
        )
        ...
        for (const textElementState of groupState.textElementStates) {
            ...
            if (
                textElementState.viewDistance === undefined ||
                textElementState.viewDistance < 0
            ) {
                if (placementStats) {
            ...
            const textElementStyle = this.m_textStyleCache.getTextElementStyle(
                textElement.style
            )
    ...
    private async addTextCanvas(
        ...
        if (this.m_textCanvases.has(fontCatalogConfig.name)) {
            ...
            const newLoadPromise = loadFontCatalog(
                ...
                .catch((error) => {
                    logger.info(
                        'rendering without font catalog, only icons possible',
                        error
                    )
                    --this.m_loadPromisesCount
                })
    ...
    private initializeGlyphDebugMesh() {
        ...
        this.m_debugGlyphTextureCacheMesh = new THREE.Mesh(
            planeGeometry,
            material
        )
    ...
    private updateTextElements(dataSourceTileList: DataSourceTileList[]) {
        ...
        const updateStartTime =
            this.overloaded && this.m_viewState.isDynamic
    ...
    private updateTextElementsFromSource(
        ...
        this.createSortedGroupsForSorting(
            tileDataSource,
            storageLevel,
            sortedTiles,
            sortedGroups
            ...
            for (const group of tile.textElementGroups.groups.values()) {
                ...
                if (foundGroup === undefined) {
                    groupedPriorityLists.set(
                        group.priority,
                        new TextElementLists([
                            new TileTextElements(tile, group),
                        ])
                    )
                } else {
    ...
    private selectTextElementsToUpdateByDistance(
        ...
        const maxViewDistance = getMaxViewDistance(
            this.m_viewState,
            farDistanceLimitRatio
        )
    ...
    private placeTextElements(time: number, placeNewTextElements: boolean) {
        ...
        if (placeNew) {
            ...
            this.placeNewTextElements(
                currentPriorityBegin,
                groupStates.length,
                renderParams
            )
        }
        ...
        if (placementStats) {
            placementStats.numRenderedTextElements =
        ...
        for (const textElement of this.m_overlayTextElements!) {
            ...
            if (!(textElement.type === TextElementType.PathLabel)) {
                ...
                tempScreenPosition.x =
                    screenXOrigin + textElement.position.x * screenSize.width
                tempScreenPosition.y =
                    screenYOrigin - textElement.position.y * screenSize.height
            ...
            if (fadeFar > fadeNear) {
                distanceFadeValue =
                    ...
                    THREE.MathUtils.clamp(
                        (textDistance / maxVisibilityDist - fadeNear) /
                            (fadeFar - fadeNear),
                        0.0,
                        1.0
                    )
    ...
    private addPointLabel(
        ...
        const textRenderState: RenderState | undefined =
        ...
        if (
            ...
            (pointLabel.fadeFar <= 0.0 ||
                pointLabel.fadeFar * this.m_viewState.maxVisibilityDist <
                    textDistance)
        ) {
        ...
        const iconReady =
            renderIcon &&
            this.m_poiRenderer.prepareRender(pointLabel, this.m_viewState.env)
        ...
        if (
            textCanvas &&
            shouldRenderPointText(labelState, this.m_viewState, this.m_options)
        ) {
            ...
            const requiredIconRejected =
                iconRejected && iconReady && !iconIsOptional
            const textRejected =
                requiredIconRejected || placeResult === PlacementResult.Rejected
            ...
            const textNeedsDraw =
                !textInvisible &&
                ((!textRejected &&
                    shouldRenderPoiText(labelState, this.m_viewState)) ||
                    textRenderState!.isFading())

            if (textNeedsDraw) {
                if (!textRejected) {
                    textRenderState!.startFadeIn(
                        renderParams.time,
                        this.m_options.disableFading
                    )
                }
        ...
        if (iconReady && !iconInvisible) {
            ...
            if (opacity > 0) {
                ...
                const allocateSpace =
                    poiInfo!.reserveSpace !== false && !iconRejected
    ...
    private addLineMarkerLabel(
        ...
        if (
            !this.m_poiRenderer?.prepareRender(
                lineMarkerLabel,
                this.m_viewState.env
            )
        ) {
    ...
    private addPathLabel(
        ...
        if (
            ...
            (pathLabel.fadeFar <= 0.0 ||
                pathLabel.fadeFar * this.m_viewState.maxVisibilityDist <
                    labelState.renderDistance)
        ) {
            // The label is farther away than fadeFar value, which means it is totally
            // transparent
        ...
        for (let i = 0; i < screenPoints.length - 1; ++i) {
            textPath.add(
                new SimpleLineCurve(screenPoints[i], screenPoints[i + 1])
            )
        }
        ...
        textCanvas.textRenderStyle.backgroundOpacity =
            textCanvas.textRenderStyle.opacity *
            pathLabel.renderStyle!.backgroundOpacity
    ...
    private labelPotentiallyVisible(
        ...
        const maxDistance = THREE.MathUtils.clamp(
            this.m_options.maxPoiDistanceToBorder ?? 0,
            0,
            1
        )
