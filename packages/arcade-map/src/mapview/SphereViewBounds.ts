...
import {
    EarthConstants, GeoCoordinates, GeoPolygon, GeoPolygonCoordinates,
    isAntimeridianCrossing, Projection, ProjectionType
} from '@arca/geoutils'
import { assert } from '@arca/utils'

import {
    CanvasSide, nextCanvasSide, previousCanvasSide, SphereHorizon
} from './SphereHorizon'
...
export function computeEdgeDivisions(
    ...
    return Math.ceil(
        Math.max(
            latitudeSpan / maxLatitudeSpan,
            longitudeSpan / maxLongitudeSpan
        )
    )
}
...
export class SphereViewBounds implements ViewBounds {
    ...
    private addSideSegmentSubdivisions(
        ...
        const NDCStep = new Vector2(
            NDCEnd.x - NDCStart.x,
            NDCEnd.y - NDCStart.y
        ).multiplyScalar(1 / divisionCount)
    ...
    private addSideIntersections(
        ...
        if (geoStartCorner) {
            ...
            const geoHorizonPoint =
                this.projection.unprojectPoint(worldHorizonPoint)
            this.addSideSegmentSubdivisions(
                coordinates,
                startNDCCorner,
            ...
            if (prevSideIntersections.length === 0) {
                ...
                prevSideIntersections = horizon.getSideIntersections(
                    previousCanvasSide(prevSide)
                )
            }
        ...
        if (horizonIntersections.length > 1) {
            ...
            const geoHorizonStart =
                this.projection.unprojectPoint(worldHorizonStart)
            const geoHorizonEnd =
                this.projection.unprojectPoint(worldHorizonEnd)
    ...
    private findBoundsIntersections(): GeoCoordinates[] {
        ...
        const [cornerCoordinates, numCorners] =
            this.addCanvasCornerIntersection()
    ...
    private wrapAroundPoles(coordinates: GeoCoordinates[]) {
        ...
        const northPoleCenter = new Vector3(
            0,
            0,
            EarthConstants.EQUATORIAL_RADIUS
        )
        const southPoleCenter = new Vector3(
            0,
            0,
            -EarthConstants.EQUATORIAL_RADIUS
        )
        const northPoleInView = MapViewUtils.closeToFrustum(
            northPoleCenter,
            this.camera
        )
        const southPoleInView = MapViewUtils.closeToFrustum(
            southPoleCenter,
            this.camera
        )
        ...
        const hSphereCrossEndIndex = coordinates.findIndex(
            (value: GeoCoordinates) => {
                const crossesAntimer = isAntimeridianCrossing(
                    prevLon,
                    value.lng
                )
                const sameSign =
                    Math.sign(crossLon - value.lng) ===
                    Math.sign(crossLon - prevLon)
        ...
        const geoWrapRight = new GeoCoordinates(
            wrapLat,
        ...
        // Added to ensure antimeridian crossing detection when coordinates are wrapped around it by
        // GeoPolygon (all polygon sides must have longitude spans smaller than 180 degrees).
        const geoWrapLeft = new GeoCoordinates(
            wrapLat,
            camLon - wrapSideOffset
        ).normalized()
        const geoWrapTopLeft = new GeoCoordinates(
            wrapLat,
        ...
        // camera antimeridian.
        let crossLerp = GeoCoordinates.lerp(
            crossStart,
            crossEnd,
            0.01,
            isGwAntimerCross
    ...
    private addCanvasCornerIntersection(): [
        Array<GeoCoordinates | undefined>,
        number
    ] {
