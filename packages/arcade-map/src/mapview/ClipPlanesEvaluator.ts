...
namespace SphericalProj {
    ...
    export function getProjNearPlaneTanDistance(
        ...
        const camToTanVec = tmpVectors[1]
            .copy(fwdDir)
            .multiplyScalar(-R)
            .sub(camera.position)
    ...
    export function getProjSphereIntersectionDistance(
        ...
        return intersection !== null
            ? intersection
                  .sub(camera.position)
                  .dot(camera.getWorldDirection(tmpVectors[1]))
            : undefined
    }
     ...
     */
    export function getNormalToFwdAngle(
        camera: THREE.PerspectiveCamera
    ): number {
        const camToOriginDir = tmpVectors[0]
            .copy(camera.position)
            .negate()
            .normalize()
        const cosAngle = camToOriginDir.dot(
            camera.getWorldDirection(tmpVectors[1])
        )
 ...
 */
export abstract class ElevationBasedClipPlanesEvaluator
...
export class TopViewClipPlanesEvaluator extends ElevationBasedClipPlanesEvaluator {
    ...
    protected evaluateDistancePlanarProj(
        ...
        // Apply margins
        const nearFarMargin =
            (this.nearFarMarginRatio * (nearPlane + farPlane)) / 2
        nearPlane = Math.max(nearPlane - nearFarMargin / 2, this.nearMin)
        farPlane = Math.max(
            farPlane + nearFarMargin / 2,
            nearPlane + nearFarMargin
        )
    ...
    protected evaluateDistanceSphericalProj(
        ...
        const farTangent = SphericalProj.getFarDistanceFromElevatedHorizon(
            camera,
            d,
            r,
            maxR
    ...
    protected getFovBasedFarPlane(
        ...
        this.m_tmpQuaternion.setFromAxisAngle(
            this.m_tmpVectors[0],
            fovAngle / 2
        )
        ...
        const tVec = this.m_tmpVectors[1]
            .copy(cameraFwdVec)
        ...
        assert(
            a2 <= r2,
            'Please use this evaluator only for top view camera poses.'
        )
...
export class TiltViewClipPlanesEvaluator extends TopViewClipPlanesEvaluator {
    ...
    protected evaluateDistancePlanarProj(
        ...
        const topDist = Math.max(
            0,
            (z - this.minElevation) / Math.cos(topAngle)
        )
        ...
        const bottomDist = Math.max(
            0,
            (z - this.maxElevation) / Math.cos(bottomAngle)
        )
    ...
    private computeNearDistSphericalProj(
        ...
        const defaultNear =
            projection.groundDistance(camera.position) - this.maxElevation
    ...
    private computeFarDistSphericalProj(
        ...
        return SphericalProj.getFarDistanceFromElevatedHorizon(
            camera,
            d,
            r,
            maxR
    ...
    private applyViewRangeConstraints(
        ...
        // Apply the constraints.
        const farMin =
            projection.groundDistance(camera.position) - this.minElevation
        ...
        // Apply margins.
        const nearFarMargin =
            (this.nearFarMarginRatio * (viewRanges.near + viewRanges.far)) / 2
        viewRanges.near = Math.max(
            viewRanges.near - nearFarMargin / 2,
            this.nearMin
        )
 ...
 * @internal
 */
export const createDefaultClipPlanesEvaluator = () =>
