...
import { getOptionValue, MathUtils } from '@arca/utils'
            ...
            for (const sourceInfo of source) {
                const existingInfo = result.find(
                    (findItem) =>
                        ...
                        (findItem.label !== undefined &&
                            findItem.label === sourceInfo.label)
                )

                if (existingInfo === undefined) {
                    result.push({ ...sourceInfo })
                } else {
                    existingInfo.year = MathUtils.max2(
                        sourceInfo.year,
                        existingInfo.year
                    )
                    existingInfo.label = getOptionValue(
                        sourceInfo.label,
                        existingInfo.label
                    )
                    existingInfo.link = getOptionValue(
                        sourceInfo.link,
                        existingInfo.link
                    )
                }
