...
import { GeoBox } from '@arca/geoutils'
import { getOptionValue, ILogger, LoggerManager } from '@arca/utils'
...
export abstract class CopyrightCoverageProvider implements CopyrightProvider {
    /** Logger instance. */
    protected readonly logger: ILogger = LoggerManager.instance.create(
     ...
     */
    abstract getCopyrightCoverageData(
    ...
    async getCopyrights(
        ...
        for (const entry of matchingEntries) {
            ...
            if (level >= minLevel && level <= maxLevel) {
                if (
                    result.find((item) => item.id === entry.label) === undefined
                ) {
