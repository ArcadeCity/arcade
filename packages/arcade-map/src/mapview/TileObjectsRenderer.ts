...
export class TileObjectRenderer {
            ...
            for (const object of tile.objects) {
                ...
                if (tile.localTangentSpace) {
                    object.setRotationFromMatrix(
                        tile.boundingBox.getRotationMatrix()
                    )
                }
    ...
    setupRenderer() {
        ...
        const painterSortStable = (a: RenderItem, b: RenderItem): number => {
            ...
            const dataSourceOrder =
                mapObjectAdapterA?.dataSource?.dataSourceOrder
            const otherDataSourceOrder =
                mapObjectAdapterB?.dataSource?.dataSourceOrder
            if (
            ...
            if (
                mapObjectAdapterA?.level !== undefined &&
                mapObjectAdapterB?.level !== undefined
            ) {
                ...
                const eitherIsBuilding =
                    mapObjectAdapterA.kind?.find((s) => s === 'building') !==
                        undefined ||
                    mapObjectAdapterB.kind?.find((s) => s === 'building') !==
                        undefined
                const sameLevel =
                    mapObjectAdapterA.level === mapObjectAdapterB.level
    ...
    private updateStencilRef(object: TileObject) {
        ...
        if (
            object.renderOrder !== undefined &&
            object instanceof SolidLineMesh
        ) {
            const material = object.material
            if (Array.isArray(material)) {
                material.forEach(
                    (mat) =>
                        (mat.stencilRef = this.getStencilValue(
                            object.renderOrder
                        ))
                )
            } else {
    ...
    private processTileObject(
        ...
        if (
            !this.processTileObjectFeatures(
                tile,
                storageLevel,
                zoomLevel,
                object
    ...
    private processTileObjectFeatures(
        ...
        const technique: IndexedTechnique | undefined =
            object.userData.technique

        const minZoomLevel = getPropertyValue(
            technique?.minZoomLevel,
            this.m_env
        )
        const maxZoomLevel = getPropertyValue(
        ...
        const geometry: THREE.BufferGeometry | undefined = (object as any)
            .geometry
        ...
        const finalIndex =
            geometry.getIndex()?.count ?? geometry.attributes.position.count
        ...
        objInfos.forEach((properties, featureIndex) => {
            ...
            if (Expr.isExpr(technique.enabled)) {
                ...
                // create a new {@link @arca/datasource-protocol#Env} that can be used
