...
export class MapTileCuller {
    ...
    setup() {
        ...
        this.m_globalFrustumMin.set(
            Number.MAX_VALUE,
            Number.MAX_VALUE,
            Number.MAX_VALUE
        )
        this.m_globalFrustumMax.set(
            -Number.MAX_VALUE,
            -Number.MAX_VALUE,
            -Number.MAX_VALUE
        )
        ...
        for (const frustumCorner of frustumCorners) {
            ...
            this.m_globalFrustumMin.x = Math.min(
                this.m_globalFrustumMin.x,
                frustumCorner.x
            )
            this.m_globalFrustumMin.y = Math.min(
                this.m_globalFrustumMin.y,
                frustumCorner.y
            )
            this.m_globalFrustumMin.z = Math.min(
                this.m_globalFrustumMin.z,
                frustumCorner.z
            )

            this.m_globalFrustumMax.x = Math.max(
                this.m_globalFrustumMax.x,
                frustumCorner.x
            )
            this.m_globalFrustumMax.y = Math.max(
                this.m_globalFrustumMax.y,
                frustumCorner.y
            )
            this.m_globalFrustumMax.z = Math.max(
                this.m_globalFrustumMax.z,
                frustumCorner.z
            )
        }
    }
    ...
    private getFrustumCorners(): THREE.Vector3[] {
        ...
        const invProjMatrix = (this.m_camera as any)
            .projectionMatrixInverse as THREE.Matrix4

        let cornerIndex = 0

        function addPoint(x: number, y: number, z: number) {
            frustumCorners[cornerIndex++]
                .set(x, y, z)
                .applyMatrix4(invProjMatrix)
        }
