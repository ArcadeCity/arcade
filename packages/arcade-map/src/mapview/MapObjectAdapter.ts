 ...
 */
import { GeometryKind, Pickability, Technique } from '@arca/datasource-protocol'
...
export class MapObjectAdapter {
    ...
    isPickable() {
        ...
        return (
            (this.pickability === Pickability.onlyVisible &&
                this.isVisible()) ||
    ...
    private updateMaterials(context: MapAdapterUpdateEnv) {
        ...
        if (somethingChanged) {
            this.m_notCompletlyTransparent = materials.some(
                (material) => material.opacity > 0
            )
        }
