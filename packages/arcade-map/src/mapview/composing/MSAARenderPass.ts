...
import { CopyShader, MSAAMaterial } from '@arca/materials'
...
export class MSAARenderPass extends Pass {
    ...
    private readonly m_localCamera: THREE.OrthographicCamera =
    ...
    private readonly m_quadMaterial: THREE.ShaderMaterial = new MSAAMaterial(
        this.m_quadUniforms
    )
    ...
    render(
        ...
        const oldClearColor =
            rendererClearColor !== undefined ? rendererClearColor.getHex() : 0
        ...
        for (let i = 0; i < offsets.length; i++) {
            ...
            const uniformCenteredDistribution =
                -0.5 + (i + 0.5) / offsets.length
            const sampleWeight =
                1.0 / offsets.length + uniformCenteredDistribution / 32
