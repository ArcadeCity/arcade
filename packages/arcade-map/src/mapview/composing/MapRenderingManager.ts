...
import {
    IBloomEffect, IOutlineEffect, ISepiaEffect, IVignetteEffect
} from '@arca/datasource-protocol'
import { SepiaShader, VignetteShader } from '@arca/materials'
...
export interface IMapRenderingManager extends IPassManager {
     ...
     *
     * @param options - outline options from the {@link @arca/datasource-protocol#Theme}.
     */
...
export class MapRenderingManager implements IMapRenderingManager {
    ...
    private readonly m_target1: THREE.WebGLRenderTarget =
        new THREE.WebGLRenderTarget(1, 1)
    private readonly m_target2: THREE.WebGLRenderTarget =
        new THREE.WebGLRenderTarget(1, 1)
    ...
    constructor(
        ...
        this.m_msaaPass.enabled =
            antialiasSettings !== undefined
                ? antialiasSettings.msaaEnabled === true
                : false
    ...
    render(
        ...
        if (this.m_msaaPass.enabled) {
            ...
            if (
                ...
                !this.outline.enabled ||
                (this.outline.enabled && !this.bloom.enabled)
            ) {
        ...
        if (this.outline.enabled) {
            ...
            if (this.outline.needsUpdate) {
                ...
                this.m_outlineEffect.ghostExtrudedPolygons =
                    this.outline.ghostExtrudedPolygons
        ...
        if (this.bloom.enabled) {
            ...
            const nextEffectEnabled =
                this.vignette.enabled || this.sepia.enabled
