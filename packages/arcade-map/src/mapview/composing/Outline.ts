 ...
 */
import {
    EdgeMaterial, ExtrusionFeatureDefs, FadingFeature, MixinShaderProperties,
    UniformsType
} from '@arca/materials'
import { chainCallbacks } from '@arca/utils'
...
export class OutlineEffect {
    ...
    private createMaterial(originalMaterial: THREE.Material) {
        ...
        if (
            ...
            (originalMaterial as DepthPrePassProperties)
        ...
        const isExtrusionMaterial =
            (originalMaterial as MixinShaderProperties).shaderUniforms !==
                undefined &&
            (originalMaterial as any).shaderUniforms.extrusionRatio !==
                undefined

        const isFadingMaterial = FadingFeature.isDefined(
            originalMaterial as FadingFeature
        )
    ...
    private getOutlineMaterial(originalMaterial: THREE.Material) {
        const outlineMaterial =
            this.getOutlineMaterialFromCache(originalMaterial)
    ...
    private setOutlineMaterial(object: THREE.Object3D) {
        ...
        if (Array.isArray((object as THREE.Mesh).material)) {
            for (
                let i = 0,
                    il = ((object as THREE.Mesh).material as THREE.Material[])
                        .length;
                i < il;
                i++
            ) {
                ;((object as THREE.Mesh).material as THREE.Material[])[i] =
                    this.getOutlineMaterial(
                        ((object as THREE.Mesh).material as THREE.Material[])[i]
                    )
            }
        } else {
    ...
    private updateUniforms(
        ...
        if (outlineParameters !== undefined) {
            if (outlineParameters.thickness !== undefined) {
                outlineUniforms.outlineThickness.value =
                    outlineParameters.thickness
            }
            if (outlineParameters.color !== undefined) {
                outlineUniforms.outlineColor.value.fromArray(
        ...
        if (
            originalUniforms !== undefined &&
            originalUniforms.extrusionRatio !== undefined
        ) {
            const value = (originalMaterial as any).shaderUniforms
    ...
    private updateOutlineMaterial(
        ...
        if (outlineParameters !== undefined) {
            ...
            if (outlineParameters.keepAlive !== undefined) {
                this.m_cache[originalMaterial.uuid].keepAlive =
        ...
        if (
            (originalMaterial as any).wireframe === true ||
