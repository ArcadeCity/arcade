 ...
 */
import {
    OrientedBox3, Projection, ProjectionType, TileKey, TileKeyUtils,
    TilingScheme
} from '@arca/geoutils'
...
import {
    CalculationStatus, ElevationRange, ElevationRangeSource
} from './ElevationRangeSource'
...
export class FrustumIntersection {
    ...
    compute(
        ...
        const targetTileArea = Math.pow(
            this.m_tilePixelSize / this.mapView.viewportHeight,
            2
        )
        ...
        dataSources.forEach((dataSource) => {
            minGeometryHeight = Math.min(
                minGeometryHeight,
                dataSource.minGeometryHeight
            )
            maxGeometryHeight = Math.max(
                maxGeometryHeight,
                dataSource.maxGeometryHeight
            )
        })

        const cache = {
            calculationFinal: true,
            tileBounds: obbIntersections
                ? new OrientedBox3()
                : new THREE.Box3(),
        }
        ...
        while (workList.length > 0) {
            ...
            const tileKeyAndOffset = TileKeyUtils.getKeyForTileKeyAndOffset(
                tileKey,
                offset
            )
    ...
    private getTileKeyEntry(
        ...
        geoBox.southWest.altitude =
            (geoBox.southWest.altitude ?? 0) + minGeometryHeight
        geoBox.northEast.altitude =
            (geoBox.northEast.altitude ?? 0) + maxGeometryHeight

        this.mapView.projection.projectBox(geoBox, cache.tileBounds)
        const { area, distance } = this.computeTileAreaAndDistance(
            cache.tileBounds
        )
        ...
        if (tileBounds instanceof THREE.Box3) {
            if (
                (this.m_extendedFrustumCulling &&
                    !this.m_mapTileCuller.frustumIntersectsTileBox(
                        tileBounds
                    )) ||
    ...
    private computeRequiredInitialRootTileKeys(worldCenter: THREE.Vector3) {
        ...
        const tileWrappingEnabled =
            this.mapView.projection.type === ProjectionType.Planar
        ...
        const worldGeoPoint =
            this.mapView.projection.unprojectPoint(worldCenter)
        ...
        const cameraPitch = MapViewUtils.extractAttitude(
            this.mapView,
            camera
        ).pitch
        ...
        const totalAngleRad =
            THREE.MathUtils.degToRad((camera.fov * aspect) / 2) + cameraPitch
        // Length a->d2
        const worldLengthHorizontalFull =
            Math.tan(totalAngleRad) * camera.position.z
        // Length a->e
        const worldLengthHorizontalSmallerHalf =
            Math.tan(cameraPitch) * camera.position.z
        // Length e -> d2
        const worldLengthHorizontal =
            worldLengthHorizontalFull - worldLengthHorizontalSmallerHalf
        const worldLeftPoint = new THREE.Vector3(
            worldCenter.x - worldLengthHorizontal,
            worldCenter.y,
            worldCenter.z
        )
        const worldLeftGeoPoint =
            this.mapView.projection.unprojectPoint(worldLeftPoint)
        ...
        const offsetRange = THREE.MathUtils.clamp(
            Math.ceil(
                Math.abs(
                    (worldGeoPoint.longitude - worldLeftGeoPoint.longitude) /
                        360
                ) * Math.SQRT2
            ),
            0,
        ...
        for (
            ...
            this.m_rootTileKeys.push(
                new TileKeyEntry(rootTileKey, Infinity, offset)
            )
        }
    }
}
