 ...
 */
import {
    EarthConstants, GeoBox, GeoCoordinates, GeoCoordinatesLike, GeoCoordLike,
    MathUtils, OrientedBox3, Projection, ProjectionType, sphereProjection,
    TileKeyUtils, Vector2Like, Vector3Like
} from '../geoutils'
 ...
 * {@link @arca/map-controls#MapControls} never matches
 * exactly the target due to the precision loss caused by the
 * conversion from zoom level to camera distance (done in
 * {@link (MapView.lookAt:WITH_PARAMS)} and {@link @arca/map-controls#MapControls})
...
export namespace MapViewUtils {
    ...
    export function zoomOnTargetPosition(
        ...
        const zoomTarget = rayCastWorldCoordinates(
            mapView,
            targetNDCx,
            targetNDCy,
            elevation
        ...
        const newCameraDistance = calculateDistanceFromZoomLevel(
            mapView,
            zoomLevel
        )
        ...
        const newZoomTarget = rayCastWorldCoordinates(
            mapView,
            targetNDCx,
            targetNDCy,
            elevation
    ...
    export function orbitAroundScreenPoint(
        ...
        const ppalPoint = CameraUtils.getPrincipalPoint(
            mapView.camera,
            cache.vector2[0]
        )
        ...
        const orbitAroundPpalPoint =
            orbitCenter.x === ppalPoint.x && orbitCenter.y === ppalPoint.y
        const rotationTargetWorld = orbitAroundPpalPoint
            ? mapTargetWorld
            : MapViewUtils.rayCastWorldCoordinates(
                  mapView,
                  orbitCenter.x,
                  orbitCenter.y
              )
        ...
        const tiltAxis = new THREE.Vector3(1, 0, 0).applyQuaternion(
    ...
    function applyAzimuthAroundTarget(
        ...
        const headingAxis = projection.surfaceNormal(
            rotationTargetWorld,
            cache.vector3[0]
        )
        const headingQuat = cache.quaternions[0].setFromAxisAngle(
            headingAxis,
            deltaAzimuth
        )
    ...
    function computeClampedDeltaTilt(
        ...
        const tilt = extractTiltAngleFromLocation(
            projection,
            camera,
            mapTargetWorld,
            tiltAxis
        ...
        if (projection === sphereProjection) {
            const projectedRotationTargetNormal = projection
                ...
                .normalize()
            const mapTargetNormal = projection.surfaceNormal(
                mapTargetWorld,
                cache.vector3[1]
            )
            angleBetweenNormals =
                projectedRotationTargetNormal.angleTo(mapTargetNormal)
        }
    ...
    function applyTiltAroundTarget(
        ...
        const tiltQuat = cache.quaternions[0].setFromAxisAngle(
            tiltAxis,
            deltaTilt
        )
    ...
    export function getGeoTargetFromCamera(
        ...
        const targetWorldPos = getWorldTargetFromCamera(
            camera,
            projection,
            elevation
        )
    ...
    export function constrainTargetAndDistanceToViewBounds(
        ...
        const scale = Math.max(
            viewWidth / boundsSize.x,
            viewHeight / boundsSize.y
        )
        ...
        if (projection.type === ProjectionType.Planar) {
            if (scale > 1) {
                ...
                camera.position.x +=
                    constrained.target.x - unconstrained.target.x
                camera.position.y +=
                    constrained.target.y - unconstrained.target.y
            }
            return constrained
        }
        ...
        if (scale > 1) {
            ...
            const rotMatrix = targetMaxBounds.getRotationMatrix(
                cache.matrix4[0]
            )
            ...
            const constTargetDistToGround =
                Math.sqrt(targetHeightSq) -
                Math.sqrt(targetHeightSq - constTargetDistSq)
            constrained.target.addScaledVector(
                targetMaxBounds.zAxis,
                -constTargetDistToGround
            )
    ...
    export function getTargetAndDistance(
        ...
        if (target !== null) {
            ...
            const distance =
                projection.type === ProjectionType.Planar
                    ? heightAboveTerrain /
                      Math.cos(Math.min(cameraPitch, MAX_TILT_RAD))
                    : Math.sqrt(
                          Math.pow(
                              heightAboveTerrain +
                                  EarthConstants.EQUATORIAL_RADIUS,
    ...
    /**
     * Returns the {@link @arca/geoutils#GeoCoordinates} of the camera,
     ...
     */
    export function wrapWorldPointsToView(
     ...
     * @internal
     *
     * Return `GeoPoints` bounding {@link @arca/geoutils#GeoBox}
     * applicable for {@link getFitBoundsDistance}.
     *
     * @returns {@link @arca/geoutils#GeoCoordinates} set that covers `box`
     */
    ...
    export function getFitBoundsDistance(
        ...
        const targetDist = cache.vector3[0]
            .copy(worldTarget)
            .sub(camera.position)
            .length()
        ...
        const getDistanceFactor = (pointNDC: number, ppNDC: number) => {
            ...
            return Math.abs(pointNDC) > 1
                ? Math.abs(
                      (pointNDC - ppNDC) /
                          (maxNDC * Math.sign(pointNDC) - ppNDC)
                  )
                : 1
        }
        for (const point of points) {
            const pEyeZ = -cache.vector3[0]
                .copy(point)
                .applyMatrix4(camera.matrixWorldInverse).z
            const pointNDC = cache.vector3[0].applyMatrix4(
    ...
    export function getFitBoundsLookAtParams(
        ...
        const distance = getFitBoundsDistance(
            worldPoints,
            worldTarget,
            tmpCamera
        )
    ...
    export function rayCastWorldCoordinates(
        ...
        const pointInCameraSpace = pointInNDCPosition.applyMatrix4(
            cache.matrix4[1]
        )
    ...
    export function rotate(
        ...
        const pitch = MapViewUtils.extractAttitude(
            mapView,
            mapView.camera
        ).pitch
        ...
        if (mapView.projection.type === ProjectionType.Spherical) {
            ...
            const maxPitch = Math.asin(
                (EarthConstants.EQUATORIAL_RADIUS *
                    Math.sin(Math.PI - maxTiltAngleRad)) /
     ...
     * @param projection - The {@link @arca/geoutils#Projection} used to
     *                     convert between geo and world coordinates.
     *
     * @deprecated Use MapView.tilt
     */
    export function extractCameraTilt(
        camera: THREE.Camera,
        projection: Projection
    ): number {
        ...
        if (projection.type === ProjectionType.Planar) {
            const lookAt: THREE.Vector3 = camera
            ...
            const targetGeoCoords = MapViewUtils.getGeoTargetFromCamera(
                camera,
                projection
            )
     ...
     *
     * @param projection - The {@link @arca/geoutils#Projection} used when
    ...
    export function getCameraFrustumPlanes(camera: THREE.PerspectiveCamera): {
        ...
        let top =
            (near * Math.tan(THREE.MathUtils.degToRad(0.5 * camera.fov))) /
            camera.zoom
        let height = 2 * top
        ...
        left +=
            camera.filmOffset !== 0
                ? (near * camera.filmOffset) / camera.getFilmWidth()
                : 0
    ...
    export function calculateDistanceToGroundFromZoomLevel(
        ...
        const tileSize =
            EarthConstants.EQUATORIAL_CIRCUMFERENCE / Math.pow(2, zoomLevel)
    ...
    export function calculateDepthFromClipDistance(
        ...
        return (
            (1.0 - perspCam.near / viewSpaceDistance) *
            (perspCam.far / cameraRange)
        )
    }
     ...
     * @param camera - Camera applying the perspective projection.
     */
    export function cameraToWorldDistance(
        distance: number,
        camera: THREE.Camera
    ): number {
        const perspCam = camera as THREE.PerspectiveCamera
     ...
     * @deprecated
     */
    export function calculateVerticalFovByHorizontalFov(
        hFov: number,
        aspect: number
    ): number {
     ...
     */
    export function calculateHorizontalFovByVerticalFov(
        vFov: number,
        aspect: number
    ): number {
     ...
     */
    export function calculateFocalLengthByVerticalFov(
        vFov: number,
        height: number
    ): number {
     ...
     */
    export function calculateFovByFocalLength(
        focalLength: number,
        height: number
    ): number {
     ...
     */
    export const calculateScreenSizeByFocalLength =
        CameraUtils.convertWorldToScreenSize

    /**
     * @deprecated Use {@link CameraUtils.convertScreenToWorldSize}.
     */
    export const calculateWorldSizeByFocalLength =
        CameraUtils.convertScreenToWorldSize
    ...
    /**
     * @deprecated Use {@link @arca/utils#DOMUtils.getBrowserLanguages}
     */
...
export namespace TileOffsetUtils {
    /**
     * @deprecated Use {@link @arca/geoutils#TileKeyUtils.getKeyForTileKeyAndOffset}.
     */
    export const getKeyForTileKeyAndOffset =
        TileKeyUtils.getKeyForTileKeyAndOffset

    /**
     * @deprecated Use {@link @arca/geoutils#TileKeyUtils.getKeyForTileKeyAndOffset}.
     */
    export const extractOffsetAndMortonKeyFromKey =
        TileKeyUtils.extractOffsetAndMortonKeyFromKey

    /**
     * @deprecated Use {@link @arca/geoutils#TileKeyUtils.getParentKeyFromKey}.
     */
