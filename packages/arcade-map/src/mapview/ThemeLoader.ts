...
import {
    Definitions, getDefinitionValue, getStyles, ImageTexture, isJsonExpr,
    isJsonExprReference, isStylesDictionary, Style, Styles, Theme
} from '@arca/datasource-protocol'
import {
    composeUriResolvers, ContextLogger, getAppBaseUrl, getOptionValue,
    IContextLogger, ISimpleChannel, RelativeUriResolver, UriResolver
} from '@arca/utils'
...
/**
 * Options to customize {@link @arca/datasource-protocol#Theme} loading process.
 *
...
export interface ThemeLoadOptions {
     ...
     * through [{@link @arca/datasource-protocol#Theme}s `extends` property.
     *
...
export class ThemeLoader {
    /**
     * Loads a {@link @arca/datasource-protocol#Theme} from a
     ...
     * providing {@link @arca/utils#UriResolver} using {@link ThemeLoadOptions.uriResolver}
     * option.
     *
     * @param theme - {@link @arca/datasource-protocol#Theme} instance or theme URL
     *                to the theme.
    ...
    static async load(
        ...
        if (typeof theme === 'string') {
            ...
            const themeUrl =
                uriResolver !== undefined
                    ? uriResolver.resolveUri(theme)
                    : theme
            const response = await fetch(themeUrl, { signal: options.signal })
            if (!response.ok) {
                throw new Error(
                    `ThemeLoader#load: cannot load theme: ${response.statusText}`
                )
            }
        ...
        if (theme === null || theme === undefined) {
            throw new Error(
                'ThemeLoader#load: loaded resource is not valid JSON'
            )
        }

        const resolveDefinitions = getOptionValue<boolean>(
     ...
     *
     * Loads a {@link @arca/datasource-protocol#Theme} from a remote resource,
    ...
    /**
     * Resolves all {@link @arca/datasource-protocol#Theme}'s relatives URLs
     * to full URL using the {@link @arca/datasource-protocol#Theme}'s URL
     * (see: https://www.w3.org/TR/WD-html40-970917/htmlweb.html#h-5.1.2).
     *
     * This method mutates original `theme` instance.
     *
     * @param theme - The {@link @arca/datasource-protocol#Theme} to resolve.
     */
    private static resolveUrls(
        ...
        const resolveIncludes =
            options === undefined || !(options.resolveIncludeUris === false)
        if (theme.extends && resolveIncludes) {
            theme.extends = (
                Array.isArray(theme.extends) ? theme.extends : [theme.extends]
            ).map((baseTheme) => {
        ...
        const resolveResources =
            options === undefined || !(options.resolveResourceUris === false)
        if (resolveResources) {
    ...
    /**
     * Expand all `ref` expressions in {@link @arca/datasource-protocol#Theme}
    ...
    private static resolveExpressionReferences<T>(
        ...
        function resolveInternal(node: any) {
            if (isJsonExprReference(node)) {
                ...
                if (def === undefined) {
                    contextLogger.warn(
                        `invalid reference '${defName}' - not found`
                    )
                    failed = true
                    return undefined
                }
     ...
     * its base {@link @arca/datasource-protocol#Theme}.
     *
     * @param theme - {@link @arca/datasource-protocol#Theme} object
     * @param options - Optional, a {@link ThemeLoadOptions} objects
     *                  containing any custom settings for
     *                  this load request.
     */
    private static async resolveBaseThemes(
        ...
        if (maxInheritanceDepth <= 0) {
            throw new Error(
                `maxInheritanceDepth reached when attempting to load base theme`
            )
        }

        const baseThemes = !Array.isArray(theme.extends)
            ? [theme.extends]
            : theme.extends
        ...
        for (const baseTheme of baseThemes) {
            ...
            baseThemesMerged = ThemeLoader.mergeThemes(
                actualBaseTheme,
                baseThemesMerged
            )
        }
    ...
    private static resolveResources(
        theme: Theme,
        childUrlResolver: UriResolver
    ) {
        if (theme.sky && theme.sky.type === 'cubemap') {
            for (let i = 0; i < SKY_CUBEMAP_FACE_COUNT; ++i) {
                const faceUrl: string | undefined = (theme.sky as any)[
                    SkyCubemapFaceId[i]
                ]
                if (faceUrl !== undefined) {
                    ;(theme.sky as any)[SkyCubemapFaceId[i]] =
                        childUrlResolver.resolveUri(faceUrl)
                }
            }
        }
        if (theme.images) {
        ...
        if (theme.styles !== undefined) {
            for (const style of getStyles(theme.styles)) {
                ...
                ;[
                    ...
                    'displacementMap',
                    'roughnessMap',
                ].forEach((texturePropertyName) => {
                    const textureProperty = (style.attr! as any)[
                        texturePropertyName
                    ]
                    if (
                        textureProperty &&
                        typeof textureProperty === 'string'
                    ) {
