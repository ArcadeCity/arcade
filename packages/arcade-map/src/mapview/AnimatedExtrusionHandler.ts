...
import {
    getPropertyValue, isExtrudedPolygonTechnique, MapEnv, Technique
} from '@arca/datasource-protocol'
import { TileKey } from '@arca/geoutils'
import { ExtrusionFeature, ExtrusionFeatureDefs } from '@arca/materials'
import { MathUtils } from '@arca/utils'
...
export class AnimatedExtrusionHandler {
    ...
    setAnimationProperties(technique: Technique, env: MapEnv) {
        ...
        const animateExtrusionValue = getPropertyValue(
    ...
    update(zoomLevel: number) {
        const extrusionVisible =
            this.m_dataSourceMap.size > 0 && zoomLevel >= this.m_minZoomLevel
    ...
    add(tile: Tile, materials: ExtrusionFeature[]): void {
        ...
        this.getOrCreateTileMap(tile.dataSource).set(
    ...
    private skipAnimation(tile: Tile): boolean {
        return (
            this.wasAnyAncestorAnimated(tile) ||
            this.wasAnyDescendantAnimated(tile)
        )
    }
    ...
    private wasAnyDescendantAnimated(tile: Tile): boolean {
        const distanceToMaxLevel =
            tile.dataSource.maxDataLevel - tile.tileKey.level
        ...
        for (let deltaDown = 1; deltaDown <= levelsDown; ++deltaDown) {
            ...
            for (const tileKey of nextTileKeys) {
                for (const childTileKey of tilingScheme.getSubTileKeys(
                    ...
                    if (
                        tileMap.get(childTileKey.mortonCode())?.animated ??
                        false
                    ) {
                        return true
                    }
