...
import { Fog } from '@arca/datasource-protocol'
import { HighPrecisionLineMaterial, RawShaderMaterial } from '@arca/materials'
import { assert, MathUtils } from '@arca/utils'
...
export class MapViewFog {
    ...
    /**
     * Sets the fog depending on the {@link @arca/datasource-protocol#Theme}
    ...
    update(mapView: MapView, viewDistance?: number) {
        if (
            ...
            const viewRange =
                viewDistance !== undefined ? viewDistance : mapView.camera.far
            ...
            const density = MathUtils.smoothStep(
                horizontalDensity,
                verticalDensity,
                t
            )
            ...
            this.m_fog.far = THREE.MathUtils.lerp(
                viewRange * endRatio,
                viewRange,
                density
            )
