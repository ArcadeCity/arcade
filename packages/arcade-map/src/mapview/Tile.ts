 ...
 */
import {
    DecodedTile, GeometryKindSet, GeometryType, TextPathGeometry
} from '@arca/datasource-protocol'
import {
    assert, CachedResource, chainCallbacks, LoggerManager
} from '@arca/utils'

import {
    GeoBox, OrientedBox3, Projection, TileKey, TileKeyUtils
} from '../geoutils'
...
import {
    TextElementGroupPriorityList
} from './text/TextElementGroupPriorityList'
...
export function getFeatureDataSize(featureData: TileFeatureData): number {
    ...
    if (featureData.objInfos !== undefined) {
        ...
        numBytes +=
            featureData.objInfos.length * MINIMUM_SMALL_OBJECT_SIZE_ESTIMATION
    }
...
export class Tile implements CachedResource {
    ...
    constructor(
        ...
        this.m_uniqueKey = TileKeyUtils.getKeyForTileKeyAndOffset(
            this.tileKey,
            this.offset
        )
        if (dataSource.useGeometryLoader) {
            this.m_tileGeometryLoader = new TileGeometryLoader(
                this,
    ...
    get isVisible(): boolean {
        ...
        try {
            return (
                this.frameNumLastRequested >=
                this.dataSource.mapView.frameNumber - 1
            )
        } catch (error) {
            logger.debug(error)
            return false
    ...
    set isVisible(visible: boolean) {
        this.frameNumLastRequested = visible
            ? this.dataSource.mapView.frameNumber
            : -1

        if (
            !visible &&
            this.m_tileGeometryLoader &&
            !this.m_tileGeometryLoader.isSettled
        ) {
    ...
    /**
     * The {@link @arca/geoutils#Projection} currently used by the {@link MapView}.
     */
    ...
    get userTextElements(): TextElementGroup {
        let group = this.m_textElementGroups.groups.get(
            TextElement.HIGHEST_PRIORITY
        )
        if (group === undefined) {
    ...
    set elevationRange(elevationRange: ElevationRange) {
        if (
            elevationRange.minElevation ===
                this.m_elevationRange.minElevation &&
            elevationRange.maxElevation ===
                this.m_elevationRange.maxElevation &&
            elevationRange.calculationStatus ===
                this.m_elevationRange.calculationStatus
        ) {
            return
        }
        ...
        if (
            ...
            this.m_minGeometryHeight !== undefined
        ) {
    ...
    clear() {
        const disposeMaterial = (material: THREE.Material) => {
            Object.getOwnPropertyNames(material).forEach((property: string) => {
                ...
                if (
                    ...
                    materialProperty instanceof THREE.Texture
                ) {
        ...
        const disposeObject = (object: TileObject & DisposableObject) => {
            ...
            if (
                ...
                this.shouldDisposeObjectMaterial(object)
            ) {
    ...
    addDisposeCallback(callback: TileCallback) {
        this.m_disposeCallback = chainCallbacks(
            this.m_disposeCallback,
            callback
        )
    }
     ...
     * The {@link @arca/datasource-protocol#DecodedTile} is still available.
     */
    ...
    private computeResourceInfo(): void {
        ...
        if (
            this.m_decodedTile !== undefined &&
            this.m_decodedTile.tileInfo !== undefined
        ) {
