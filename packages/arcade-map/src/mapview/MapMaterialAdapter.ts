...
import {
    disableBlending, enableBlending, RawShaderMaterial
} from '@arca/materials'
...
export class MapMaterialAdapter {
    ...
    static create(
        ...
        return MapMaterialAdapter.install(
            new MapMaterialAdapter(material, styledProperties)
        )
    }

    static ensureUpdated(
        material: THREE.Material,
        context: MapAdapterUpdateEnv
    ): boolean {
    ...
    private readonly m_dynamicProperties: Array<
        [string, Expr | StylePropertyEvaluator]
    >
    ...
    constructor(material: THREE.Material, styledProperties: StyledProperties) {
        ...
        for (const propName in styledProperties) {
            ...
            if (
                Expr.isExpr(propDefinition) ||
                typeof propDefinition === 'function'
            ) {
    ...
    private setupStaticProperties() {
        ...
        if (updateBaseColor) {
            const color =
                (this.currentStyledProperties.color as number) ?? 0xff0000
            const opacity =
                (this.currentStyledProperties.opacity as number) ?? 1
