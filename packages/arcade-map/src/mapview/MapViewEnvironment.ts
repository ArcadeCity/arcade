...
import { GradientSky, Light, Sky } from '@arca/datasource-protocol'
import { ProjectionType, Vector3Like } from '@arca/geoutils'
...
export class MapViewEnvironment {
    ...
    constructor(
        ...
        options: MapViewEnvironmentOptions
    ) {
        ...
        if (
            ...
            this.m_backgroundDataSource.setTilingScheme(
                options.backgroundTilingScheme
            )
        }
    ...
    updateSkyBackground(sky?: Sky, clearColor?: string) {
        if (
            this.m_skyBackground instanceof SkyBackground &&
            sky !== undefined
        ) {
        ...
        this.m_createdLights.forEach((element) => {
            ...
            if (directionalLight.isDirectionalLight === true) {
                ...
                const cameraHeight =
                    ...
                    Math.cos(THREE.MathUtils.degToRad(tilt))
                ...
                directionalLight.position.addScaledVector(
                    lightDirection,
                    -lightPosHyp
                )
    ...
    private addNewSkyBackground(sky: Sky, clearColor: string | undefined) {
        if (
            sky.type === 'gradient' &&
            (sky as GradientSky).groundColor === undefined
        ) {
    ...
    private updateSkyBackgroundColors(
        ...
        if (this.m_skyBackground !== undefined) {
            this.m_skyBackground.updateTexture(
                sky,
                this.m_mapView.projection.type
            )
            this.m_mapView.scene.background = this.m_skyBackground?.texture
