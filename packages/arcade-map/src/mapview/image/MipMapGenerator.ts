...
export class MipMapGenerator {
    ...
    generateTextureAtlasMipMap(image: ImageItem): ImageData[] {
        ...
        const { width: paddedWidth, height: paddedHeight } =
            MipMapGenerator.getPaddedSize(imageData.width, imageData.height)
        this.copyImageWithPadding(imageData, paddedWidth, paddedHeight)
        mipMaps.push(
            this.m_paddingContext!.getImageData(0, 0, paddedWidth, paddedHeight)
        )
    ...
    private resizeImage(
        ...
        const paddedImage = this.copyImageWithPadding(
            image,
            image.width,
            image.height
        )
