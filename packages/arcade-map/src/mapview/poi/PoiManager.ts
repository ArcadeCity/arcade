...
import {
    AttributeMap, composeTechniqueTextureName, DecodedTile, ImageTexture,
    IndexedTechnique, isLineMarkerTechnique, isPoiTechnique, MapEnv, PoiGeometry
} from '@arca/datasource-protocol'
import { assert, assertExists, LoggerManager } from '@arca/utils'
...
/**
 * Interface for the {@link @arca/datasource-protocol#ImageTexture}s
 ...
 * {@link @arca/datasource-protocol#PoiGeometry} objects
 * from the {@link @arca/datasource-protocol#DecodedTile},
...
export class PoiManager {
    ...
    private static readonly m_missingPoiTableName: Map<string, boolean> =
        new Map()
     ...
     *                      {@link @arca/datasource-protocol#PoiGeometry}
    ...
    addPois(tile: Tile, decodedTile: DecodedTile): void {
        ...
        for (const poiGeometry of poiGeometries) {
            ...
            const technique = decodedTile.techniques[
                techniqueIndex
            ] as IndexedTechnique
     ...
     *
     * @remarks
     * Creates an {@link @arca/datasource-protocol#ImageTexture}
    ...
    async addTextureAtlas(
        ...
        try {
            ...
            for (const textureName of Object.getOwnPropertyNames(jsonAtlas)) {
                const imageTextureDef = jsonAtlas[
                    textureName
                ] as ImageTextureDef
            ...
            logger.error(
                `addTextureAtlas: Failed to load textureAtlas '${atlas}' : ${error}`
            )
        }
    }

    /**
     * Add an {@link @arca/datasource-protocol#ImageTexture} such that it
     * is available as a named entity for techniques in theme files.
     *
     * @param imageTexture - {@link @arca/datasource-protocol#ImageTexture}
    ...
    addImageTexture(imageTexture: ImageTexture) {
        ...
        if (this.m_imageTextures.get(imageTexture.name) !== undefined) {
            logger.warn(
                `addImageTexture: Name already used: ${imageTexture.name}` +
                    ` (overriding it)`
            )
        }
    ...
    /**
     * Return the {@link @arca/datasource-protocol#ImageTexture}
     * registered under the specified name.
     *
     * @param name - Name of the {@link @arca/datasource-protocol#ImageTexture}.
     */
    ...
    updatePoiFromPoiTable(pointLabel: TextElement): boolean {
        ...
        if (
            poiTableEntry.iconName !== undefined &&
            poiTableEntry.iconName.length > 0
        ) {
        ...
        poiInfo.iconMinZoomLevel =
            poiTableEntry.iconMinLevel ?? poiInfo.iconMinZoomLevel
        poiInfo.iconMaxZoomLevel =
            poiTableEntry.iconMaxLevel ?? poiInfo.iconMaxZoomLevel
        poiInfo.textMinZoomLevel =
            poiTableEntry.textMinLevel ?? poiInfo.textMinZoomLevel
        poiInfo.textMaxZoomLevel =
            poiTableEntry.textMaxLevel ?? poiInfo.textMaxZoomLevel
