 ...
 */
import { MemoryUsage } from '@arca/text-canvas'
...
export class BoxBuffer {
    /**
     * {@link @arca/datasource-protocol#BufferAttribute} holding the `BoxBuffer` position data.
     */
    private m_positionAttribute?: THREE.BufferAttribute

    /**
     * {@link @arca/datasource-protocol#BufferAttribute} holding the `BoxBuffer` color data.
     */
    private m_colorAttribute?: THREE.BufferAttribute

    /**
     * {@link @arca/datasource-protocol#BufferAttribute} holding the `BoxBuffer` uv data.
     */
    private m_uvAttribute?: THREE.BufferAttribute

    /**
     * {@link @arca/datasource-protocol#BufferAttribute} holding the `BoxBuffer` index data.
     */
     ...
     * {@link @arca/datasource-protocol#BufferAttribute}s.
     */
    ...
    canAddElements(glyphCount = 1): boolean {
        ...
        if (
            ...
            if (
                indexAttribute.array.length >=
                this.m_maxElementCount * NUM_INDICES_PER_ELEMENT
            ) {
                return false
            }

            const newSize = Math.min(
                this.m_maxElementCount,
                this.size === 0 ? 256 : this.size * 2
            )
            this.resize(newSize)
        }
        return true
    ...
    cleanUp() {
        ...
        if (
            this.m_indexAttribute!.count === 0 &&
            this.size > START_BOX_BUFFER_SIZE
        ) {
    ...
    /**
     * Creates a new {@link @arca/datasource-protocol#Geometry} object
    ...
    resize(newSize?: number, forceResize?: boolean): BoxBufferMesh {
        ...
        if (
            newSize !== undefined &&
            (forceResize === true || newSize > this.size)
        ) {
    ...
    private isPixelTransparent(
        ...
        const { u, v } = screenToUvCoordinates(
            xScreenPos,
            yScreenPos,
            box,
            uvBox
        ...
        const { width, height } =
            image instanceof SVGImageElement ? image.getBBox() : image
        const x = width * u
        const y = height * v
    ...
    private resizeBuffer(newSize: number) {
        ...
        if (
            this.m_positionAttribute !== undefined &&
            this.m_positionAttribute.array.length > 0
        ) {
        ...
        if (this.m_uvAttribute !== undefined) {
            ...
            this.m_uvAttribute = new THREE.BufferAttribute(
                newUvArray,
                NUM_UV_VALUES_PER_VERTEX
            )
            this.m_uvAttribute.count = 0
        ...
        const numIndexValues =
            newSize * NUM_INDICES_PER_ELEMENT * NUM_INDEX_VALUES_PER_VERTEX
