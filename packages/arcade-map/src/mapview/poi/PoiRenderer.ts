 ...
 */
import { Env, getPropertyValue, ImageTexture } from '@arca/datasource-protocol'
import { IconMaterial } from '@arca/materials'
import { MemoryUsage } from '@arca/text-canvas'
import { assert, LoggerManager, Math2D } from '@arca/utils'
 ...
 * ({@link @arca/datasource-protocol#ImageTexture}).
 *
...
class PoiBatch {
    ...
    updateMemoryUsage(info: MemoryUsage) {
        if (this.imageItem.image !== undefined) {
            const imageBytes =
                this.imageItem.image.width * this.imageItem.image.height * 4
...
function findImageItem(
    ...
    assert(poiInfo.imageTextureName !== undefined)
    const imageTextureName = imageTexture
        ? imageTexture.image
    ...
    missingTextureName.set(
        imageTextureName,
        missingTextureCount ? missingTextureCount + 1 : 0
    )
...
export class PoiRenderer {
    ...
    constructor(
        ...
        this.m_poiBatchRegistry = new PoiBatchRegistry(
    ...
    private preparePoi(pointLabel: TextElement, env: Env): void {
        ...
        imageItem
            ...
            .catch((error) => {
                logger.error(
                    `preparePoi: Failed to load imageItem: '${imageItem.url}`,
                    error
                )
                poiInfo.isValid = false
            })
     ...
     * @param imageTexture - Shared {@link @arca/datasource-protocol#ImageTexture},
    ...
    private setupPoiInfo(
        ...
        const paddedSize = MipMapGenerator.getPaddedSize(
            imageWidth,
            imageHeight
        )
        const trilinearFiltering = PoiBatch.trilinear && imageItem.mipMaps
        const paddedImageWidth = trilinearFiltering
            ? paddedSize.width
            : imageWidth
        const paddedImageHeight = trilinearFiltering
            ? paddedSize.height
            : imageHeight

        const iconWidth =
            imageTexture?.width !== undefined ? imageTexture.width : imageWidth
        const iconHeight =
            imageTexture?.height !== undefined
                ? imageTexture.height
                : imageHeight
        ...
        const height =
            imageTexture?.height !== undefined
                ...
                : imageHeight
        const xOffset =
            imageTexture?.xOffset !== undefined ? imageTexture.xOffset : 0
        const yOffset =
            imageTexture?.yOffset !== undefined ? imageTexture.yOffset : 0
        ...
        let iconScaleH =
            technique.iconScale !== undefined ? technique.iconScale : 1
        let iconScaleV =
            technique.iconScale !== undefined ? technique.iconScale : 1
