 ...
 */
import {
    PoiStackMode, PoiTableDef, PoiTableEntryDef, PoiTableRef
...
class PoiTableEntry implements PoiTableEntryDef {
    ...
    static verifyJSON(jsonEntry: PoiTableEntryDef): boolean {
        let isOK =
            ...
            (jsonEntry.altNames === undefined ||
                Array.isArray(jsonEntry.altNames)) &&
            (jsonEntry.stackMode === undefined ||
                jsonEntry.stackMode === 'yes' ||
                jsonEntry.stackMode === 'no' ||
                jsonEntry.stackMode === 'parent') &&
            (jsonEntry.visible === undefined ||
                typeof jsonEntry.visible === 'boolean') &&
            (jsonEntry.priority === undefined ||
                typeof jsonEntry.priority === 'number') &&
            (jsonEntry.iconMinLevel === undefined ||
                typeof jsonEntry.iconMinLevel === 'number') &&
            (jsonEntry.iconMaxLevel === undefined ||
                typeof jsonEntry.iconMaxLevel === 'number') &&
            (jsonEntry.textMinLevel === undefined ||
                typeof jsonEntry.textMinLevel === 'number') &&
            (jsonEntry.textMaxLevel === undefined ||
                typeof jsonEntry.textMaxLevel === 'number')
...
export class PoiTable {
    ...
    async load(
        ...
        if (!response.ok) {
            throw new Error(
                `load: Cannot load POI table at ${poiTableUrl}:` +
                    ` ${response.statusText}`
            )
        }
        ...
        try {
            logger.debug(
                `load: Loading POI table '${poiTableUrl}' for table '${this.name}'`
            )

            if (
                jsonPoiTable.poiList !== undefined &&
                Array.isArray(jsonPoiTable.poiList)
            ) {
            ...
            logger.error(
                `load: Failed to load POI table ` + `'${poiTableUrl}' : ${ex}`
            )
            this.m_loadedOk = false
            this.finishedLoading()
            return false
 ...
 * can be defined in the {@link @arca/datasource-protocol#Theme} sfile.
 */
export class PoiTableManager {
    private m_isLoading = false
    private m_poiTables: Map<string, PoiTable> = new Map()
    private readonly m_abortControllers: Map<string, AbortController> =
        new Map()
     ...
     * {@link @arca/datasource-protocol#Theme}.
     *
     * @remarks
     * Note that duplicate names of {@link PoiTable}s in the
     * {@link @arca/datasource-protocol#Theme} will lead to inaccessible {@link PoiTable}s.
     *
     * @param poiTables - {@link @arca/datasource-protocol#PoiTableRef[]}
     *                containing all {@link PoiTable}s to load.
     *
     * @returns Resolved once all the {@link PoiTable}s in
     *          the {@link @arca/datasource-protocol#Theme} have been loaded.
     */
            ...
            if (poiTables !== undefined) {
                ...
                poiTables.forEach((poiTableRef: PoiTableRef) => {
                    if (
                        ...
                        if (
                            poiTableRef.url !== undefined &&
                            typeof poiTableRef.url === 'string'
                        ) {
                            this.addTable(poiTable)
                            this.m_abortControllers.set(
                                poiTableRef.name,
                                new AbortController()
                            )
                            loadPromises.push(
                            ...
                            logger.error(
                                `POI table definition has no valid url: ${poiTableRef}`
                            )
                        }
                    } else {
                        logger.error(
                            `POI table definition has no valid name: ${poiTableRef}`
                        )
                    }
                })
     ...
     * are specified in the {@link @arca/datasource-protocol#Theme}.
     *
     * @remarks
    ...
    getPoiTable(poiTableName: string | undefined): PoiTable | undefined {
        return poiTableName === undefined
            ? undefined
            : this.m_poiTables.get(poiTableName)
    }
