...
import {
    composeTechniqueTextureName, Env, getPropertyValue, IndexedTechniqueParams,
    LineMarkerTechnique, MapEnv, PoiTechnique
...
/**
 * Constructs {@link PoiInfo} objects from {@link @arca/datasource-protocol/Technique} and
 * an icon.
 */
export class PoiBuilder {
    ...
    private m_technique?: (PoiTechnique | LineMarkerTechnique) &
    ...
    withTechnique(
        ...
        this.m_iconMinZoomLevel =
            getPropertyValue(
                technique.iconMinZoomLevel ?? technique.minZoomLevel,
                this.m_env
            ) ?? undefined
        this.m_iconMaxZoomLevel =
            getPropertyValue(
                technique.iconMaxZoomLevel ?? technique.maxZoomLevel,
                this.m_env
            ) ?? undefined
        this.m_textMinZoomLevel =
            getPropertyValue(
                technique.textMinZoomLevel ?? technique.minZoomLevel,
                this.m_env
            ) ?? undefined
        this.m_textMaxZoomLevel =
            getPropertyValue(
                technique.textMaxZoomLevel ?? technique.maxZoomLevel,
                this.m_env
            ) ?? undefined
    ...
    build(textElement: TextElement): PoiInfo | undefined {
        ...
        // data.
        const poiName =
            technique.poiTable !== undefined
                ? technique.poiName ?? imageTextureName
                : undefined
        ...
        if (imageTextureName === undefined && poiName === undefined) {
            textElement.minZoomLevel =
                textElement.minZoomLevel ?? this.m_textMinZoomLevel
            textElement.maxZoomLevel =
                textElement.maxZoomLevel ?? this.m_textMaxZoomLevel
        ...
        const renderTextDuringMovements = !(
            technique.renderTextDuringMovements === false
        )
        const iconMayOverlap =
            technique.iconMayOverlap ?? technique.textMayOverlap

        const iconReserveSpace =
            technique.iconReserveSpace ?? technique.textReserveSpace
        const iconColorRaw = getPropertyValue(technique.iconColor, env)
        const iconColor =
            iconColorRaw !== null
                ? ColorCache.instance.getColor(iconColorRaw)
                : undefined
