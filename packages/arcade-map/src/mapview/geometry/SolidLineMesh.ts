...
import { OrientedBox3 } from '@arca/geoutils'
import { SolidLineMaterial } from '@arca/materials'
import { assert } from '@arca/utils'

import {
    displaceBox, DisplacedBufferGeometry, DisplacementRange
...
function getAttributeInfo(
    ...
    const isBufferAttribute =
        (attribute as THREE.BufferAttribute).isBufferAttribute === true
...
function computeFeatureBoundingSphere(
    ...
    for (let i = featureBeginIndex; i < featureEndIndex; i += SEGMENT_STRIDE) {
        ...
        bbox.expandByPoint(
            vertex.fromBufferAttribute(pos, indices[i + VERTEX_STRIDE])
        )
    }
    ...
    if (displacementRange) {
        ...
        return displaceBox(bbox, displacementRange, normal).getBoundingSphere(
            sphere
        )
    }
 ...
 */
function intersectClosestEndCap(
    ray: THREE.Ray,
    line: THREE.Line3,
    hWidth: number
): number {
...
function intersectLine(
    ...
    if (
        interPlane.equals(ray.origin) &&
        ray.direction.dot(plane.normal) === 0
    ) {
        // Corner case: ray is coplanar to extruded line, find distance to extruded line sides
        // and end caps.
        const extrLineT = intersectExtrudedLine(
            ray,
            line,
            vExtrusion,
            plane.normal,
    ...
    const distSq = interPlane.distanceToSquared(
        line.closestPointToPoint(interPlane, true, tmpV4)
    )
...
function intersectFeature(
    ...
    for (let i = beginIdx; i < endIdx; i += SEGMENT_STRIDE) {
        ...
        plane.setFromCoplanarPoints(
            line.start,
            tmpV3.copy(line.start).add(vExt),
            line.end
        )
...
function intersectGroup(
    ...
    const halfWidth =
        (solidLineMaterial.lineWidth + solidLineMaterial.outlineWidth) / 2
    ...
    const localHalfWidth =
        halfWidth / ((mesh.scale.x + mesh.scale.y + mesh.scale.z) / 3)
    ...
    while (beginIdx < groupEndIdx) {
        ...
        const endIdx =
            featureIdx < lastFeatureIdx
                ? featureStarts[++featureIdx]
                : groupEndIdx
        if (bVolumeIdx >= bVolumes.length) {
            ...
            bVolumes.push(
                computeFeatureBoundingSphere(geometry, beginIdx, endIdx)
            )
        }
        intersectFeature(
            mesh,
...
export class SolidLineMesh extends THREE.Mesh {
    ...
    static raycast(
        ...
        const localRay = tmpRay
            .copy(raycaster.ray)
            .applyMatrix4(tmpInverseMatrix)
        ...
        if (
            ...
            mesh.userData.feature.attributeInfo =
                getAttributeInfo(positionAttribute)
        }
