...
import { GeometryType } from '@arca/datasource-protocol'
import { reconstructLineWidth } from '@arca/lines'
...
export function isLineAccessor(arg: any): arg is ILineAccessor {
    ...
    return (
        typeof arg.isLineAccessor === 'function' &&
        arg.isLineAccessor() === true
    )
}
...
export function isObject3dAccessor(arg: any): arg is IObject3dAccessor {
    return (
        typeof arg.isObject3dAccessor === 'function' &&
        arg.isObject3dAccessor() === true
    )
}
 ...
 */
export abstract class BufferedGeometryAccessorBase
    implements IGeometryAccessor
{
    ...
    constructor(
        ...
        if (bufferGeometry.type !== 'BufferGeometry') {
            logger.error(
                'IndexedBufferedGeometryAccessor#constructor: BufferGeometry has wrong ' +
                    'type'
            )
        }
        assert(
        ...
        this.position = this.bufferGeometry.getAttribute(
            'position'
        ...
        const getColor = (material: THREE.Material) => {
            ...
            if (
                ...
                if (rawShaderMaterial.name === 'SolidLineMaterial') {
                    return rawShaderMaterial.uniforms.diffuseColor
...
export abstract class IndexedBufferedGeometryAccessor extends BufferedGeometryAccessorBase {
    ...
    constructor(
        ...
        if (!this.indices) {
            logger.warn(
                'IndexedBufferedGeometryAccessor#constructor: BufferGeometry has no ' +
                    'index'
            )
            assert(!!this.indices)
        } else {
    ...
    implements ILineAccessor
{
    ...
    get width(): number | undefined {
        assert(
            this.checkSetUp(),
            'RoIndexedBufferedGeometryLineAccessor not setup'
        )
        ...
        if (itemSize === 3) {
            ...
            for (let i = start, j = 0; i < end; i++, j++) {
                ...
                for (let k = 0; k < itemSize; k++) {
                    result[j * itemSize + k] =
                        positionArray[index * itemSize + k]
                }
            }
        }
