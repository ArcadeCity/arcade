...
import { MapMeshBasicMaterial, MapMeshStandardMaterial } from '../../materials'
...
export namespace Object3DUtils {
    ...
    function estimateTextureSize(
        ...
        if (
            ...
            visitedObjects.get(texture.uuid) === true
        ) {
            return
        }
    ...
    function estimateMaterialSize(
        ...
        if (
            ...
            visitedObjects.get(material.uuid) === true
        ) {
            return
        }
        ...
        if (
            ...
            estimateTextureSize(
                standardMaterial.displacementMap,
                objectSize,
                visitedObjects
            )
            estimateTextureSize(
    ...
    function estimateGeometrySize(
        ...
        const isNewObject =
            geometry.uuid === undefined ||
            visitedObjects.get(geometry.uuid) !== true
        ...
        if (attributes === undefined) {
            logger.warn(
                'estimateGeometrySize: unidentified geometry: ',
                geometry
            )
            return
        }
    ...
    function estimateMeshSize(
        ...
        if (
            ...
            visitedObjects.get(object.uuid) === true
        ) {
            return
        }
