 ...
 */
import {
    DecodedTile, GeometryKind, GeometryKindSet, isExtrudedLineTechnique,
    isExtrudedPolygonTechnique, isFillTechnique, isLineMarkerTechnique,
    isLineTechnique, isPoiTechnique, isSegmentsTechnique, isSolidLineTechnique,
    isTextTechnique, Technique
} from '@arca/datasource-protocol'
import { PerformanceTimer, TaskQueue } from '@arca/utils'
...
export class TileGeometryLoader {
    ...
    static prepareAvailableGeometryKinds(
        ...
        for (const technique of decodedTile.techniques) {
            const geometryKind =
                TileGeometryLoader.compileGeometryKind(technique)
    ...
    private m_state: TileGeometryLoaderState =
        TileGeometryLoaderState.Initialized
    ...
    /**
     * Set the {@link @arca/datasource-protocol#DecodedTile} of the tile.
     *
     * @remarks
    ...
    setDecodedTile(decodedTile: DecodedTile): DecodedTile {
        ...
        if (this.hasDecodedTile) {
            this.m_availableGeometryKinds =
                TileGeometryLoader.prepareAvailableGeometryKinds(
                    this.m_decodedTile
                )
        }
    ...
    private queueGeometryCreation(
        ...
        this.m_taskQueue.add({
            execute: this.createGeometry.bind(
                this,
                enabledKinds,
                disabledKinds
            ),
    ...
    private async createGeometry(
        ...
        geometryCreator.initDecodedTile(
            decodedTile,
            enabledKinds,
            disabledKinds
        )
    ...
    private addStats(stats: PerformanceStatistics, now: number) {
        ...
        currentFrame.addValue(
            ...
            decodedTile.techniques.length
        )
        currentFrame.addValue(
            'geometryCount.numPoiGeometries',
            decodedTile.poiGeometries !== undefined
                ? decodedTile.poiGeometries.length
                : 0
        )
        currentFrame.addValue(
            'geometryCount.numTextGeometries',
            decodedTile.textGeometries !== undefined
                ? decodedTile.textGeometries.length
                : 0
        )
        currentFrame.addValue(
        ...
        currentFrame.addValue(
            ...
            decodedTile.pathGeometries !== undefined
                ? decodedTile.pathGeometries.length
                : 0
        )
