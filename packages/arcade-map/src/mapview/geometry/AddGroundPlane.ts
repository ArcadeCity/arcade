...
import { GeometryKind, Pickability } from '@arca/datasource-protocol'
import {
    EdgeLengthGeometrySubdivisionModifier, SphericalGeometrySubdivisionModifier,
    SubdivisionMode
} from '@arca/geometry'
import { Projection, ProjectionType } from '@arca/geoutils'
import { MapMeshBasicMaterial, MapMeshStandardMaterial } from '@arca/materials'
...
export function addGroundPlane(
    tile: Tile,
    renderOrder: number,
    materialOrColor: THREE.Material | THREE.Material[] | number = tile.mapView
        .clearColor,
    opacity: number = 1,
...
function toLocalTargetCoords(
    ...
    for (let i = 0; i < attr.array.length; i += 1) {
        ...
        tile.projection
            .reprojectPoint(srcProjection, tmpV, tmpV)
            .sub(tile.center)
...
function createGroundPlaneGeometry(
    ...
    if (useLocalTargetCoords) {
        for (const corner of cornersArray) {
            projection
                .reprojectPoint(sourceProjection, corner, corner)
                .sub(tile.center)
        }
    }
    ...
    const bufferArray = useLocalTargetCoords
        ? new Float32Array(12)
        : new Float64Array(12)
    const posAttr = new THREE.BufferAttribute(bufferArray, 3).copyVector3sArray(
        cornersArray
    )
    ...
    if (receiveShadow) {
        ...
        const normAttr = new THREE.BufferAttribute(
            new Float32Array(12),
            3
        ).copyVector3sArray(Array(4).fill(tmpV))
