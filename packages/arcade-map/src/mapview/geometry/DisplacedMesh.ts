...
import { DisplacementFeature, hasDisplacementFeature } from '@arca/materials'
import { assert } from '@arca/utils'

import { DisplacedBufferAttribute } from './DisplacedBufferAttribute'
import {
    DisplacedBufferGeometry, DisplacementRange
} from './DisplacedBufferGeometry'

function isDisplacementMaterial(
    material: any
): material is DisplacementFeature {
    const isDisplacementFeature = hasDisplacementFeature(material)
    assert(
        isDisplacementFeature,
        'Material does not support displacement maps.'
    )
    return isDisplacementFeature
}

function isDataTextureMap(
    map?: THREE.Texture | null
): map is THREE.DataTexture {
    if (!map) {
        return false
    }
    ...
    raycast(
        ...
        if (this.displacedGeometry) {
            ...
            this.displacedGeometry = new DisplacedBufferGeometry(
                ...
                displacementMap,
                displacementRange,
                DisplacedMesh.getDisplacedPositionAttribute(
                    geometry,
                    displacementMap
                )
            )
