...
import { GeometryType, getFeatureId } from '@arca/datasource-protocol'
import { assert, LoggerManager } from '@arca/utils'

import { Tile, TileFeatureData } from '../Tile'
import {
    BufferedGeometryLineAccessor, BufferedGeometryObject3dAccessor,
    IGeometryAccessor, ILineAccessor, IndexedBufferedGeometryLineAccessor,
    IObject3dAccessor, isLineAccessor, isObject3dAccessor
} from './TileGeometry'
...
export class TileDataAccessor {
    ...
    protected visitObject(object: THREE.Object3D): void {
        ...
        if (geometryType === undefined) {
            logger.warn(
                '#visitObject: visiting object failed, no geometryType',
                object
            )
            return
        }
        ...
        assert(
            Array.isArray(featureData.starts),
            'featureData.starts is not an array'
        )
        if (
            featureData.objInfos !== undefined &&
        ...
        if (object.type !== 'Mesh') {
            logger.warn(
                "#visitObject: visiting object failed, not of type 'Mesh'",
                object
            )
            return
        }
    ...
    protected getGeometryAccessor(
        ...
        switch (geometryType) {
            ...
            case GeometryType.ExtrudedLine:
            case GeometryType.TextPath:
                return new BufferedGeometryLineAccessor(
                    object,
                    geometryType,
                    bufferGeometry
                )
            ...
            case GeometryType.ExtrudedPolygon:
                ...
                return undefined
            case GeometryType.Object3D:
                return new BufferedGeometryObject3dAccessor(
                    object,
                    geometryType,
                    bufferGeometry
                )
    ...
    protected visitMesh(
        ...
        for (
            ...
            if (geometryAccessor === undefined) {
                ...
                if (geometryAccessor === undefined) {
                    logger.warn(
                        '#visitObject: no accessor geometryType',
                        geometryType
                    )
                    continue
                }
            ...
            switch (geometryType) {
                ...
                case GeometryType.TextPath:
                    ...
                    this.visitor.visitLine(
                        featureId,
                        geometryAccessor as any as ILineAccessor
                    )
                    break
                case GeometryType.Polygon:
                case GeometryType.ExtrudedPolygon:
