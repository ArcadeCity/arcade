 ...
 */
import {
    Attachment, BaseTechniqueParams, BufferAttribute, CallExpr, DecodedTile,
    Env, Expr, ExprEvaluatorContext, ExtrudedPolygonTechnique, FillTechnique,
    Geometry, GeometryKindSet, getArrayConstructor, getPropertyValue,
    IndexedTechnique, InterleavedBufferAttribute, isCirclesTechnique,
    isExtrudedLineTechnique, isExtrudedPolygonTechnique, isFillTechnique,
    isJsonExpr, isLineMarkerTechnique, isLineTechnique, isPoiTechnique,
    isSegmentsTechnique, isSolidLineTechnique, isSquaresTechnique,
    isStandardTechnique, isTerrainTechnique, isTextTechnique,
    MakeTechniqueAttrs, MapEnv, needsVertexNormals, OperatorDescriptor,
    Pickability, SolidLineTechnique, StandardExtrudedLineTechnique, Technique,
    TerrainTechnique, TextPathGeometry, transientToPickability, Value
} from '@arca/datasource-protocol'
import { EarthConstants, ProjectionType } from '@arca/geoutils'
import {
    EdgeMaterial, EdgeMaterialParameters, ExtrusionFeature, FadingFeature,
    hasExtrusionFeature, isHighPrecisionLineMaterial, MapMeshDepthMaterial,
    MapMeshStandardMaterial, setDisplacementMapToMaterial,
    setShaderMaterialDefine, SolidLineMaterial
} from '@arca/materials'
import { assert, LoggerManager } from '@arca/utils'

import {
    applyBaseColorToMaterial, buildMetricValueEvaluator, buildObject,
    createMaterial, getBufferAttribute, usesObject3D
} from '../DecodedTileHelpers'
import {
    createDepthPrePassMesh, isRenderDepthPrePassEnabled, setDepthPrePassStencil
} from '../DepthPrePass'
...
class AttachmentCache {
    readonly bufferAttributes = new Map<
        BufferAttribute,
...
class AttachmentInfo {
    ...
    getInterleavedBufferAttributes(description: InterleavedBufferAttribute) {
        const interleavedAttributes =
            this.cache.interleavedAttributes.get(description)
...
/**
 * Support class to create geometry for a {@link Tile} from a {@link @arca/datasource-protocol#DecodedTile}.
 * @internal
 */
export class TileGeometryCreator {
    ...
    initDecodedTile(
        ...
        for (const technique of decodedTile.techniques) {
            ...
            if (
                kind === undefined ||
                (kind instanceof Set && kind.size === 0)
            ) {
            ...
            technique._kindState =
                !(
                    disabledKinds !== undefined &&
                    disabledKinds.hasOrIntersects(kind)
                ) ||
                (enabledKinds !== undefined &&
                    enabledKinds.hasOrIntersects(kind))
        }
    ...
    createAllGeometries(tile: Tile, decodedTile: DecodedTile): Promise<void> {
        ...
        if (
            tile.dataSource.addGroundPlane &&
     ...
     * @param decodedTile - The {@link @arca/datasource-protocol#DecodedTile}.
     * @param textFilter -: Optional filter. Should return true for any text technique that is
     *      applicable.
     */
    createTextElements(
        ...
        const discreteZoomEnv = new MapEnv(
        ...
        if (decodedTile.textGeometries !== undefined) {
            for (const text of decodedTile.textGeometries) {
                if (
                    text.technique === undefined ||
                    text.stringCatalog === undefined
                ) {
                    continue
                }
        ...
        for (const attachment of this.getAttachments(decodedTile)) {
            ...
            for (let groupIndex = 0; groupIndex < groupCount; ) {
                ...
                for (
                    ...
                    groups[groupIndex].technique === techniqueIndex;
                    ++groupIndex
                ) {
                ...
                const extrusionAnimationEnabled: boolean =
                    animatedExtrusionHandler?.setAnimationProperties(
                        technique,
                        discreteZoomEnv
                    ) ?? false

                let material: THREE.Material | undefined =
                    materials[techniqueIndex]
                ...
                srcGeometry.vertexAttributes?.forEach((vertexAttribute) => {
                    const buffer =
                        attachment.getBufferAttribute(vertexAttribute)
                ...
                if (index) {
                    bufferGeometry.setIndex(
                        attachment.getBufferAttribute(index)
                    )
                }

                if (
                    !bufferGeometry.getAttribute('normal') &&
                    needsVertexNormals(technique)
                ) {
                ...
                const hasSolidLinesOutlines: boolean =
                    ...
                    technique.secondaryWidth !== undefined
                ...
                object.renderOrder = getPropertyValue(
                    technique.renderOrder,
                    mapView.env
                )
                ...
                if (
                    ...
                    ;(object as MapViewPoints).enableRayTesting =
                        technique.enablePicking!
                }
                ...
                if (renderDepthPrePass) {
                    const depthPassMesh = createDepthPrePassMesh(
                    ...
                    if (extrusionAnimationEnabled) {
                        addToExtrudedMaterials(
                            depthPassMesh.material,
                            extrudedMaterials
                        )
                    }
                ...
                if (
                    ...
                    const buildingTechnique =
                        technique as ExtrudedPolygonTechnique
                    ...
                    const extrusionAttribute =
                        bufferGeometry.getAttribute('extrusionAxis')
                    ...
                    edgeGeometry.setIndex(
                        attachment.getBufferAttribute(
                            attachment.info.edgeIndex!
                        )
                    )

                    // Read the uniforms from the technique values (and apply the default values).
                    const extrudedPolygonTechnique =
                        technique as ExtrudedPolygonTechnique
                    ...
                    const materialParams: EdgeMaterialParameters = {
                        ...
                        extrusionRatio: extrusionAnimationEnabled
                            ? 0
                            : undefined,
                        vertexColors: bufferGeometry.getAttribute('color')
                            ? true
                            : false,
                ...
                if (hasSolidLinesOutlines) {
                    ...
                    const outlineMaterial =
                        material.clone() as SolidLineMaterial
                    ...
                    const outlineMaterialAdapter = MapMaterialAdapter.create(
                        outlineMaterial,
                        {
                            ...
                            lineWidth: (frameMapView: MapAdapterUpdateEnv) => {
                                ...
                                const mainLineWidth =
                                    mainMaterialAdapter.currentStyledProperties
                                        .lineWidth
                                ...
                                const opacity = outlineMaterialAdapter
                                    .currentStyledProperties.opacity as
                                    | number
                                    | null
                                if (
    ...
    /**
     * Gets the attachments of the given {@link @arca/datasource-protocol#DecodedTile}.
     *
     * @param decodedTile - The {@link @arca/datasource-protocol#DecodedTile}.
     */
    private *getAttachments(
        decodedTile: DecodedTile
    ): Generator<AttachmentInfo> {
    ...
    diffuseColor *= texelColor;
        ...
        ;(material as MapMeshStandardMaterial).displacementMap!.needsUpdate =
            true
    }
    ...
    private addUserData(
        ...
        if (isTerrainTechnique(technique)) {
            ...
            const displacementMap = (
                srcGeometry.objInfos as DisplacementMap[]
            )[0]
