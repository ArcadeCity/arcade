...
import { PerformanceTimer, Task, TaskQueue } from '@arca/utils'
...
export class MapViewTaskScheduler extends THREE.EventDispatcher {
    ...
    processPending(frameStartTime: number) {
        ...
        if (this.throttlingEnabled) {
            ...
            availableTime =
                availableTime > 2 ? availableTime - 2 : availableTime
            currentFrameEvent?.setValue(
                'TaskScheduler.estimatedAvailableTime',
                availableTime
            )
            ...
            while (availableTime > 0 && numItemsLeft > 0) {
                ...
                function shouldProcess(task: Task) {
                    ...
                    availableTime -=
                        task.estimatedProcessTime?.() ??
                        DEFAULT_PROCESSING_ESTIMATE_TIME
