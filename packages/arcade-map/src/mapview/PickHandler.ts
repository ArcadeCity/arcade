...
import {
    GeometryType, getFeatureId, Technique
} from '@arca/datasource-protocol'
import { OrientedBox3, TileKey } from '@arca/geoutils'
...
export class PickHandler {
    ...
    intersectMapObjects(
        ...
        if (this.mapView.textElementsRenderer !== undefined) {
            ...
            this.mapView.textElementsRenderer.pickTextElements(
                scenePosition,
                pickListener
            )
        }
        ...
        for (const { tile, distance } of intersectedTiles) {
            if (
                pickListener.done &&
                pickListener.furthestResult!.distance < distance
            ) {
    ...
    private createResult(
        ...
        const featureData: TileFeatureData =
            intersection.object.userData.feature
    ...
    private getIntersectedTiles(
        ...
        tiles.sort(
            (
                lhs: { tile: Tile; distance: number },
                rhs: { tile: Tile; distance: number }
            ) => {
    ...
    private addObjInfo(
        ...
        if (
            ...
            (typeof intersect.faceIndex !== 'number' &&
