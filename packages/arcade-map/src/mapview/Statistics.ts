...
import { LoggerManager, PerformanceTimer } from '@arca/utils'
...
export namespace RingBuffer {
    ...
    export class Iterator<T> {
         ...
         */
        constructor(
            private readonly m_buffer: RingBuffer<T>,
...
export function computeArrayStats(samples: number[]): Stats | undefined {
    ...
    if (samples.length === 1) {
        ...
        median =
            samples.length % 2 === 0
                ? samples[mid - 1] * 0.5 + samples[mid] * 0.5
                : samples[mid]
...
export class MultiStageTimer {
     ...
     */
    get value(): number | undefined {
        return this.statistics.getTimer(this.stages[this.stages.length - 1])
            .value
    }
...
export class Statistics {
    ...
    createTimer(name: string, keepSamples = true): Timer {
        const timer = keepSamples
            ? new SampledTimer(this, name)
            : new SimpleTimer(this, name)
    ...
    log(header?: string, footer?: string) {
        ...
        this.timers.forEach((timer: Timer) => {
            let s =
                timer.name +
                ': ' +
                ' '.repeat(maxNameLength - timer.name.length)
            s += print(timer.value)
            ...
            if (timer instanceof SampledTimer) {
                ...
                if (simpleStats !== undefined) {
                    s +=
                        `  [ min=${print(simpleStats.min)}, max=${print(
                            simpleStats.max
                        )}, ` +
                        `avg=${print(simpleStats.avg)}, med=${print(
                            simpleStats.median
                        )}, ` +
...
export class FrameStatsArray {
    ...
    reset() {
        this.frameEntries.forEach(
            (buffer: RingBuffer<number>, name: string) => {
                buffer.clear()
            }
        )
        this.messages.clear()
...
export class PerformanceStatistics {
    ...
    static get instance(): PerformanceStatistics {
        if (PerformanceStatistics.m_instance === undefined) {
            PerformanceStatistics.m_instance = new PerformanceStatistics(
                false,
                0
            )
        }
    ...
    addWebGLInfo(webGlInfo: THREE.WebGLInfo) {
        if (webGlInfo.render !== undefined) {
            ...
            this.currentFrame.setValue(
                'gl.numTriangles',
                webGlInfo.render.triangles === null
                    ? 0
                    : webGlInfo.render.triangles
            )
        }
        if (webGlInfo.memory !== undefined) {
            this.currentFrame.setValue(
                'gl.numGeometries',
                webGlInfo.memory.geometries === null
                    ? 0
                    : webGlInfo.memory.geometries
            )
            this.currentFrame.setValue(
                'gl.numTextures',
                webGlInfo.memory.textures === null
                    ? 0
                    : webGlInfo.memory.textures
            )
        }
