...
import { getUrlOrigin, LoggerManager } from '@arca/utils'

import {
    isWorkerBootstrapRequest, WorkerBootstrapResponse
} from './WorkerBootstrapDefs'
...
export class WorkerLoader {
    ...
    static startWorker(
        ...
        return this.startWorkerImmediately(scriptUrl, timeout).catch(
            (error) => {
    ...
    static waitWorkerInitialized(
        ...
        return new Promise<Worker>((resolve, reject) => {
            ...
            const timerId = setTimeout(() => {
                ...
                reject(
                    new Error(
                        'Timeout exceeded when waiting for first message from worker.'
                    )
                )
            }, timeout)
        })
