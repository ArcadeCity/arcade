...
import { ViewRanges } from '@arca/datasource-protocol'
...
export class FixedClipPlanesEvaluator implements ClipPlanesEvaluator {
    ...
    constructor(
        readonly minNear: number = 1,
        readonly minFarOffset: number = 10
    ) {
    ...
    private invalidatePlanes(near: number, far: number) {
        ...
        const farDist: number = Math.max(
            this.minFar,
            far,
            nearDist + this.minFarOffset
        )
