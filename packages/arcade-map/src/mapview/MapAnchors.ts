...
import { StylePriority } from '@arca/datasource-protocol'
import {
    GeoCoordinates, GeoCoordLike, isGeoCoordinatesLike, isVector3Like,
    Projection, Vector3Like
} from '@arca/geoutils'

/**
 * An interface describing [[THREE.Object3D]]s anchored on
 * given {@link @arca/geoutils#GeoCoordinates}.
 *
 * @remarkks
 * @example
 * Example:
...
export type MapAnchor<T extends THREE.Object3D = THREE.Object3D> = T & {
    /**
     * The position of this [[MapAnchor]] in {@link @arca/geoutils#GeoCoordinates}.
     * @deprecated Use [[anchor]] instead.
     */
    geoPosition?: GeoCoordinates

    /**
     * The anchor of this Object3D in {@link @arca/geoutils#GeoCoordinates}
...
export class MapAnchors {
    ...
    remove(mapAnchor: MapAnchor) {
        const index = this.m_anchors.findIndex(
            (element) => element === mapAnchor
        )
        if (index > -1) {
        ...
        this.m_anchors.forEach((mapAnchor: MapAnchor) => {
            if (mapAnchor.styleSet !== undefined) {
                const priority = this.m_priorities?.findIndex(
                    (entry) =>
                        entry.group === mapAnchor.styleSet &&
                        entry.category === mapAnchor.category
                )
            ...
            const anchor =
                mapAnchor.geoPosition !== undefined
