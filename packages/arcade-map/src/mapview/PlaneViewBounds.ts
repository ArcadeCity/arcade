...
import {
    Frustum, Line3, Matrix4, PerspectiveCamera, Plane, Ray, Vector3
} from 'three'
...
export class PlaneViewBounds implements ViewBounds {
    ...
    generate(): GeoPolygon | undefined {
        ...
        const worldCorners: TileCorners = this.getWorldConers(
            this.projection
        ) as TileCorners
        ...
        frustum.planes.forEach((plane) => {
            ...
            if (intersection) {
                ...
                if (MapViewUtils.closeToFrustum(intersection, this.camera)) {
                    const geoIntersection =
                        this.projection.unprojectPoint(intersection)
    ...
    private getVerticalHorizonPositionInNDC(): number | undefined {
        ...
        const topMidFarPoint = new Vector3(-1, 1, 1)
            ...
            .add(new Vector3(1, 1, 1).unproject(this.camera))
            .multiplyScalar(0.5)
        const farPlaneVerticalCenterLine = new Line3(
            bottomMidFarPoint,
            topMidFarPoint
        )
        ...
        if (
            !this.m_groundPlane.intersectLine(
                farPlaneVerticalCenterLine,
