 ...
 */
import {
    ImageDefinitions, ImageTexture, PoiTableRef, Theme
} from '@arca/datasource-protocol'
import { LoggerManager, UriResolver } from '@arca/utils'
...
export class MapViewThemeManager {
    ...
    async setTheme(theme: Theme | string): Promise<Theme> {
        if (this.isUpdating()) {
            logger.warn(
                'Formerly set Theme is still updating, update will be canceled'
            )
