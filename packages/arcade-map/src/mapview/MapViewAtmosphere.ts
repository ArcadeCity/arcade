...
import { Theme } from '@arca/datasource-protocol'
import { EarthConstants, Projection, ProjectionType } from '@arca/geoutils'
import {
    GroundAtmosphereMaterial, SkyAtmosphereMaterial
} from '@arca/materials'
...
export class MapViewAtmosphere {
    ...
    set lightMode(lightMode: AtmosphereLightMode) {
        if (
            this.m_materialVariant !== AtmosphereShadingVariant.ScatteringShader
        ) {
            return
        }
     ...
     * {@link @arca/datasource-protocol#Theme} instance provided.
     *
     ...
     *
     * @param theme - A {@link @arca/datasource-protocol#Theme} instance.
     */
    reset(theme: Theme) {
    ...
    private addToMapAnchors(mapAnchors: MapAnchors) {
        assert(
            !MapViewAtmosphere.isPresent(mapAnchors),
            'Atmosphere already added'
        )
        if (this.m_skyMesh !== undefined) {
            mapAnchors.add(
                createMapAnchor(this.m_skyMesh, Number.MIN_SAFE_INTEGER)
            )
        }
        if (this.m_groundMesh !== undefined) {
            mapAnchors.add(
                createMapAnchor(this.m_groundMesh, Number.MAX_SAFE_INTEGER)
            )
        }
    ...
    private createSkyGeometry() {
            ...
            case ProjectionType.Spherical:
                this.m_skyGeometry = new THREE.SphereGeometry(
                    EarthConstants.EQUATORIAL_RADIUS *
                        (1 + SKY_ATMOSPHERE_ALTITUDE_FACTOR),
                    256,
                    256
                )
                break
        ...
        if (
            ...
            this.m_materialVariant === AtmosphereShadingVariant.SimpleColor
        ) {
    ...
    private createGroundGeometry() {
        switch (this.m_projection.type) {
            case ProjectionType.Spherical:
                this.m_groundGeometry = new THREE.SphereGeometry(
                    EarthConstants.EQUATORIAL_RADIUS *
                        (1 + GROUND_ATMOSPHERE_ALTITUDE_FACTOR),
                    256,
                    256
                )
                break
        ...
        this.m_groundMesh = new THREE.Mesh(
            this.m_groundGeometry,
            this.m_groundMaterial
        )
...
function createMapAnchor(
    mesh: THREE.Mesh,
    renderOrder: number
): MapAnchor<THREE.Mesh> {
