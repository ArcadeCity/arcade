...
import { EarthConstants, sphereProjection } from '@arca/geoutils'
import { Math2D } from '@arca/utils'
...
export class SphereHorizon {
    ...
    constructor(
        ...
        const xAxis = new THREE.Vector3()
            .setFromMatrixColumn(m_camera.matrixWorld, 0)
            .normalize()
        const zAxis = m_camera.position.clone().normalize()
        const yAxis = new THREE.Vector3().crossVectors(zAxis, xAxis)

        const cameraHeight = m_camera.position.length()
        this.m_normalToTangentAngle = Math.asin(
            EarthConstants.EQUATORIAL_RADIUS / cameraHeight
        )

        const tangentDistance = Math.sqrt(
            cameraHeight * cameraHeight - earthRadiusSq
        )
        this.m_distanceToHorizonCenter =
            tangentDistance * Math.cos(this.m_normalToTangentAngle)
        const horizonCenterLength =
            cameraHeight - this.m_distanceToHorizonCenter
        this.m_radius = Math.sqrt(
            earthRadiusSq - horizonCenterLength * horizonCenterLength
        )
    ...
    getPoint(
        ...
        const endAngle =
            arcEnd >= arcStart ? arcEnd * twoPi : (arcEnd + 1) * twoPi
        const deltaAngle = endAngle - startAngle
        const angle = startAngle + t * deltaAngle

        target.set(
            this.m_radius * Math.cos(angle),
            this.m_radius * Math.sin(angle),
            0
        )
    ...
    getDivisionPoints(
        ...
        const numPoints = Math.max(
            Math.ceil(
                ((tEnd < tStart ? 1 + tEnd : tEnd) - tStart) * maxNumPoints
            ),
            1
        )
        ...
        switch (side) {
            case CanvasSide.Top: {
                const eyeToTangentAngle =
                    this.m_normalToTangentAngle - this.m_cameraPitch
                return (
                    CameraUtils.getTopFov(this.m_camera) >=
                    Math.abs(eyeToTangentAngle)
                )
            }
            case CanvasSide.Bottom: {
                const eyeToTangentAngle =
                    this.m_normalToTangentAngle + this.m_cameraPitch
                return (
                    CameraUtils.getBottomFov(this.m_camera) >=
                    Math.abs(eyeToTangentAngle)
                )
            }
            case CanvasSide.Left: {
                ...
                return (
                    CameraUtils.getLeftFov(this.m_camera) >=
                        Math.abs(eyeToTangentAngle) &&
                    this.m_cameraPitch <=
                        CameraUtils.getBottomFov(this.m_camera)
                )
            ...
            case CanvasSide.Right: {
                const eyeToTangentAngle = this.m_normalToTangentAngle
                return (
                    CameraUtils.getRightFov(this.m_camera) >=
                        Math.abs(eyeToTangentAngle) &&
                    this.m_cameraPitch <=
                        CameraUtils.getBottomFov(this.m_camera)
                )
            ...
            switch (side) {
                case CanvasSide.Bottom: {
                    sideIntersections.push(
                        ...this.computeTBIntersections(yBottom)
                    )
                    break
                }
                case CanvasSide.Right: {
                    const rightFov = CameraUtils.getRightFov(this.m_camera)
                    const intersections = this.computeLRIntersections(
                        yBottom,
                        rightFov
                    )
                    if (intersections) {
                ...
                case CanvasSide.Top: {
                    ...
                    sideIntersections.push(
                        ...this.computeTBIntersections(yTop).reverse()
                    )
                    break
                }
                case CanvasSide.Left: {
                    ...
                    if (leftFov === CameraUtils.getRightFov(this.m_camera)) {
                        ...
                        const isections = this.computeLRIntersections(
                            yBottom,
                            leftFov
                        )
                        if (isections) {
    ...
    private computeLRIntersections(
        ...
        const eyeToHorizon =
            this.m_distanceToHorizonCenter / Math.cos(this.m_cameraPitch)
        const yMiddle =
            this.m_distanceToHorizonCenter * Math.tan(this.m_cameraPitch)
        ...
        const hasBottomIntersection =
            -yTopRight >= yBottom && intersections.x2 !== undefined
