...
import {
    Color, CubeTexture, DataTexture, Line3, Plane, RGBFormat, Texture, Vector3
} from 'three'

import { GradientSky } from '@arca/datasource-protocol'
import { ProjectionType } from '@arca/geoutils'
import { getOptionValue } from '@arca/utils'
...
export class SkyGradientTexture {
    ...
    constructor(
        ...
        this.m_width =
            this.m_projectionType === ProjectionType.Planar
                ? 1.0
                : this.m_height
        this.m_faceCount =
            this.m_projectionType === ProjectionType.Planar ? 1.0 : 6.0
        this.m_faces = []
     ...
     */
    get texture(): Texture {
        return this.m_projectionType === ProjectionType.Planar
            ? this.m_faces[0]
            : this.m_skybox!
    }
    ...
    private setHorizonPosition(camera: THREE.Camera) {
        ...
        this.m_topMidFarPoint!.copy(this.m_farClipPlaneCorners![2])
            ...
            .multiplyScalar(0.5)
        this.m_farClipPlaneDividedVertically!.set(
            this.m_bottomMidFarPoint,
            this.m_topMidFarPoint
        )
    ...
    private updateTexturePosition() {
        const coveredBySky = this.m_bottomMidFarPoint!.distanceTo(
            this.m_horizonPosition!
        )
        ...
        const ratio =
            this.m_horizonPosition!.length() === 0
                ? 1
                : skyRatio - 2 / this.m_height
        ...
        // up.
        this.m_faces[0].offset.set(
            0,
            this.m_bottomMidFarPoint!.z <= 0 ? -ratio : skyRatio
        )
    }
}
