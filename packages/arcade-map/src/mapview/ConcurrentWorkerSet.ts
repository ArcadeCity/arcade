 ...
 */
import {
    RequestController, WorkerServiceProtocol
} from '@arca/datasource-protocol'
import {
    getOptionValue, IWorkerChannelMessage, LoggerManager, LogLevel,
...
export function isLoggingMessage(
    message: IWorkerChannelMessage
): message is IWorkerChannelMessage {
    return (
        message &&
        typeof message.level === 'number' &&
        message.type === WORKERCHANNEL_MSG_TYPE
    )
}
...
export class ConcurrentWorkerSet {
    private readonly m_workerChannelLogger =
        LoggerManager.instance.create('WorkerChannel')
    private readonly m_eventListeners = new Map<
        string,
        (message: any) => void
    >()
    ...
    private readonly onWorkerMessage = (
        ...
        if (WorkerServiceProtocol.isResponseMessage(event.data)) {
            ...
            if (response.messageId === null) {
                logger.error(
                    `[${this.m_options.scriptUrl}]: Bad ResponseMessage: no messageId`
                )
                return
            }
            ...
            if (workerId >= 0 && workerId < this.m_workers.length) {
                ...
                logger.error(
                    `[${this.m_options.scriptUrl}]: onWorkerMessage: invalid workerId`
                )
            }
    ...
    private postRequestMessage(
        ...
        if (this.m_workers.length === 0) {
            throw new Error(
                'ConcurrentWorkerSet#postMessage: no workers started'
            )
        }
    ...
    private terminateWorkers() {
        ...
        this.m_workerPromises.forEach((workerPromise) => {
            workerPromise.then((workerEntry) => {
                ...
                workerEntry.worker.removeEventListener(
                    'message',
                    workerEntry.listener
                )
    ...
    private checkWorkerRequestQueue() {
        if (
            this.m_workerRequestQueue.length === 0 ||
            this.m_availableWorkers.length === 0
        ) {
            return
        }
        this.m_workerRequestQueue.sort(
            (a: WorkerRequestEntry, b: WorkerRequestEntry) => {
                return (
                    a.requestController!.priority -
                    b.requestController!.priority
                )
            }
        )
