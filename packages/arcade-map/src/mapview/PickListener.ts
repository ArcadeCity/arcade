...
function defaultSort(lhs: PickResult, rhs: PickResult) {
    ...
    const haveRenderOrder =
        lhs.renderOrder !== undefined && rhs.renderOrder !== undefined
...
export class PickListener {
    ...
    addResult(result: PickResult): void {
        ...
        const foundFeatureIdx = this.m_results.findIndex((otherResult) => {
            ...
            const sameDataSource =
                dataSource &&
                otherResult.intersection?.object.userData?.dataSource ===
                    dataSource
            const sameId =
                result.featureId !== undefined &&
                otherResult.featureId === result.featureId
            const noId =
                result.featureId === undefined &&
                otherResult.featureId === undefined
            const sameUserData =
                result.userData && otherResult.userData === result.userData
            return (
                sameType && sameDataSource && (sameId || (noId && sameUserData))
            )
        })
    ...
    get results(): PickResult[] {
        assert(
            this.m_finished,
            'finish() was not called before getting the results'
        )
    ...
    get furthestResult(): PickResult | undefined {
        ...
        return this.m_results.length > 0
            ? this.m_results[this.m_results.length - 1]
            : undefined
    }
