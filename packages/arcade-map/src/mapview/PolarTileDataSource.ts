 ...
 */
import {
    getStyles, StandardGeometryKind, Technique, Theme
} from '@arca/datasource-protocol'
import {
    MapEnv, StyleSetEvaluator
} from '@arca/datasource-protocol/index-decoder'
import {
    GeoCoordinates, MercatorConstants, polarTilingScheme, TileKey, TilingScheme,
...
export class PolarTileDataSource extends DataSource {
    ...
    private readonly m_maxLatitude = THREE.MathUtils.radToDeg(
    ...
    private createTileGeometry(tile: Tile): void {
        ...
        const techniqueEntry = isNorthPole
            ? this.m_northPoleEntry
            : this.m_southPoleEntry
        ...
        const box = this.m_tilingScheme.boundingBoxGenerator.getWorldBox(
            tile.tileKey
        )

        const pBL = srcProjection.unprojectPoint(
            new THREE.Vector3(box.min.x, box.min.y, 0)
        )
        const pBR = srcProjection.unprojectPoint(
            new THREE.Vector3(box.max.x, box.min.y, 0)
        )
        const pTR = srcProjection.unprojectPoint(
            new THREE.Vector3(box.max.x, box.max.y, 0)
        )
        const pTL = srcProjection.unprojectPoint(
        ...
        if (tile.tileKey.level === 1) {
            ...
            const pPole = srcProjection.unprojectPoint(
                new THREE.Vector3(poleX, poleY, 0)
            )
            ...
            const isSomePointsOut = isNorthPole
                ? lmin < poleLat
                : lmax > poleLat
        ...
        if (needsGeometryCut) {
            ...
            const center = srcProjection.unprojectPoint(
            ...
            const level =
                tile.tileKey.level -
                this.storageLevelOffset +
                this.m_geometryLevelOffset
        ...
        const vertices: THREE.Vector3[] = points.map((point) => {
            const projected = dstProjection.projectPoint(
                point,
                new THREE.Vector3()
            )
        ...
        if (this.m_debugTiles) {
            const color = Math.round(
                Math.abs(Math.sin(11 * tile.tileKey.mortonCode())) * 0xffffff
            )
        ...
        MapObjectAdapter.create(mesh, {
            ...
            kind: [
                isNorthPole
                    ? StandardGeometryKind.Water
                    : StandardGeometryKind.Background,
            ],
        })
