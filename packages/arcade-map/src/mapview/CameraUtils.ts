...
function setCameraParams(
    ...
    if (hFov < MIN_FOV_RAD || hFov > MAX_FOV_RAD) {
        ...
        const focalLength = computeFocalLengthFromFov(
            hFov,
            viewportWidth,
            ppalPoint.x
        )
 ...
 */
function computeFocalLengthFromFov(
    fov: number,
    viewportSide: number,
    ppOffset: number
): number {
...
export namespace CameraUtils {
    ...
    export function setVerticalFov(
        ...
        verticalFov = THREE.MathUtils.clamp(
            verticalFov,
            MIN_FOV_RAD,
            MAX_FOV_RAD
        )
     ...
     */
    export function setPrincipalPoint(
        camera: THREE.PerspectiveCamera,
        ...
        camera.setViewOffset(
            width,
            height,
            (-THREE.MathUtils.clamp(ndcCoords.x, -maxNdcCoord, maxNdcCoord) *
                width) /
                2,
            (THREE.MathUtils.clamp(ndcCoords.y, -maxNdcCoord, maxNdcCoord) *
                height) /
                2,
            width,
            height
