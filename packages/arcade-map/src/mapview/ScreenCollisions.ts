...
import { LoggerManager, Math2D } from '@arca/utils'
...
export class DetailedCollisionBox extends CollisionBox {
    constructor(
        box: Math2D.Box | THREE.Box2 | IBox,
...
export class ScreenCollisions {
    ...
    allocate(bounds: Math2D.Box | CollisionBox | DetailedCollisionBox): void {
        const bbox = !(bounds instanceof CollisionBox)
            ? new CollisionBox(bounds)
            : bounds
    ...
    isAllocated(bounds: Math2D.Box | CollisionBox): boolean {
        const collisionBox =
            bounds instanceof CollisionBox
                ? bounds
                : tmpCollisionBox.copy(bounds)
    ...
    private intersectsLine(
        ...
        if (lineXDiffTransformed !== 0) {
            ...
            const D =
                line.start.y -
                (lineYDiffTransformed / lineXDiffTransformed) * line.start.x
...
export class ScreenCollisionsDebug extends ScreenCollisions {
    ...
    update(width: number, height: number) {
        ...
        this.m_renderingEnabled = debugContext.getValue(
            'DEBUG_SCREEN_COLLISIONS'
        )
    }
    ...
    intersectsDetails(testBox: CollisionBox, boxes: IBox[]): boolean {
        ...
        if (this.m_renderingEnabled && this.m_renderContext !== null) {
            ...
            this.m_renderContext.strokeStyle = collisionFound
                ? '#FF0000'
                : '#00ff00'
            this.m_renderContext.strokeRect(
