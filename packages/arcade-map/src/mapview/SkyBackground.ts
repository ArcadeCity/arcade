...
import { CubemapSky, GradientSky } from '@arca/datasource-protocol'
...
export class SkyBackground {
    ...
    constructor(
        ...
        switch (this.m_sky.type) {
            case 'gradient':
                this.m_skyTexture = new SkyGradientTexture(
                    this.m_sky,
     ...
     */
    updateTexture(
        params: GradientSky | CubemapSky,
        projectionType: ProjectionType
    ) {
        const isSameSkyType =
            this.m_sky.type === params.type &&
            this.m_projectionType === projectionType
        switch (params.type) {
            case 'gradient':
                if (isSameSkyType) {
                    ;(this.m_skyTexture! as SkyGradientTexture).updateTexture(
                        params
                    )
                } else {
                    this.m_skyTexture = new SkyGradientTexture(
            ...
            case 'cubemap': {
                if (isSameSkyType) {
                    ;(this.m_skyTexture! as SkyCubemapTexture).updateTexture(
                        params
                    )
                } else {
