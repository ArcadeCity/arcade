...
import { LineCaps, LineDashes } from '@arca/datasource-protocol'
...
import {
    enforceBlending, getShaderMaterialDefine, setShaderDefine,
    ...
    implements DisplacementFeature, FadingFeature
{
                ...
                uniforms: THREE.UniformsUtils.merge([
                    {
                        ...
                        extrusionWidth: new THREE.Uniform(
                            SolidLineMaterial.DEFAULT_WIDTH
                        ),
                        outlineWidth: new THREE.Uniform(
                            SolidLineMaterial.DEFAULT_OUTLINE_WIDTH
                        ),
                        offset: new THREE.Uniform(
                            SolidLineMaterial.DEFAULT_OFFSET
                        ),
                        opacity: new THREE.Uniform(
                            SolidLineMaterial.DEFAULT_OPACITY
                        ),
                        tileSize: new THREE.Uniform(new THREE.Vector2()),
                        fadeNear: new THREE.Uniform(
                            FadingFeature.DEFAULT_FADE_NEAR
                        ),
                        fadeFar: new THREE.Uniform(
                        ...
                        dashSize: new THREE.Uniform(
                            SolidLineMaterial.DEFAULT_DASH_SIZE
                        ),
                        gapSize: new THREE.Uniform(
    ...
    get caps(): LineCaps {
        ...
        if (
            typeof capsMode === 'number' &&
            DefinesLineCapsMapping.hasOwnProperty(capsMode)
        ) {
    ...
    get dashes(): LineDashes {
        ...
        if (
            typeof dashesMode === 'number' &&
            DefinesLineDashesMapping.hasOwnProperty(dashesMode)
        ) {
    ...
    set dashes(value: LineDashes) {
        ...
        if (LineDashesDefinesMapping.hasOwnProperty(value)) {
            setShaderMaterialDefine(
                this,
                'DASHES_MODE',
                LineDashesDefinesMapping[value]
            )
        }
    }
