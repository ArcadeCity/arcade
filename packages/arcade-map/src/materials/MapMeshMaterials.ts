...
import {
    DisplacementFeature, DisplacementFeatureParameters
...
import {
    disableBlending, enableBlending, insertShaderInclude, setShaderDefine
} from './Utils'
 ...
 */
export interface FadingFeature
 ...
 */
export function hasExtrusionFeature(
    material: any
): material is ExtrusionFeature {
...
namespace DisplacementFeature {
    ...
    export function updateDisplacementFeature(
        ...
        if (useDisplacementMap) {
            ...
            displacementMaterial.shaderUniforms!.displacementMap.value =
                emptyTexture
        }
    }
...
export class DisplacementFeatureMixin
    implements DisplacementFeature, MixinShaderProperties
{
    needsUpdate?: boolean
    uniformsNeedUpdate?: boolean
    defines?: any
     ...
     */
    protected applyDisplacementParameters(
        params?: DisplacementFeatureParameters
    ) {
...
export namespace FadingFeature {
    ...
    export function updateFadingFeature(fadingMaterial: FadingFeature): void {
        ...
        if (useFading) {
            fadingMaterial.shaderUniforms!.fadeNear.value =
                fadingMaterial.fadeNear
            fadingMaterial.shaderUniforms!.fadeFar.value =
...
export class FadingFeatureMixin implements FadingFeature {
    ...
    protected applyFadingParameters(params?: FadingFeatureParameters) {
        ...
        // Create uniforms with default values, this ensures they are always set created,
        // so no need for checks in setters.
        this.shaderUniforms!.fadeNear = new THREE.Uniform(
            FadingFeature.DEFAULT_FADE_NEAR
        )
        this.shaderUniforms!.fadeFar = new THREE.Uniform(
            FadingFeature.DEFAULT_FADE_FAR
        )
    ...
    protected copyFadingParameters(source: FadingFeature) {
        this.setFadeNear(
            source.fadeNear === undefined
                ? FadingFeature.DEFAULT_FADE_NEAR
                : source.fadeNear
        )
        this.setFadeFar(
            source.fadeFar === undefined
                ? FadingFeature.DEFAULT_FADE_FAR
                : source.fadeFar
        )
        return this
...
export namespace ExtrusionFeature {
     ...
     */
    export function updateExtrusionFeature(
 ...
 * {@link @arca/mapview#AnimatedExtrusionHandler} that is
...
{
    constructor(
        params?: THREE.MeshDepthMaterialParameters & ExtrusionFeatureParameters
    ) {
        super(params)
