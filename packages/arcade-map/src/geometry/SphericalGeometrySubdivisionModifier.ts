...
import { Projection, sphereProjection } from '@arca/geoutils'
...
export class SphericalGeometrySubdivisionModifier extends SubdivisionModifier {
    ...
    protected shouldSplitTriangle(
        ...
        const aa = sphereProjection.reprojectPoint(
            this.projection,
            a,
            VERTEX_POSITION_CACHE[0]
        )
        const bb = sphereProjection.reprojectPoint(
            this.projection,
            b,
            VERTEX_POSITION_CACHE[1]
        )
        const cc = sphereProjection.reprojectPoint(
            this.projection,
            c,
