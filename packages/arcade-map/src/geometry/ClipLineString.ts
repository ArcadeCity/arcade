...
import {
    EarthConstants, GeoCoordinates, webMercatorProjection
} from '@arca/geoutils'
 ...
 */
export function wrapLineString(
    coordinates: GeoCoordinates[]
): Partial<WrappedLineString> {
    const worldP = new Vector3()
    ...
    return {
        left: wrapMultiLineStringHelper(
            multiLineString,
            WRAP_LEFT_CLIP_EDGES,
            360
        ),
        middle: wrapMultiLineStringHelper(
            multiLineString,
            WRAP_MIDDLE_CLIP_EDGES,
            0
        ),
        right: wrapMultiLineStringHelper(
            multiLineString,
            WRAP_RIGHT_CLIP_EDGES,
            -360
        ),
    }
}
