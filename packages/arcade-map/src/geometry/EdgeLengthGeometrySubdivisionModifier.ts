...
import { Box3Like, GeoBox, Projection, ProjectionType } from '@arca/geoutils'
...
export class EdgeLengthGeometrySubdivisionModifier extends SubdivisionModifier {
    ...
    constructor(
        ...
        const northEast = projection.projectPoint(
            geoBox.northEast,
            VERTEX_POSITION_CACHE[0]
        )
        const southWest = projection.projectPoint(
            geoBox.southWest,
        ...
        this.m_maxLength = Math.sqrt(
            this.m_maxLengthX * this.m_maxLengthX +
                this.m_maxLengthY * this.m_maxLengthY
        )
    }
