...
import {
    QUAD_VERTEX_MEMORY_FOOTPRINT, TextGeometry
} from './rendering/TextGeometry'
import { SdfTextMaterial } from './rendering/TextMaterials'
import {
    FontVariant, TextLayoutStyle, TextRenderStyle
} from './rendering/TextStyle'
import { LineTypesetter } from './typesetting/LineTypesetter'
import {
    PathTypesetter, PathTypesettingParameters
} from './typesetting/PathTypesetter'
...
export class TextCanvas {
    private static readonly defaultTextRenderStyle: TextRenderStyle =
        new TextRenderStyle()
    private static readonly defaultTextLayoutStyle: TextLayoutStyle =
        new TextLayoutStyle()
    /**
     * Minimum amount of glyphs each [[TextCanvas]] layer can store.
     */
    ...
    private placeText(params: TextPlacementParameters): boolean {
        if (
            ...
            this.m_currentTextLayoutStyle.maxLines! === 0
        ) {
        ...
        if (params.computeTextBuffer === true) {
            tempVertexBuffer = new Float32Array(
                glyphArray.length * QUAD_VERTEX_MEMORY_FOOTPRINT
            )
        }
        ...
        const typesettingParams:
            ...
            smallCapsArray: smallCapsEnabled
                ? smallCapsTransformations
                : undefined,
            globalBounds: params.bounds,
            individualBounds: glyphBounds,
            vertexBuffer:
                params.computeTextBuffer === true
                    ? tempVertexBuffer
                    : undefined,
        }
