...
export class LineTypesetter implements Typesetter {
    ...
    arrangeGlyphs(params: TypesettingParameters): boolean {
        ...
        this.m_tempScale =
            this.m_tempPixelSize / this.m_currentParams.fontCatalog.size
        ...
        this.m_tempSmallCaps =
            this.m_currentParams!.smallCapsArray !== undefined
        ...
        for (let i = 0; i < this.m_currentParams.glyphs.length; ++i) {
            ...
            if (
                !isBidirectionalLine &&
                glyphData.direction === -this.m_tempLineDirection
            ) {
            ...
            if (UnicodeUtils.isPrintable(glyphData.codePoint)) {
                lineCurrX +=
                    (glyphData.advanceX +
                        this.m_currentParams.textLayoutStyle.tracking) *
                    this.m_tempScale *
                    (this.m_tempSmallCaps
            ...
            if (
                isNewLine ||
                (this.m_currentParams.textLayoutStyle.wrappingMode ===
                    WrappingMode.Character &&
                    lineCurrX >
                        this.m_currentParams.textLayoutStyle.lineWidth) ||
                (this.m_currentParams.textLayoutStyle.wrappingMode ===
                    WrappingMode.Word &&
                    lineCurrX >
                        this.m_currentParams.textLayoutStyle.lineWidth &&
                ...
                // all glyphs in it.
                const lineAlignment =
                    this.m_tempLineDirection === UnicodeUtils.Direction.RTL &&
                    isBidirectionalLine
                        ? 1.0 +
                          this.m_currentParams.textLayoutStyle
                              .horizontalAlignment
                        : this.m_currentParams.textLayoutStyle
                              .horizontalAlignment
                ...
                while (
                    ...
                    UnicodeUtils.isWhiteSpace(
                        this.m_currentParams.glyphs[i + 1].codePoint
                    )
                ) {
                    ++i
                }
    ...
    private placeLine(
        ...
        for (let i = startIdx; i <= endIdx; ++i) {
            ...
            // strong direction in the run.
                ...
                if (
                    glyphDataArray[neutralIdx].direction !==
                    this.m_tempRunDirection
                ) {
                    if (isRTL) {
                        position.x = origin + offset
                    }
                    if (
                        !this.placeRun(runStart, i - 1, this.m_tempRunDirection)
                    ) {
                        return false
                    }
                    if (!isRTL) {
    ...
    private placeRun(
        ...
        const start =
            direction === UnicodeUtils.Direction.LTR ? startIdx : endIdx
        ...
        for (
            ...
            if (
                ...
                while (
                    ...
                    (weakGlyph.direction === UnicodeUtils.Direction.Weak ||
                        (weakGlyph.direction ===
                            UnicodeUtils.Direction.Neutral &&
                ...
                this.placeRun(
                    Math.max(weakRunStart, startIdx),
                    i,
                    UnicodeUtils.Direction.LTR
                )
            ...
            const isSmallCaps = this.m_tempSmallCaps
                ? smallCapsArray![i] &&
                  textRenderStyle.fontVariant === FontVariant.SmallCaps
                : false
            ...
            for (let j = 0; j < 4; ++j) {
                ...
                const horizontalOffset =
                    isItalicEmulated && j > 1
                        ? TypesettingUtils.OBLIQUE_OFFSET *
                          glyphFontMetrics.size
                        : 0.0
