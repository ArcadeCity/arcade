...
export const QUAD_VERTEX_MEMORY_FOOTPRINT =
    VERTICES_PER_QUAD * VERTEX_BUFFER_STRIDE
export const QUAD_INDEX_MEMORY_FOOTPRINT =
    INDICES_PER_QUAD * INDEX_BUFFER_STRIDE
...
export class TextGeometry {
    ...
    constructor(
        ...
        this.m_vertexBuffer = new THREE.InterleavedBuffer(
            new Float32Array(
                this.m_currentCapacity * QUAD_VERTEX_MEMORY_FOOTPRINT
            ),
            VERTEX_BUFFER_STRIDE
        )
        this.m_vertexBuffer.setUsage(THREE.DynamicDrawUsage)
        this.m_positionAttribute = new THREE.InterleavedBufferAttribute(
            this.m_vertexBuffer,
            4,
            0
        )
        this.m_uvAttribute = new THREE.InterleavedBufferAttribute(
            this.m_vertexBuffer,
            4,
            4
        )
        this.m_colorAttribute = new THREE.InterleavedBufferAttribute(
            this.m_vertexBuffer,
            4,
            8
        )
        this.m_bgColorAttribute = new THREE.InterleavedBufferAttribute(
            this.m_vertexBuffer,
            4,
            12
        )
        ...
        this.m_indexBuffer = new THREE.BufferAttribute(
            new Uint32Array(
                this.m_currentCapacity * QUAD_INDEX_MEMORY_FOOTPRINT
            ),
            INDEX_BUFFER_STRIDE
        )
    ...
    update() {
        if (this.drawCount > this.m_updateOffset) {
            ...
            this.m_vertexBuffer.updateRange.count =
                (this.m_drawCount - this.m_updateOffset) *
                QUAD_VERTEX_MEMORY_FOOTPRINT
    ...
    add(
        ...
        for (let i = 0; i < VERTICES_PER_QUAD; ++i) {
            ...
            const mirroredUVIdx = mirrored
                ? ((i + 1) % 2) + Math.floor(i / 2) * 2
                : i
    ...
    addToBuffer(
        ...
        for (let i = 0; i < VERTICES_PER_QUAD; ++i) {
            ...
            const mirroredUVIdx = mirrored
                ? ((i + 1) % 2) + Math.floor(i / 2) * 2
                : i
            buffer[vertexOffset + 4] =
                glyphData.dynamicTextureCoordinates[mirroredUVIdx].x
            buffer[vertexOffset + 5] =
                glyphData.dynamicTextureCoordinates[mirroredUVIdx].y
    ...
    addTextBufferObject(
        ...
        for (let i = 0; i < textBufferObject.glyphs.length; ++i) {
            ...
            const mirrored =
                buffer[srcOffset + 4] >
                buffer[srcOffset + VERTEX_BUFFER_STRIDE + 4]
            ...
            for (let j = 0; j < VERTICES_PER_QUAD; ++j) {
                ...
                this.m_positionAttribute.setXYZW(
                    ...
                    buffer[srcOffset + j * VERTEX_BUFFER_STRIDE + 3] +
                        rotSign * r
                )
                const mirroredUVIdx = mirrored
                    ? ((j + 1) % 2) + Math.floor(j / 2) * 2
                    : j
                this.m_uvAttribute.setXYZW(
                    targetOffset + i * VERTICES_PER_QUAD + j,
    ...
    private resizeBuffers(size: number) {
        ...
        const newVertexBuffer = new Float32Array(
            size * QUAD_VERTEX_MEMORY_FOOTPRINT
        )
        ...
        const newIndexBuffer = new Uint32Array(
            size * QUAD_INDEX_MEMORY_FOOTPRINT
        )
