...
import { LRUCache } from '@arca/utils'
...
export class GlyphTextureCache {
    ...
    constructor(
        ...
        this.m_cacheWidth =
            nRows * this.m_cacheHeight < capacity ? nRows + 1 : nRows
        ...
        if (
            this.m_textureSize.y > MAX_TEXTURE_SIZE ||
            this.m_textureSize.x > MAX_TEXTURE_SIZE
        ) {
        ...
        this.m_rt = new THREE.WebGLRenderTarget(
            this.m_textureSize.x,
            this.m_textureSize.y,
            {
        ...
        this.m_copyVertexBuffer = new THREE.InterleavedBuffer(
        ...
        this.m_copyGeometry.setAttribute(
            'position',
            this.m_copyPositionAttribute
        )
        this.m_copyGeometry.setAttribute('uv', this.m_copyUVAttribute)

        const copyIndexBuffer = new THREE.BufferAttribute(
            new Uint32Array(capacity * 6),
            1
        )
        ...
        this.m_clearGeometry.setAttribute(
            'position',
            this.m_clearPositionAttribute
        )
        const clearIndexBuffer = new THREE.BufferAttribute(
            new Uint32Array(capacity * 6),
            1
        )
    ...
    update(renderer: THREE.WebGLRenderer): void {
        ...
        if (willClearGeometry) {
            ...
            if (this.m_clearGeometry.index === null) {
                throw new Error(
                    'GlyphTextureCache clear geometry index is uninitialized!'
                )
            }
            this.m_clearPositionAttribute.needsUpdate = true
            this.m_clearPositionAttribute.updateRange.offset = 0
            this.m_clearPositionAttribute.updateRange.count =
            ...
            this.m_clearGeometry.index.updateRange.count =
                this.m_clearGeometryDrawCount * 6
            this.m_clearGeometry.setDrawRange(
                0,
                this.m_clearGeometryDrawCount * 6
            )
        ...
        if (willCopyGeometry) {
            ...
            const nCopies = Math.ceil(
                this.m_copyTextureSet.size / MAX_NUM_COPY_PAGES
            )
            for (let copyIndex = 0; copyIndex < nCopies; copyIndex++) {
                ...
                for (let i = 0; i < MAX_NUM_COPY_PAGES; i++) {
                    ...
                    if (pageIndex < this.m_copyTextureSet.size) {
                        this.m_copyMaterial.uniforms['page' + i].value =
                            srcPages[pageIndex]
                    }
                }
    ...
    private initCacheEntries() {
        ...
        for (let i = 0; i < this.m_cacheHeight; i++) {
            for (let j = 0; j < this.m_cacheWidth; j++) {
                ...
                this.m_entryCache.set(
                    `Dummy_${i * this.m_cacheHeight + j}`,
                    dummyEntry
                )
            }
        }
