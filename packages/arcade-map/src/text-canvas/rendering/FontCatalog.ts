...
export class FontCatalog {
    ...
    static async load(
        ...
        const replacementDirUrl = new URL(
            `${fontCatalog.name}${REPLACEMENT_PATH}`,
            url
        )
        ...
        const replacementFont = fontCatalog.fonts.find(
            (font: Font) => font.name === 'Extra'
        )
        ...
        for (const char of charset) {
            ...
            if (glyph === undefined) {
                ...
                if (glyphPromise === undefined) {
                    if (
                        !font.charset.includes(String.fromCodePoint(codePoint))
                    ) {
                        const replacementGlyph = this.createReplacementGlyph(
                            codePoint,
                            char,
                            font
                        )
                        fontGlyphMap!.set(codePoint, replacementGlyph)
                        this.m_glyphTextureCache.add(
                            glyphHash,
                            replacementGlyph
                        )
                        continue
    ...
    getGlyph(
        ...
        const fontGlyphMap = this.m_loadedGlyphs.get(
            `${font.name}_${fontStyle}`
        )
        if (fontGlyphMap === undefined) {
            return undefined
        }
    ...
    getGlyphs(
        ...
        for (const character of input) {
            const transformedCharacter = shouldTransform
    ...
    updateMemoryUsage(info: MemoryUsage) {
        ...
        let textureBytes =
            this.m_glyphTextureCache.textureSize.x *
            this.m_glyphTextureCache.textureSize.y *
            4
        ...
        for (const page in this.m_loadedPages.entries) {
            if (this.m_loadedPages.get(page) !== undefined) {
                ...
                if (loadedPage !== undefined) {
                    textureBytes +=
                        loadedPage.image.width * loadedPage.image.height * 4
                }
            }
        }
    ...
    private createReplacementGlyph(
        ...
        ;(replacementGlyph as any).isReplacement =
            UnicodeUtils.isPrintable(codePoint)
    ...
    private async loadAssets(
        ...
        const sourceGlyphData = (json.chars as SrcGlyphData[]).find(
            (char) => char.id === codePoint
        )
        const assetsPath = this.getAssetsPath(fontStyle, font)
