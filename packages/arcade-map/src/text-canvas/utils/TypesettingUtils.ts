...
export namespace TypesettingUtils {
    ...
    export function getSmallCapsScale(
        ...
        const isSmallCaps =
            smallCapsTransformations[index] &&
            fontVariant === FontVariant.SmallCaps
        return isSmallCaps
            ? glyphs[index].font.metrics.xHeight /
                  glyphs[index].font.metrics.capHeight
            : 1.0
    }
    ...
    export function updateBounds(
        ...
        if (individualBounds !== undefined) {
            if (individualBounds.array[individualBounds.offset] !== undefined) {
                individualBounds.array[individualBounds.offset].min.set(
                    minX,
                    minY
                )
                individualBounds.array[individualBounds.offset].max.set(
                    maxX,
                    maxY
                )
            } else {
