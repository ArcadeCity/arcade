...
) => [vertices[tl].pos, vertices[tr].pos, vertices[br].pos, vertices[bl].pos] as const
...
const useRectToPatch = (mesh: SkiaValue<CubicBezierHandle[]>, indices: readonly number[]) =>
  useComputedValue(() => {
...
export const CoonsPatchMeshGradient = ({
  ...
  const window = useMemo(() => Skia.XYWHRect(0, 0, width, height), [height, width])

  const clock = useClockValue()
  const image = useImage(require('./debug.png'))
  const dx = width / cols
  ...
  const meshNoise = useComputedValue(() => {
    return defaultMesh.map((pt, i) => {
      ...
      return {
        ...
        c1: add(
          pt.c1,
          vec(A * noiseC1.noise2D(clock.current / F, 0), A * noiseC1.noise2D(0, clock.current / F))
        ),
        c2: add(
          pt.c1,
          vec(A * noiseC2.noise2D(clock.current / F, 0), A * noiseC2.noise2D(0, clock.current / F))
        ),
      }
    })
  }, [clock])
  ...
  return (
    <Canvas style={{ width, height }} onTouch={onTouch}>
      ...
      {defaultMesh.map(({ pos }, index) => {
        ...
        return <Cubic key={index} mesh={mesh} index={index} color={colors[index]} />
      })}
    </Canvas>
  )
}
...
const RectPatch = ({ r, debug, lines, colors, mesh, defaultMesh }: RectPatchProps) => {
